// ============================================================================
// RegisterBloomFairyPrefabs.cs  (에디터 전용 · 1회성 셋업)
//
// ┌─ 사용법 ────────────────────────────────────────────────────────────────┐
// │  상단 메뉴  Hexiege > Setup > Register BloomFairy Prefabs (Game)  실행.     │
...
//      BloomFairy(UnitType 27) 엔트리가 있는지 확인한다.
//        · 없으면  → 리스트 끝에 새 엔트리를 추가하고 type=BloomFairy,
//                    blue=Unit_BloomFairy_Blue, red=Unit_BloomFairy_Red 를 배선한다.
...
//   Instantiate 한다. _transcendencePrefabs 에 BloomFairy 프리팹이 등록돼 있지
//   않으면 BloomFairy 생산 시 "프리팹이 설정되지 않았습니다" 에러가 나며 유닛이
...
//
// 주의:
//   · 이 스크립트는 "프리팹 등록"까지만 담당한다. 어떤 생산 건물이 BloomFairy 를
//     생산 목록에 노출하는지(생산 라인 매핑)는 ProductionPanelUI 의 씬 직렬화
//     데이터(_buildingUnitMappings)이며, 설계 확인이 필요해 여기서 배선하지 않는다.
//     (자세한 사유는 이번 작업 결과 보고 참조.)
// ============================================================================
...
{
    /// <summary>
    /// UnitFactory._transcendencePrefabs 에 BloomFairy(27) 프리팹 엔트리를
    /// 멱등하게 등록하는 1회성 에디터 스크립트.
    /// </summary>
    public static class RegisterBloomFairyPrefabs
    {
        // GameBootstrapper/UnitFactory 가 배치된 씬. 열려 있지 않으면 이 씬을 연다.
        private const string GameScenePath = "Assets/_Project/Scenes/Game.unity";

        // 등록할 BloomFairy 프리팹 경로(팀별).
        ...
        // 이번에 등록할 유닛 타입(초월계 힐러). 열거형 값은 27.
        private const UnitType TargetType = UnitType.BloomFairy;

        /// <summary>
        /// 메뉴 진입점. Game 씬의 UnitFactory 에 BloomFairy 프리팹 엔트리를 멱등 등록한다.
        /// </summary>
        [MenuItem("Hexiege/Setup/Register BloomFairy Prefabs (Game)")]
        public static void Run()
        {
            ...
            {
                Debug.LogError(
                    "[Setup] BloomFairy 프리팹을 찾지 못했습니다. 경로를 확인하세요.\n" +
            ...
            RegisterResult result =
                WireBloomFairyEntry(factory, bluePrefab, redPrefab);

            if (result == RegisterResult.FieldMissing)
            {
                // 필드명이 바뀌었거나 컴포넌트가 다른 경우 — WireBloomFairyEntry 가 에러 로그를 남긴다.
                return;
            }
        ...
        /// <summary>
        /// UnitFactory._transcendencePrefabs 에서 type==BloomFairy 엔트리를 찾아
        /// blue/red 프리팹을 멱등하게 배선한다. 없으면 리스트 끝에 추가한다.
        /// </summary>
        private static RegisterResult WireBloomFairyEntry(

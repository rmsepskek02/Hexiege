// ============================================================================
// RegisterQuakeSpiritPrefabs.cs  (에디터 전용 · 1회성 셋업)
//
// ┌─ 사용법 ────────────────────────────────────────────────────────────────┐
// │  상단 메뉴  Hexiege > Setup > Register QuakeSpirit Prefabs (Game) 실행.    │
// │  (Game 씬이 열려 있지 않아도 됨 — 없으면 스크립트가 자동으로 열고 저장한다.)  │
// └────────────────────────────────────────────────────────────────────────┘
//
// 무엇을 하는가:
//   1) Game 씬에서 UnitFactory 컴포넌트를 찾는다.
//   2) UnitFactory 의 private [SerializeField] _spiritPrefabs 리스트(=정령계 프리팹 목록)에
//      QuakeSpirit(UnitType 15, "흙정령3") 엔트리가 있는지 확인한다.
//        · 없으면  → 리스트 끝에 새 엔트리를 추가하고 type=QuakeSpirit,
//                    blue=Unit_QuakeSpirit_Blue, red=Unit_QuakeSpirit_Red 를 배선한다.
...
//
// 왜 필요한가:
//   UnitFactory 는 종족(정령계) + UnitType + 팀 조합으로 프리팹을 찾아 유닛을
//   Instantiate 한다. _spiritPrefabs 에 QuakeSpirit 프리팹이 등록돼 있지 않으면
//   QuakeSpirit 생산 시 "프리팹이 설정되지 않았습니다" 에러가 나며 유닛이 생성되지
//   않는다. 이 배선은 프리팹 GUID 를 직렬화하는 작업이라 반드시 Unity 에디터
//   런타임(SerializedObject) 에서 수행해야 한다 → 그래서 에디터 스크립트다.
//
// 멱등성:
//   여러 번 실행해도 안전하다. 엔트리가 이미 있고 blue/red 가 올바르면 no-op 이며,
//   프리팹이 누락됐거나 잘못 연결된 경우에만 해당 슬롯을 바로잡는다.
//   (조사 결과: 현재 Game 씬에는 이미 type=15 가 올바른 프리팹으로 배선돼 있어
//    이 스크립트는 실행 시 "이미 올바르게 등록됨(변경 없음)" 을 리포트한다.)
//
// ⚠️ 함정 주의(반드시 UnitFactory 로 타입 특정):
//   BuildingFactory 도 유사한 이름의 프리팹 리스트 필드를 갖는다. UnitType 15 와
//   BuildingType 15(FireSpire)는 숫자만 겹칠 뿐 전혀 다른 데이터다. 반드시
//   FindFirstObjectByType<UnitFactory>() 로 UnitFactory 를 특정해야 한다.
//
// ⚠️ enum 직렬화:
//   UnitType 은 값이 비연속(…,13,14,15,16,…)이므로 SerializedProperty 에서
//   enumValueIndex(오디널)가 아닌 intValue 로 읽고/쓴다.
//
// 주의:
//   · 이 스크립트는 "프리팹 등록"까지만 담당한다. 어떤 생산 건물이 QuakeSpirit 를
//     생산 목록에 노출하는지(생산 라인 매핑)는 ProductionPanelUI 의 씬 직렬화
//     데이터(_buildingUnitMappings)이며, 별도의 WireEarthSpiritProduction.cs 가 담당한다.
// ============================================================================
...
{
    /// <summary>
    /// UnitFactory._spiritPrefabs 에 QuakeSpirit(15) 프리팹 엔트리를
    /// 멱등하게 등록하는 1회성 에디터 스크립트.
    /// </summary>
    public static class RegisterQuakeSpiritPrefabs
    {
        // GameBootstrapper/UnitFactory 가 배치된 씬. 열려 있지 않으면 이 씬을 연다.
        private const string GameScenePath = "Assets/_Project/Scenes/Game.unity";

        // 등록할 QuakeSpirit 프리팹 경로(팀별). ⚠️ 정령계는 Units/Spirit/ 하위에 있다.
        ...
        // UnitFactory 의 정령계 프리팹 리스트 필드명(정확히 일치해야 한다).
        private const string SpiritListFieldName = "_spiritPrefabs";
        ...
        // 이번에 등록할 유닛 타입(정령계 흙 라인 3단계 착탄 즉발 AoE 탱커). 열거형 값은 15.
        private const UnitType TargetType = UnitType.QuakeSpirit;

        /// <summary>
        /// 메뉴 진입점. Game 씬의 UnitFactory 에 QuakeSpirit 프리팹 엔트리를 멱등 등록한다.
        /// </summary>
        [MenuItem("Hexiege/Setup/Register QuakeSpirit Prefabs (Game)")]
        public static void Run()
        {
            ...
            {
                Debug.LogError(
                    "[Setup] QuakeSpirit 프리팹을 찾지 못했습니다. 경로를 확인하세요.\n" +
            ...
            RegisterResult result =
                WireQuakeSpiritEntry(factory, bluePrefab, redPrefab);

            if (result == RegisterResult.FieldMissing)
            {
                // 필드명이 바뀌었거나 컴포넌트가 다른 경우 — WireQuakeSpiritEntry 가 에러 로그를 남긴다.
                return;
            }
            ...
            Debug.Log(
                "[Setup] QuakeSpirit 프리팹 등록 완료.\n" +
                $"  · UnitFactory.{SpiritListFieldName}[type={(int)TargetType} {TargetType}]: {entryState}\n" +
                $"  · Blue: {BluePrefabPath}\n" +
                $"  · Red : {RedPrefabPath}\n" +
                $"  · 씬: {saveState}");
        ...
        /// ⚠️ BuildingFactory 와 혼동하지 않도록 타입을 UnitFactory 로 특정한다.
        /// </summary>
        ...
        /// <summary>
        /// UnitFactory._spiritPrefabs 에서 type==QuakeSpirit 엔트리를 찾아
        /// blue/red 프리팹을 멱등하게 배선한다. 없으면 리스트 끝에 추가한다.
        /// </summary>
        private static RegisterResult WireQuakeSpiritEntry(
            UnitFactory factory, GameObject bluePrefab, GameObject redPrefab)
        {
            var so = new SerializedObject(factory);
            SerializedProperty listProp = so.FindProperty(SpiritListFieldName);
            if (listProp == null || !listProp.isArray)
            {
                Debug.LogError(
                    $"[Setup] UnitFactory 에 '{SpiritListFieldName}' 리스트 필드가 없습니다. " +
                    "필드명이 바뀌었는지 확인하세요.");
                return RegisterResult.FieldMissing;
            }

            // ── 기존 엔트리 탐색: type == QuakeSpirit ─────────────────────────
            // 열거형 SerializedProperty 는 내부 정수값을 intValue 로 읽고/쓴다.
            // (UnitType 은 값이 비연속이므로 enumValueIndex(오디널)가 아닌 intValue 사용.)

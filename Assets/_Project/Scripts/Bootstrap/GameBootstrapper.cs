...
namespace Hexiege.Bootstrap
{
    ...
    public partial class GameBootstrapper : MonoBehaviour, IGameServices
    {
        ...
        // ────────────────────────────────────────────────────────────────────
        // [주석 비활성화] 멀티플레이 임시 고정 root seed (무작위 맵 3단계 I)
        //
        // 🔴 왜 비활성화하는가:
        //    3단계에서 멀티의 맵은 Host 가 **로비에서** 확정해 Client 에게 통째로 보낸다
        //    (Infrastructure/Network/NetworkMapTransfer.cs). 그래서 "양쪽이 각자 같은 고정
        //    seed 로 같은 맵을 만든다"는 임시 방편이 더 이상 필요하지 않고, 실제로 멀티에서는
        //    이 상수를 읽던 분기(GameBootstrapper.Map.cs 의 CreateRootSeed)가 **호출조차
        //    되지 않는다.** 남겨 두면 다음 사람이 "멀티는 고정 seed 구나" 라고 사실과 다르게
        //    읽게 되므로 비활성화한다.
        //
        // 🔴 왜 지우지 않는가:
        //    3단계 I 는 멀티 경로 전체를 한 커밋으로 갈아끼우는 작업이라, 문제가 생기면
        //    임시 고정 seed 로 되돌릴 수 있어야 한다. **이 블록과 CreateRootSeed 안의
        //    두 줄의 주석을 푸는 것**이 되돌리기의 전부가 되도록 원문 그대로 남겨 둔다.
        //    (아래 XML 주석까지 통째로 주석 처리한 이유: 선언을 지운 채 /// 주석만 남기면
        //     "설명이 붙을 대상이 없다"는 컴파일 경고 CS1587 이 난다.)
        // ────────────────────────────────────────────────────────────────────
        //
        // /// <summary>

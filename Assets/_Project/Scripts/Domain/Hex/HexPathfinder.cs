...
namespace Hexiege.Domain
{
    public static class HexPathfinder
    {
        ...
        /// <summary>
        /// goal이 walkable하지 않을 때 사용하는 보조 경로 탐색.
        /// goal의 인접 타일 중 walkable하고 start에서 가장 가까운 타일을 목표로 FindPath 호출.
        ///
        /// 사용 시나리오:
        ///   근접 유닛이 건물(성) 타일 방향으로 이동할 때,
        ///   건물 타일은 IsWalkable=false이므로 FindPath가 null을 반환.
        ///   이 메서드는 건물 인접 walkable 타일까지의 경로를 대신 찾아줌.
        ///   호출자(UnitMovementUseCase)가 반환된 경로 끝에 goal을 추가하여
        ///   UnitView가 건물 방향으로 Lerp 이동할 수 있도록 함.
        ///
        /// 탐색 순서:
        ///   1. goal의 6방향 인접 타일 중 walkable하고 blockedCoords에 없는 후보 수집
        ///   2. start에서 HexCoord.Distance가 가장 가까운 후보 선택
        ///   3. 해당 후보로 FindPath 호출하여 경로 반환
        /// </summary>
        /// <param name="grid">탐색할 헥스 그리드</param>
        /// <param name="start">출발 좌표</param>
        /// <param name="goal">도착 좌표 (walkable하지 않은 타일, 예: 건물)</param>
        /// <param name="blockedCoords">추가로 이동 불가 처리할 좌표. null 허용.</param>
        /// <returns>인접 walkable 타일까지의 경로. 적합한 인접 타일이 없으면 null.</returns>
        public static List<HexCoord> FindPathToNeighbor(HexGrid grid, HexCoord start, HexCoord goal,
            HashSet<HexCoord> blockedCoords = null)
        {
            // goal의 인접 6방향 중 walkable하고 blockedCoords에 없는 후보 수집
            List<HexCoord> candidates = null;

            for (int i = 0; i < HexDirectionExtensions.Count; i++)
            {
                HexDirection dir = (HexDirection)i;
                HexCoord neighbor = dir.Neighbor(goal);

                HexTile tile = grid.GetTile(neighbor);
                // 타일이 없거나 이동 불가면 후보에서 제외
                if (tile == null || !tile.IsWalkable) continue;
                // 차단 좌표에 포함된 타일은 후보에서 제외 (아군 유닛 위치 등)
                if (blockedCoords != null && blockedCoords.Contains(neighbor)) continue;

                if (candidates == null) candidates = new List<HexCoord>(HexDirectionExtensions.Count);
                candidates.Add(neighbor);
            }

            // 적합한 인접 타일이 하나도 없으면 경로 없음
            if (candidates == null || candidates.Count == 0) return null;

            // start에서 HexCoord.Distance가 가장 가까운 후보를 선택.
            // 여러 후보 중 가장 가까운 곳으로 가야 이동 거리가 최소화됨.
            HexCoord bestCandidate = candidates[0];
            int bestDist = HexCoord.Distance(start, bestCandidate);

            for (int i = 1; i < candidates.Count; i++)
            {
                int dist = HexCoord.Distance(start, candidates[i]);
                if (dist < bestDist)
                {
                    bestDist = dist;
            ...
            // 이미 start가 최적 인접 타일 위에 있으면 단일 경로 반환
            if (start == bestCandidate)
                return new List<HexCoord> { start };

            // 최적 인접 타일까지 A* 경로 탐색
            return FindPath(grid, start, bestCandidate, blockedCoords);
        }

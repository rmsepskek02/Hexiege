            ...
            if (!resource.CanAfford(team, cost))
{
            ...
            //    PlaceBuilding 내부에서 타일 존재·건설 가능 여부·팀 소유 검증 실행.
            //    🔴 여기서 말하는 건설 가능은 "이동 가능(IsWalkable)"이 아니라
            //       "일반 건물을 얹을 수 있는가"다. 두 판정은 서로 다르다
            //       (건설 불가 타일은 지나갈 수는 있어도 건물은 못 짓는다).
            //       판정의 단일 소스는 HexTile 의 Accepts*** 계산 프로퍼티이며,
            //       판정식은 그쪽에만 적는다(여기 복사하면 원본이 바뀔 때 거짓이 된다).
            // ----------------------------------------------------------------

...
//
// 왜 피해 적용을 델리게이트로 주입하는가:
//   핸들러가 UnitCombatUseCase의 private 로직에 직접 접근하지 않도록 하여
//   결합을 낮춘다. 주 타깃 경로와 AoE 경로가 "동일 헬퍼"를 공유하므로
//   멀티플레이 HP 동기화(OnEntityDamaged 발행 형식)가 일관된다.
//
...
{
    /// <summary>
    /// 특수 공격 핸들러 실행에 필요한 데이터와 피해 적용 수단을 담는 컨텍스트.

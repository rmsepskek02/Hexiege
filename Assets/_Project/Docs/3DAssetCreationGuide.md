...
**Hexiege** — 헥스 타일 기반 1v1 PvP 실시간 전략 게임 (모바일, 세로 모드)
...
#### 인간계 (Human)
- **컨셉**: 총기류 기반 군대
- **유닛 예시**: 권총병, 기관총병, 저격수, 기병
- **아트 방향**: 두툼한 갑옷 + 총기 조합, 밝은 금속 + 원색 천 소재감
- **레퍼런스**: CoC 바바리안/건맨 계열

#### 정령계 (Elemental)
- **컨셉**: 9가지 원소 정령
- **유닛 예시**: 불 정령, 물 정령, 전기 정령, 빛 정령, 어둠 정령
- **아트 방향**: 반투명 에너지 몸체, 원소별 대표색 (불=주황/빨강, 전기=하늘/노랑 등), 인간형 실루엣
- **레퍼런스**: 원소 기반 캐릭터 (Genshin, League 정령 스킨 등)

#### 초월계 (Transcendent)
- **컨셉**: 의인화된 동식물
- **유닛 예시**: 곰 전사, 사자 기사, 여우 마법사, 나무 수호자, 꽃 힐러
- **아트 방향**: 동물 특징(귀/꼬리/체형) + 판타지 장비, 자연 소재감 (가죽/나무/꽃)
- **레퍼런스**: Clash Royale 동물 캐릭터, 짐승화(Kemono) 판타지 스타일
...
|----------|--------|------|
| 건물 | Castle | 양 팀 본기지 |
| 건물 | Barracks | 유닛 생산 건물 |
| 건물 | MiningPost | 채굴소 (금광 위 건설) |
| 오브젝트 | GoldMineTile | 금광 타일 오브젝트 |
| 타일 | HexTile_FlatTop | FlatTop 헥스 타일 (기본) |
| 타일 | HexTile_PointyTop | PointyTop 헥스 타일 |
| 유닛 | Unit_Pistoleer | 권총병 (인간계 1번 유닛) |

---

## 프롬포트 작성 공통 원칙

> **Gemini에게 프롬포트 작성을 요청할 때, 아래 3가지 원칙을 반드시 지정한다.**

### 원칙 1 — 프롬포트는 영어로 작성

Nano Banana(이미지 생성 AI)는 영어 프롬포트에서 최적 성능을 발휘한다.
Gemini에게 요청할 때 명시:
...
### 원칙 2 — 영어 프롬포트와 함께 한글 설명 제공

작성된 영어 프롬포트의 의미를 바로 확인할 수 있도록 한글 설명을 함께 요청한다.
Gemini에게 요청할 때 명시:

```
영어로 작성한 프롬포트의 각 항목을 한글로 설명해줘.
```

### 원칙 3 — 첨부 레퍼런스 이미지 안내

Nano Banana 또는 Meshy AI에 스타일 레퍼런스로 첨부할 이미지도 함께 안내받는다.
Gemini에게 요청할 때 명시:

```
이 에셋과 잘 어울리는 레퍼런스 이미지를 어디서 찾으면 좋을지 알려줘.
(예: 어떤 게임의 어떤 캐릭터, 어떤 키워드로 검색하면 좋은지 등)
```

### Gemini 요청 전체 템플릿 (원칙 포함)
...
> **공통 원칙 3가지** (영어 작성 / 한글 설명 / 레퍼런스 이미지 안내)를 항상 포함해서 요청한다.
> 전체 요청 템플릿은 위의 [프롬포트 작성 공통 원칙 섹션](#프롬포트-작성-공통-원칙) 참고.

### Gemini 활용 팁

- **스타일 레퍼런스 명시**: "Clash of Clans 스타일" 처럼 구체적인 게임을 언급하면 스타일이 일관됨
- **종족 컨셉 명시**: 인간계/정령계/초월계 중 어느 종족인지 설명 → 스타일 방향 통일

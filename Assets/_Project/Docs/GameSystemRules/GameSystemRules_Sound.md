...
> **미정 — 음소거 내부 구현**: "음소거"를 별도 mute 플래그로 관리할지, 슬라이더 값 0 처리로 구현할지 등 내부 구현 방식은 본 문서에서 결정하지 않는다.
> 본 규칙은 관찰 가능한 UI 동작(버튼 표출 전환, 슬라이더 색상 전환, 초기화 리셋값)만 규정한다. 구현 방식 확정 후 이 노트를 삭제하고 규칙을 보완한다.

...
### 전투 규칙 — 방어력 데미지 감쇄 (연구소 유닛 강화 시스템, 설계 확정 / 구현 예정)

> **상태: 확정 설계 / 미구현.** 아래 규칙은 연구소 기반 유닛 강화 시스템(신규 방어력 스탯)의 전투 데미지 계약이다. **아직 코드 미반영(구현 예정)** — 현재 빌드의 전투는 감쇄 없는 직격이다. 시스템 전체 계약(트랙 구조·연구소 운영·비용·×10 스케일)은 `GameSystemRules_Upgrade.md` 참조.

연구소 강화의 신규 스탯 **방어력**은 받는 피해를 비율로 줄인다. 모든 유닛은 방어력 0에서 시작하며(연구 전에는 감쇄 0% → 기존과 완전히 동일), 오직 연구로만 올린다. 방어력 감쇄는 유닛이 피해를 받는 **최종 데미지 지점 전부**(직격·스플래시·DoT 틱)에 동일하게 삽입되어야 한다.

**규칙 44. 방어력 데미지 감쇄 공식 — 모든 최종 데미지 지점 일괄 적용 (설계 확정 / 구현 예정)**
전투 최종 데미지를 피격 대상의 방어력으로 비율 감쇄한 뒤 적용한다.
- **공식:** `데미지 = Max(1, Round(공격력 × (1 − 방어력 / (방어력 + 120))))`. K = 120, 방어력 트랙 Lv0~5 = 0/8/16/24/32/40.
- **최소 데미지 1 보장(floor) 필수.** 감쇄율 하드캡 60~65%를 미래 확장 안전장치로 코드에 포함(현 Lv5는 25%로 미도달).
- **삽입 지점 = 모든 최종 데미지 수렴점 일괄** — 직격(`ApplyDamageToVictim`), 스플래시(QuakeSpirit `ApplyFixedDamageToVictim` 경유 등), DoT 틱(`ApplyBlastDot`/`ApplyInfernoDot`, 규칙 40·42) 전부. 감쇄 계산은 순수 함수 헬퍼(Domain 권장)로 두어 모든 지점이 동일 공식을 쓴다.
- **방어력은 피격 대상 팀의 연구 레벨로 결정** — 데미지 계산 시 대상(`UnitData`)의 팀·그룹으로 방어 레벨을 조회해 감쇄를 적용한다((B) 실시간 배율, `GameSystemRules_Upgrade.md` 규칙 4).
- **공격력 연구와의 관계(규칙 6, Upgrade):** 공격력 배율은 공격력을 쓰는 피해(직격·스플래시·파도)에만 적용하고 **고정 DoT 틱값에는 미적용**이지만, **방어력 감쇄는 DoT 틱에도 적용**한다(공격력 배율 미적용 ≠ 방어력 감쇄 미적용).
- **하위호환:** 방어력 0이면 `방어력/(방어력+120)=0` → 감쇄 0% → `Max(1, Round(공격력×1))=공격력`으로 연구 이전의 모든 전투가 회귀 없이 기존과 동일하다. 이 하위호환 덕에 in-place 수정이 안전하나, 데미지 공식은 전 전투에 영향을 주므로 구현 후 실기에서 직격·스플래시·DoT·타워 등 모든 경로의 회귀 여부를 확인한다.
- **타워 데미지 경로:** 방어 타워가 유닛에게 주는 피해도 동일 감쇄 대상인지는 구현 시 확인(유닛 대상 피해면 동일 헬퍼 사용, `GameSystemRules_Buildings.md` 규칙 9 서버 권위 데미지).

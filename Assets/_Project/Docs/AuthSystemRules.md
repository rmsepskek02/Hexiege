...
| 닉네임 설정 화면 | 이메일 인증 후 최초 로그인 성공 직후, 또는 Google 최초 로그인 후 |
...
익명 계정은 닉네임 입력 없이 `게스트_임의문자숫자` 형식으로 자동 생성된 표시 이름을 사용한다.
...
**규칙 3. 최초 로그인 감지 및 닉네임 설정**

Google 로그인 성공 후, 해당 플레이어의 Cloud Save에 nickname 필드가 없거나 비어 있으면
"최초 로그인"으로 판정하고 NicknameSetupView를 표시한다.
닉네임 설정 또는 스킵 완료 후 Lobby 씬으로 이동한다.
...
닉네임은 회원가입 시점이 아니라, 이메일 인증 후 **최초 로그인 성공 직후**에 NicknameSetupView에서 수집한다.
(회원가입 시점에는 아직 UGS 로그인 세션이 없어 Cloud Save 저장이 불가능하므로, 세션이 존재하는 첫 로그인 직후로 통일한다.)
...
회원가입 완료 즉시 이메일 인증 메일을 발송하고, 곧바로 이메일 인증 화면으로 이동하여 인증 완료를 유도한다.
회원가입 시점에는 닉네임을 수집하지 않는다. (이 시점에는 UGS 세션이 없어 Cloud Save 저장이 실패하기 때문)
전체 흐름: **가입 → 이메일 인증 → (최초) 로그인 → NicknameSetupView(닉네임 설정/스킵) → 로비**. (이메일 인증 규칙 참조)
...
닉네임은 동일한 이름이 허용되며, 4자리 숫자 코드를 붙여 플레이어를 구분한다.
(예: 전사#4729, Hero#0012)
코드는 서버가 자동 할당하며 같은 닉네임 내에서 중복되지 않는다.
표시 형식: {닉네임}#{4자리코드}
...
| 로그인 유형 | 수집 흐름 |
|------------|-----------|
| 이메일 (최초 로그인) | 가입 → 이메일 인증 → 로그인 성공 + Cloud Save에 닉네임 없음 감지 → NicknameSetupView 표시 → (닉네임 설정 또는 스킵) → Lobby 씬 |
| Google 로그인 (최초) | 로그인 성공 + Cloud Save에 닉네임 없음 감지 → NicknameSetupView 표시 → (닉네임 설정 또는 스킵) → Lobby 씬 |
| 익명 로그인 | NicknameSetupView 없음, `게스트_임의문자숫자` 자동 생성 |

**규칙 4. 닉네임 스킵**

스킵 시 아래 형식으로 자동 생성된다.

| 로그인 유형 | 자동 생성 형식 |
|------------|---------------|
| Google 로그인 스킵 | `구글_임의문자숫자` |
| 이메일 로그인 스킵 | `사용자_임의문자숫자` |
| 익명 | `게스트_임의문자숫자` (스킵 없이 자동 생성) |

**규칙 5. 닉네임 변경**

최초 1회 변경은 무료다.
이후 변경은 인앱 결제 항목을 통해서만 가능하다.
무료 변경 사용 여부는 UGS Cloud Save의 hasUsedFreeNicknameChange 필드로 관리한다.

**규칙 6. 닉네임 저장**

닉네임과 코드는 UGS Cloud Save에 저장한다.
필드명: nickname (string), code (string, 4자리 숫자)

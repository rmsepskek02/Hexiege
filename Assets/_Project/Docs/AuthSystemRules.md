...
| 닉네임 설정 화면 | 이메일 회원가입 완료 직후, 또는 Google 최초 로그인 후 |
...
닉네임은 회원가입 완료 직후 NicknameSetupView에서 별도로 수집한다.
...
회원가입 완료 즉시 이메일 인증 메일을 발송하고, 이메일 인증 화면 이동 전에 먼저 NicknameSetupView를 표시하여 닉네임을 수집한다.
닉네임 설정 또는 스킵 완료 후 이메일 인증 화면으로 이동하여 인증 완료를 유도한다. (이메일 인증 규칙 참조)
...
| 로그인 유형 | 수집 흐름 |
|------------|-----------|
| 이메일 회원가입 | 가입 성공 → NicknameSetupView 표시 → (닉네임 설정 또는 스킵) → EmailVerifyView |

...
### 유닛 스탯 (인간계 — 확정, 2026-03-13)

| 항목 | 권총병 (Pistoleer) | 돌격소총병 (Assault) | 저격총병 (Sniper) |
|------|-------------------|---------------------|-----------------|
| HP | 30 | 50 | 30 |
| 공격력 | 3 | 6 | 20 |
| 사거리 (float 타일) | 0.5 | 1.0 | 3.0 |
| 사거리 (world units 임계값) | 0.533 | 0.966 | 2.698 |
| 이동속도 (초/칸) | 1.0 | 1.0 | 0.5 |
| 생산시간 | 5초 | 10초 | 15초 |
| 골드비용 | 50 | 100 | 200 |
| 인구 소비 | 1 | 1 | 1 |
| UnitType enum | Pistoleer = 0 | Assault = 1 | Sniper = 2 |

**사거리 계산식**: `임계값 = 사거리 × TileHeight(0.866) + 0.1`

**각 유닛 역할**:
- **권총병**: 대량 생산 러시 전략의 주력. 사거리 0.5 — 타일 이동 중 50% 지점에서 근접 교전 시작. 낮은 DPS로 수적 우위 필요
- **돌격소총병**: 범용 주력 유닛. 사거리 1.0 — 인접 타일 진입 직전 교전. 체력과 공격력이 균형잡혀 안정적
- **저격총병**: 고가 특수 유닛. 사거리 3.0 — 약 3타일 전방에서 원거리 교전. 높은 단발 화력이나 이동 속도가 절반으로 느리고 비용이 높아 소수 정예 운용 필요

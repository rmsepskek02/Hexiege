...
| 버전 | 날짜 | 변경 내용 |
|------|------|-----------|
| 0.7.0 | 2026-02-15 | 금광+자원 시스템: 금광 타일(HasGoldMine, 이동 불가), 채굴소 금광 전용 배치(인접 팀 타일 조건), 시작 채굴소 자동 건설, 중립 금광 2개 배치, 기본 수입 0, 채굴소 수입 10골드/초. 승리/패배 시스템: Castle 파괴 감지(GameEndUseCase), 승리/패배 팝업+게임 일시정지+다시하기(GameEndUI) |

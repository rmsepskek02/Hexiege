...
> **상태: ✅ 구현 완료 (2026-06-23)** — 본 계획대로 약 30개 파일의 히스토리성 주석/폐기 코드 제거, `_enableAI` 블록(주석 처리 코드+메모)·`_confirmPopup` 전환 설명 블록 제거, `NetworkGameFlow.cs` 빈 섹션 헤더 제거, `GameBootstrapper.Setup.cs` 중복 RaceId 배열 → 지역 변수 통합 적용. 런타임 동작 변경 없음. 미래 사용 의도 주석 미발견(플래그 없음).

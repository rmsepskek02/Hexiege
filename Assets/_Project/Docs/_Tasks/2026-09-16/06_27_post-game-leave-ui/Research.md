...
## 8. 구현 후 대조 — 이 문서의 서술 중 달라진 것 (2026-09-21 추가)

> **[이 절은 append 다 — 위 §1~§7 의 조사 기록은 한 글자도 고치지 않았다.]**
> 조사 문서는 **그 시점의 코드 상태를 적은 기록**이므로 소급 수정하지 않고, **어디가 더 이상 현재가 아닌지만** 여기에 모아 적는다.

**무엇을 하는 절인가 (자연어 설명 — CLAUDE.md 규칙 13)**
이 문서는 2026-09-16 에 *"고치기 전의 코드가 어떤 상태인가"* 를 적은 조사 기록입니다.
그 뒤 **단계 1·2·3 이 실제로 구현되면서 여기 적힌 몇 줄이 더 이상 현재가 아니게 됐습니다.**
원래 문장을 지우면 *"무엇을 왜 고쳤는지"* 를 나중에 알 수 없으므로, **원문은 그대로 두고 이 표에 바뀐 자리만 모읍니다.**

### 8-1. 더 이상 현재가 아닌 서술

| 이 문서의 자리 | 2026-09-16 조사 시점 | 2026-09-21 현재 (직접 실측) |
|---|---|---|
| §2-1 「카운트다운 길이」 | 코드 기본값 `30f` · `Game.unity` 직렬화 값 `30` | **둘 다 `60`** — 코드 `60f` · 씬 `60`(규칙 D-4, 단계 3) |
| §2-1 「재경기 요청 시」 · §2-1 아래 🔴 불릿 | `_restartButton.interactable = false` 와 **`_backToLobbyButton.interactable = false` 를 둘 다 실행**한다 / *"「갇힘」의 직접 원인은 그 한 줄"* | 🔴 **로비 버튼 쪽 한 줄이 제거됐다 — 잔존 0건**(규칙 D-3, 단계 2). 남은 것은 `_restartButton` 쪽 하나뿐이며 **그것은 규칙대로다.** ⚠️ **그 리스너에 `StopCountdown()` 이 없다는 서술은 그대로 유효하다** — 단계 7 의 몫이다 |
| §2-2 「공용 팝업 API」 | `ShowConfirm` **하나뿐**이며 `ShowAlert` 는 **없다** | **`ShowAlert` 가 생겼다** — `IUIManager` 선언 + `UIManager` 위임 + `ConfirmPopup.ShowAlert(title, message, buttonLabel, onClick)`(단계 1) |
| §2-2 「`ConfirmPopup` 의 직렬화 필드」 · 「타이틀」 · 「버튼 수」 | 필드 6개 · 타이틀 **없음** · 버튼 **2개 고정** | **`_titleText` 가 추가돼 7개**, 제목과 취소 버튼을 `SetActive` 로 여닫아 **1버튼 모드가 가능**해졌다 |
| §2-2 아래 🔴 불릿 | *"규칙 D-5 의 「타이틀 + 본문 + 버튼 1개」는 지금의 `ConfirmPopup` 구조로는 만들 수 없다"* | **그 구조로 만들 수 있게 고쳤다.** 🔴 **다만 코드만으로는 성립하지 않는다 — `ConfirmPopup.prefab` 의 Panel 에 VerticalLayoutGroup 이 있어야 한다.** 없으면 숨긴 제목 자리가 빈 공간으로 남아 **제목 없는 기존 팝업의 본문이 아래로 밀린다.** 그 전제는 2026-09-21 에 프리팹에 반영됐다 |
| §2-2 「`ShowConfirm` 실호출처 5건」 | 5건 | **변경 없이 5건 그대로**(직접 실측). **이 줄은 달라지지 않았다는 것 자체가 결과다** — 단계 1 의 완료 판정이었다 |

### 8-2. ✅ §5-3 이 「고치지 않고 적어 둔다」고 한 자리 하나가 해소됐다

§5-3 표의 두 번째 행 — `ROADMAP.md` 2026-09-14 (3차) 행이 *"`RestoreRematchButton()` 은 `OnNetworkRematchDeclined` 구독 한 곳에서만 호출된다"* 고 적은 것 —
은 **2026-09-21 문서 반영 회차에 그 행에 정정이 덧붙었다.** 실측대로 **2곳**(`OnNetworkRematchDeclined` · `OnNetworkRematchMapFailed`)이며,
🔴 **`ROADMAP.md` 의 원문은 지우지 않고 맨 앞에 정정 블록을 덧붙이는 방식**을 썼다(`.claude/MEMORY.md` B-7).
**이 문서(§5-3)의 서술은 그대로 유효하다** — 그때 「고치지 않겠다」고 한 판단도, 실측값도 맞았다.

### 8-3. ⚠️ 그대로 유효한 서술 (함께 부정하지 말 것)

- **§2-3 네트워크 전부** — `BackToLobby()` 에 통보 단계가 없다 · `ForceWin` 의 `_announced` 가드 · `ReconnectionHandler` 의 죽은 30초 대기 · `NetworkMapTransfer` 의 `Disconnected` 처리. **단계 5·6·9 가 미착수라 하나도 바뀌지 않았다.**
- **§2-4 전송 계층** — `m_DisconnectTimeoutMS` 는 **두 씬 모두 여전히 30000** 이다(단계 4 미착수).
- **§2-1 의 구독 목록 6개** — 「상대가 나갔다」 채널은 **아직 없다**(단계 5).
- **§4 시스템 한계** — 구현으로 없앨 수 없는 성질이라 그대로다.
- **§6 미결 항목** — **14-4(강제 실패 플래그)는 여전히 미착수**이고, **규칙 M-3·M-4 의 문구·버튼 라벨도 여전히 미정**이다(다만 *"임시 문구로 먼저 간다"* 가 사용자 확정이라 **단계 10 의 착수 조건은 아니다** — `Plan.md` §9 확정 블록).

> **결과의 단일 소스는 `Plan.md` §10 실행 기록 표와 §11 이며, 검증 경계는 `PROJECT_STATUS.md` 「⚠️ 부분 구현 (2026-09-21)」 문단이다.** 여기에 옮겨 적지 않는다.

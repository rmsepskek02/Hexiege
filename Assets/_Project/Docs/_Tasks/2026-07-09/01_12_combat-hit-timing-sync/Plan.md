...
## 수정 3 (2차 검증 후속, 2026-07-12) — 승인 완료

수정 1·2 적용 후 클라이언트 로그(Research 8절 참조)를 분석한 결과, 특정 유닛(Assault 62)이 75초간 서버는 전투 중인데 클라이언트만 Walk 상태에 갇혀 공격 모션이 재생되지 않는 기존 버그가 드러났다(승패 무관 시각 버그). 이 버그의 핵심은 서버가 "클라이언트가 Attack 루프를 벗어난 사실"을 알면서도 `_combatAnimationSent` 가드 때문에 `StartCombatClientRpc`를 재전송하지 못하는 데 있다. 이를 해소하기 위한 후속 수정이다. 코드 수정은 **game-programmer** 에이전트가 수행한다.

### 수정 3 [필수] — Walk RPC 전송 시 StartCombat 재전송 가드 해제

**대상**: `Infrastructure/Network/NetworkCombatController.cs`

- `OnUnitWalkStarted` 구독 핸들러(`StartWalkAnimationClientRpc`를 전송하는 지점)에서, 해당 유닛을 `_combatAnimationSent` 집합에서 **제거**한다.
- **근거**: Walk RPC 전송은 "클라이언트가 Attack 루프를 벗어나 Walk로 전환됨"을 서버가 아는 **유일한 시점**이다. 이 시점에 가드를 해제하면 이후 전투 재진입 시 `StartCombatClientRpc` 재전송이 보장되어, 클라이언트가 Walk에 갇히는 경쟁 조건이 사라진다. 비전투 유닛(순수 이동)의 Walk에 대해서는 집합에 원소가 없어 Remove가 no-op이므로 부작용이 없다.
- **근거 규칙**: 신설 규칙 초안 U-19(피격 표현 큐)의 전제 — 공격자의 로컬 타격 프레임(`OnAttackHit`)이 존재해야 피격 표현이 정상 방출된다 — 를 보장하기 위한 기존 전투 상태 RPC 설계 보완이다.
- **규칙 신설 필요 여부**: 신규 규칙 초안 **[U-21]** 한 줄 문안을 추가한다. "클라이언트가 Attack 루프를 이탈(Walk RPC 전송)한 시점에 서버는 해당 유닛의 StartCombat 재전송 가드를 해제한다." (승인·검증 완료 후 document-manager가 GameSystemRules_Units에 반영)

### 재검증(3차) 계획

수정 3 적용 후 동일한 임시 계측 로그로 재테스트한다. 판정 기준은 다음과 같다.
- 타임아웃 방출이 **특정 유닛에 집중되지 않고**, 0 또는 소수로 감소할 것.

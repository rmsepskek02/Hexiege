...
**[🔴 2026-09-15 추가 — 위 절은 한 글자도 고치지 않고, 그 뒤에 일어난 일만 덧붙인다]**

> - ✅ **바로 위 「해소되지 않은 것」이 해소됐다** — **재경기 맵이 구현됐고 2026-09-15 실기에서 멀티 9판(재경기 7회 연속) · 싱글 2판이 전부 성공**했다. 실행 기록·실기 결과·남은 항목의 **단일 소스는 같은 폴더 `Plan.md` 의 §13 과 그 아래 「실행 결과 상세 (2026-09-15 추가)」**이며, 수치를 여기에 옮겨 적지 않는다.
> - ⚠️ **위 표의 §12 행이 말한 「남은 3건」은 이제 2건이다** — **§12-5(테스트 모드 삭제 순서)가 결정되고 실행까지 끝났다**(아래 §12-5 에 덧붙였다). **남은 것은 §12-1 · §12-3 이며 실기 완료 시점에도 열려 있다.**
> - 🔴 **그 외의 절별 유·무효 판정은 이번 구현으로 바뀌지 않았다.** 이 문서는 여전히 **조사 기록**이며, 구현 결과는 `Plan.md` 가 들고 있다.
...
> **[🔴 2026-09-15 — 위 표는 한 칸도 고치지 않는다. 구현과 2026-09-15 실기로 상태가 달라진 행만 아래에 적는다]**
>
> - **2번(「전투 → 전투」 재로드 생존)** — 🔴 **이 질문은 답해진 것이 아니라 없어졌다.** 사용자가 §4-2 의 후보 **ⓑ(재경기마다 전송 객체를 다시 스폰)**로 확정했으므로 **후보 ⓐ 의 성립 여부가 이 값에 걸려 있지 않다.** 실기 9판이 그 방식으로 동작했다. 위 칸의 *"§4-2 후보 ⓐ 의 성립 여부가 여기에 걸려 있다"* 는 **더 이상 이 작업의 이유가 아니다**(`Plan.md` §4 · §10-3).
> - **3번(Client 쪽 로그)** — **확보됐다.** 2026-09-15 실기에서 **에디터가 Client, 실기기가 Host** 였고 양쪽 로그가 남았다(`Plan.md` 「실기 테스트 결과 상세」의 로그 경로 3개). ⚠️ **다만 그 파일들은 이 문서를 갱신하는 시점의 작업 트리에는 없어 직접 열어 대조하지 못했다 — 「없다」가 아니라 「이 체크아웃에서 확인 불가」다.**
>   - **[✅ 2026-09-15 확인 — 위 줄은 그대로 두고 이유를 덧붙인다]** 🔴 **로그 3개는 커밋 `4832a1b` · `44caebe` 로 저장소에 들어와 있다.** 이 문서를 갱신한 워킹트리의 HEAD 가 **`20ba0ab`** 이라 그 뒤 커밋의 파일이 체크아웃돼 있지 않았을 뿐이며, 메인 세션은 **fetch 한 git 오브젝트에서 직접 읽어** 분석했다. **즉 3번 항목의 「Client 로그는 이 저장소에 없다」는 조사 시점 서술은 이제 성립하지 않는다** — 열어 보려면 `4832a1b` 이후를 체크아웃하면 된다.
> - **5번(팀·종족)** — 🔴 **관측만 늘고 이유는 그대로 미확인이다.** 2026-09-15 실기에서 **에디터(Client) 8판 전부 `Team=Red, BlueRace=0, RedRace=1`** 로 유지됐다. **`NetworkGameFlow` · `LocalPlayerTeam` 은 이번에도 열지 않았다** — *"현재 유지되고 있다는 관측은 있으나 그 이유는 확인하지 않았다"* 는 위 칸의 서술이 **지금도 그대로 맞다.**
> - **7번(Unity 산출물)** — **그대로 「확인 불가」다.** 다만 간접 증거가 늘었다: 2026-09-15 실기에서 재경기마다 전송 객체가 스폰·디스폰됐다.
> - **1 · 4 · 6 · 8 번은 이번 작업으로 달라진 것이 없다.** 특히 **8번(폴백 발화 빈도)** 은 멀티 9판 · 싱글 2판이 **전부 `UsedFallback=False`** 여서 **표본만 늘었을 뿐 여전히 재지 않았다.**
...
> **[✅ 2026-09-15 해소 — 🔴 위 원문은 한 글자도 지우지 않고 결과만 덧붙인다]**
> **답은 「삭제가 먼저」였고, 계획 단계를 넘어 실제로 실행까지 끝났다.** 순서는 `Plan.md` §5-1 의 **T1 → T2 → T3 → T4 → A → B → C → D** 이며, 커밋은 **T1 `0608f85` · T2·T3 `8c83317` · A~D `20ba0ab`** 다(커밋 해시는 메인 세션 인계값 — 규칙 5 로 git 명령을 쓸 수 없어 이 문서가 재검증하지 않았다).
> 🔴 **위 첫 불릿의 전제도 함께 사라졌다** — *"규칙 14 가 `NewMap` 의 초기 골드를 「새 맵 준비 시점의 `GameConfig.MapTestModeEnabled`」로 확정하라고 명시적으로 요구한다"* 는 문장은 **T1 에서 규칙 14 의 그 조항이 제거되면서 더 이상 성립하지 않는다.** 재경기의 초기 골드도 **광산 수 표 하나**를 따른다. **제거 근거의 단일 소스는 `GameSystemRules/GameSystemRules_RandomMap.md` 규칙 3 의 개정 블록**이며 여기에 옮겨 적지 않는다.
> ⚠️ **그래서 §12 의 남은 미정은 3건이 아니라 2건이다** — **§12-1**(모달 중 countdown)과 **§12-3**(규칙 16 해석)만 남는다. **두 건 다 2026-09-15 실기 완료 시점에도 열려 있다** — §12-1 은 사용자가 「맵 작업 완료 후」로 확정한 UI 묶음에 들어 있고(`Plan.md` §7-3 · §12 의 3번 행), §12-3 은 `Plan.md` §14-2 가 그대로 열어 두고 있다.

...
# 12. 실기 관측 결과 (2026-08-19 추가)

> **§11-3 2번이 *"실기 동작 테스트를 하지 않았다"* 로 남겨 두었던 항목의 후속이다.**
> 근거: `_Logs/_editor/2026-08-19/RuntimeLog.txt`(704줄). 이 로그는 **후속 task `20_07_log-coverage-and-rpc-guard` 가 연구 흐름에 로그 4줄을 넣은 뒤**의 것이라, ① 이 실제로 도는 모습을 처음으로 파일에서 볼 수 있게 되었다.

## 12-1. 확인된 것 — **정상 경로가 로그로 관측되었다**

| 항목 | 관측 |
|---|---|
| 서버 완료 훅 → 브로드캐스트 | `서버: 연구 완료 — 레벨 브로드캐스트` **6건**(445·455·466·592·596·603행) |
| 착수 ↔ 완료 대응 | `서버: 연구 착수 성공` **6건**(397·408·420·518·525·533행)과 **1:1** — **누락 0건** |
| 레벨 상승 | `Level=1` → `Level=2` 로 정상 진행 |
| 스폰 시 서비스 부재 경고 | `NetworkControllerSpawnedWithoutGameServices` **0건** |

## 12-2. ⚠️ 이것은 **"수정이 동작한다"는 증명이 아니다** (규칙 10)

**§8-2 가 적어 둔 한계가 그대로 유효하다.**

- ① 이 고친 버그는 **`_services` 를 스폰 시점에 얻지 못하는 스폰 레이스**에서만 발현하는 **간헐 버그**다.
- 이번 회차는 그 레이스가 **발생하지 않았다**(`NetworkControllerSpawnedWithoutGameServices` **0건**). 즉 **`EnsureUpgradeSubscription()` 의 지연 구독 경로가 실제로 필요해진 상황 자체가 없었다.**
- 따라서 이번 로그가 말하는 것은 정확히 여기까지다 — **버그가 재현되지 않았고, 정상 경로는 로그로 확인되었다.**
- **"안 났으니 고쳐졌다"로 결론 내리지 않는다.** §11-3 2번의 판단 기준(*"`NetworkUpgradeController` 이름으로 스폰 시 서비스 부재 로그가 찍힌 판에서 §8-1 4~5번이 정상일 때만 직접 증거가 된다"*)은 **아직 충족되지 않았다.**

## 12-3. 그대로 남는 단서

§11-3 의 **1(카메라 코드 최종 삭제 미완) · 3(`_services` 재경기 인스턴스 위험) · 4(§2-7 ① 절반만 확인) · 5(§9 범위 밖 6항목)** 는 **이번 관측으로 달라진 것이 없다.**
§11-3 **2** 만 *"실기 테스트 미수행"* → **"정상 경로는 관측되었으나 버그 재현은 여전히 미달성"** 으로 갱신된다.

...
- 도끼병은 타겟에 따라 방향이 바뀐다. **AoE는 타겟을 향한 방향을 기준으로 5타일을 정한다.**
- `ExecuteAttack`이 데미지 직전 `attacker.Facing = FacingDirection.FromCoords(attacker.Position, target.Position)`로
  **타겟 방향으로 갱신**하므로, 그 이후 `Facing.Opposite()`(등 뒤)를 뺀 5타일을 계산하면
  **타겟은 항상 전방 5타일 안에 포함**된다. 이동 중의 옛 방향이 아니라 타겟 방향이 기준.

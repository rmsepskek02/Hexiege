...
## 6. Phase 2(타입 C) 구현 후 사실 보강 (2026-08-05, 실기+멀티 PASS)

Research 본문(§1~5)은 착수 전 조사 상태를 보존한다. 아래는 Phase 2(타입 C — 전역 상태변경) 구현·검증으로 확정된 사실만 보강한다(상세 결과는 Plan §10).

- **§3 "핵심 기술 제약" 검증 결과:** `UnitData` 스탯이 readonly라 상태변경은 원본을 못 바꾼다는 제약이 그대로 확인됨 → 예정대로 **유효 스탯 접근자(`EffectiveAttack`/`GetUnitMoveSpeedMultiplier`)에 상태 배율을 곱연산 합성**하고, 원본 미변경으로 처리. 신규는 **공격 게이트 `CanAttack`**(빙결/기절)과 **이동 코루틴의 매 프레임 배율 재조회**(빙결=0 즉시 정지, 둔화=라이브)로 국한됨. `DetectRange` 변경은 이번 플레이스홀더 범위에서 미사용.
- **§5-1 "빙결 배율 0을 A* 이동 코루틴이 오처리" 우려의 실제 해소:** 경로 발급 시 1회 캡처되던 이동배율을 **매 프레임 재조회**로 바꿔 배율 0/부분배율이 즉시 반영되도록 했고, 빙결은 추가로 `Animator.speed=0`(애니 프레임 고정)+`UnitAnimState.Frozen` 클라 동기화로 표현. **전투 종료 후 정렬 Lerp 구간만 캡처값(라이브 아님) 미세 잔여.**
- **회복 경로:** 예정대로 상태 시스템에 미저장하고 기존 HoT(`ApplyTimedEffect` Heal)를 재사용(HP는 `NetworkHealthSync` 동기화) — 이중 힐 방지 위해 멀티 상태 브로드캐스트에서 제외.
- **정리:** 구현 중 넣었던 진단 로그 코드는 LogRules(RuntimeLogger 파일 기록·raw Debug.Log 금지) 준수 위해 제거했고, 그 과정에서 `IRuntimeLogSink`/`RuntimeLoggerSink`도 삭제(상시 기능 아님). 로그 파일은 LogRules대로 보존.

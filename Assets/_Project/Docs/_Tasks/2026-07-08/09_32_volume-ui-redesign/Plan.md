...
## 결정 포인트 (사용자/기획 확인 필요 — CLAUDE.md 규칙 12)

1. **슬라이더 색상 적용 대상**: Fill 이미지만 채색할지, 트랙 배경까지 포함할지. (Plan 기본값: Fill만)
2. **초기화 처리 방식**: UI가 슬라이더 값 1.0 세팅으로 처리(현 계획) vs AudioManager에 `ResetVolumes()` 메서드 신설. (Plan 기본값: UI 슬라이더 세팅 — 자동 언뮤트 재사용)
3. **인게임 프로필 버튼**: 생성만 하고 무반응 유지 여부. (Plan 기본값: 생성·무반응)

위 3개는 구현 착수 전 확정하면 game-programmer 위임이 매끄럽다. 별도 이견이 없으면 Plan 기본값으로 진행한다.

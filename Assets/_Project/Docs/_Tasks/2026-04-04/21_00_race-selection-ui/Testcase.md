...
### SINGLE-6: 커스텀 게임 화면 전환 시 종족 선택 UI 항상 표시
...
**기댓값:**
- 커스텀 게임 화면으로 전환되더라도 종족 선택 UI는 계속 표시된다.
- 메인 화면으로 돌아왔을 때도 종족 선택 UI가 표시되며 "정령"이 유지된다.
...
### SINGLE-7: 실기기에서 캐릭터 이동 시 잔상 없음

**전제:** Android 실기기에서 로비 앱이 실행된 상태. 전투 탭 메인 화면.

**동작:**
1. 오른쪽/왼쪽 화살표를 눌러 종족을 여러 번 전환한다.
2. 캐릭터가 캐러셀 애니메이션으로 이동하는 동안 화면을 관찰한다.

**기댓값:**
- 캐릭터가 이동하는 동안 이전 위치에 잔상(ghosting)이 남지 않는다.
- 콘솔에 RenderPass 관련 에러가 발생하지 않는다.

...
- 2회: "자연" 캐릭터 오브젝트가 표시되고 종족명 텍스트가 "자연"으로 변경된다.
- 3회: 다시 "인간"으로 순환된다.
...
**기댓값:**
- "자연" 캐릭터 오브젝트가 표시되고 종족명 텍스트가 "자연"으로 변경된다 (역방향 순환).
...
**전제:** Host(인간 선택)와 Client(자연 선택)가 각각 로비에서 종족을 선택한 상태.
...
**기댓값:**
- 게임이 정상적으로 시작된다 (오류 없음).
- (다음 작업에서 확인 예정) Host: 인간 종족, Client: 자연 종족으로 게임이 진행된다.
...
**정적 분석:**
- `CmdPrev` → `(current + 3 - 1) % 3` 계산: 0→2(자연) 순환. Human에서 왼쪽 → Nature 기댓값과 일치.

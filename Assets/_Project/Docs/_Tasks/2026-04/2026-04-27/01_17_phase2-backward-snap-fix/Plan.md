...
3. **12방향 각도 기반 슬롯 규칙**에 의거하여 적을 추적하는 근접 유닛들이 분산되며 접근한다.
   - 공격 목표를 중심으로 360도를 12등분(30도 간격)하여 12개의 자리를 만든다.
   - 각 유닛은 그 중 하나의 자리를 배정받아 이동한다.
   - 자리는 항상 공격 목표 바로 주변에 있으므로, 어떤 자리를 배정받더라도 유닛은 앞(목표 방향)으로 이동하게 된다.
   - 기존 방식(인접 타일 중심 기반 18슬롯)은 유닛이 앞으로 이동한 상태에서 새 타겟이 생기면 출발 타일(뒤쪽)이 가장 가까운 슬롯으로 배정되는 문제가 있었다. 12방향 방식은 이 문제가 구조적으로 발생하지 않는다.

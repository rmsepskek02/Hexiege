...
**갱신:** 2026-08-12 — 실기 검증 중 발견된 **중첩 해소 규칙 위반 버그 수정**을 **§9-4**에 기재(커밋 `be17148`). 최상단 「기존 로직 제거 여부」에 **③ 제거 3건**을 추가(누적 **5건**), **§10 완료 판정을 실기 검증 결과로 갱신**
**현재 상태:** **코드·프리팹 구현 완료 / 에디터 싱글플레이 실기 검증 완료 · 멀티 미검증 · VFX·아이콘 미제작 · 밸런싱 미확정** (2026-08-12)
...
> **요약 (2026-08-12 갱신):** **계획 단계에서는 제거 0건**이었으나, 구현 후 보완 수정에서 **직렬화 필드 제거 2건**(2026-08-11)이,
> 이어서 중첩 해소 버그 수정에서 **제거 3건**(2026-08-12)이 발생해 **누적 5건**이 되었다.
> 아래 ①이 계획 시점 내용(원문 유지), ②·③이 추가된 내용이다.
...
### ③ 중첩 해소 버그 수정 — **제거 3건** (2026-08-12 추가 / 상세는 §9-4)

실기 검증에서 드러난 **규칙 13(중첩 금지) 위반**을 고치면서 아래 3건을 **주석 처리가 아니라 제거**했다.
셋 다 `MistShrineUseCase.cs` 한 파일 안에 있으며, **버그의 원인이거나 그 배관**이다.

| 제거 대상 | 성격 | 주석 처리가 아니라 **제거**를 택한 근거 |
|------|------|------|
| `_firingBuffer` (이번 틱 발화분 임시 버퍼) | **버그의 원인 자체** — 중첩 해소를 "이번 프레임에 발화한 물안개끼리"로 한정시켜, 버퍼에 늘 하나뿐인 상황에서 해소 코드가 **돌 기회 자체를 없앴다** | 이 버퍼를 남기면 **"발화분 기준 해소"와 "활성 전체 기준 해소" 두 갈래 판정이 공존**하게 되어 수정이 성립하지 않는다. 대체 경로(활성 물안개 전체 순회 + `FiredThisTick` 게이트)가 **같은 커밋에 함께 들어가** 기능 공백 구간이 없다 |
| `PickedUnit` / `PickedBuilding`의 `SourceBuildingId`·`ShowText` 필드 | **버그의 배관** — 적용 단계가 값 복사본만 들고 있어 **그 물안개가 이번 틱에 발화했는지를 알 수 없었다** | 적용 단계에서 `FiredThisTick`을 읽어야 하므로 값 복사본을 **`ActiveMist` 참조로 교체**했다. 두 표현을 병존시키면 어느 쪽이 진실인지 모호해진다. **동작은 동일**(같은 값을 참조로 읽을 뿐) |
| `ActiveMist` **4인자 생성자** | 대체물(5인자 생성자)이 같은 커밋에 있다 | 새 필드(공유 위상 시작값)를 반드시 받아야 하므로 4인자 생성자는 **위상 정렬을 우회하는 뒷문**이 된다. **호출부가 `Activate` 한 곳뿐임을 확인**했고, 주석 처리하면 대체 생성자와 시그니처가 겹쳐 **컴파일이 성립하지 않는다** |

> 3건 모두 **주석 처리로는 컴파일이 성립하지 않거나(생성자 중복), 남기는 것 자체가 버그를 존속시키는** 항목이다.
> WORKFLOW.md [4]의 "검증 전까지 비활성화 우선" 기본을 따르지 않은 근거를 여기에 남긴다.
...
## 9. 구현 후 보완 수정 (2026-08-11 추가 · 2026-08-12 §9-4 추가 — 사용자 실기 테스트 중 발생)

> **이 절이 왜 있는가 (자연어 설명 — 기술 용어 없이)**
>
> 계획대로 구현을 마치고(구현 커밋 `be2b396`, 에디터 셋업 스크립트 `b0b663c`),
> 사용자가 실제로 게임을 돌려 보는 과정에서 **계획 단계에서는 보이지 않던 문제들**이 드러났습니다.
>
> 앞의 세 가지(§9-1 ~ §9-3)는 **기능을 더 넣는 수정이 아닙니다.**
> ① 만들어 둔 범위 원이 **화면에 아예 안 보이던 것**, ② 화면에 보이는 원 크기와 **실제 회복 범위가 따로 놀 수 있던 것**,
> ③ 원이 **실제 범위보다 작게 그려지던 것** — 즉 **"보이는 것과 실제가 어긋나는 문제"를 맞추는 수정**입니다.
>
> 네 번째(§9-4)는 성격이 다릅니다. **눈에 보이는 문제가 아니라 규칙을 어기고 있던 문제**입니다.
> 물안개 두 개가 겹친 자리에 있는 부대가 **원래 받아야 할 회복의 두 배**를 받고 있었습니다.
> 이것은 연출 문제가 아니라 **약속한 규칙(겹쳐도 한 번만 회복)을 지키지 못하고 있던 것**이라, 로그를 붙여 원인을 찾은 뒤 고쳤습니다.
>
> 나중에 *"계획에 없던 코드가 왜 들어갔지?"* 라는 의문이 생기지 않도록, 네 수정의 **문제 → 해결 → 근거 규칙**을 여기에 남깁니다.
...
| §9-4 | `be17148` | **중첩 해소가 동작하지 않던 규칙 13 위반 수정** — 위상 정렬 + 소유권 판정 분리 | `Application/UseCases/MistShrineUseCase.cs` |
...
### 9-4. 중첩 해소가 동작하지 않던 규칙 13 위반 수정 (커밋 `be17148`)

> **성격:** §9-1~§9-3과 달리 **연출·표시 문제가 아니라 규칙 위반 버그**다.
> 계획대로 중첩 해소 코드를 넣었지만, **그 코드가 실행될 기회 자체가 없어 사실상 죽은 코드**였다.

**증상 (실기 로그 실측):** 겹쳐 지은 물안개 2개가 같은 유닛을 **각각** 회복시켜 **초당 회복량이 2배**가 되었다.

| 실측 값 | 내용 |
|------|------|
| 신전6 위치 | `(4.50, -14.72)` |
| 신전10 위치 | `(4.50, -15.59)` — 두 신전 거리 **0.87** |
| Unit3 HP | 3초에 **130 → 190 (+60)** |
| 설정상 기댓값 | **+30** (`_mistHealPerSecond: 10` × 3초) |

규칙 13이 정한 **"가장 가까운 하나만 적용"** 도 함께 어긋나, **더 먼 신전10까지 회복을 적용**하고 있었다.

**원인 — 두 사실이 겹쳤다:**

| # | 사실 | 결과 |
|:-:|------|------|
| ① | `ActiveMist.TickAccum`이 **물안개마다 독립**이라, 시전 시각이 다르면 1초 경계를 넘는 프레임이 서로 달라진다 | 실측 위상: 신전6 ≈ 매초 `.36x` / 신전10 ≈ 매초 `.73x` → **같은 밀리초에 동시 발화한 틱 0건** |
| ② | 중첩 해소(`_bestUnitByTarget`)가 **같은 `_firingBuffer`에 담긴 물안개끼리만** 비교하는 구조였다 | ① 때문에 버퍼에 늘 물안개가 하나뿐 → **해소 코드가 돌 기회 자체가 없었다(죽은 코드)** |

**수정 (두 겹으로 막는다):**

| 겹 | 내용 |
|:-:|------|
| ① **위상 정렬** | 새로 생성되는 물안개의 `TickAccum`을 **0이 아니라 기존 물안개와 같은 값**에서 출발시킨다(`GetSharedTickPhase()`). `TickMists`가 모든 물안개에 **같은 `dt`** 를 더하므로 **한 번 맞춘 위상은 계속 유지**된다 |
| ② **소유권 판정 분리** | `ApplyHealTick`이 "이번 틱 발화분"이 아니라 **활성 물안개 전체**를 순회해 대상별 최근접 소유자를 정하고, **그 소유자가 이번 틱에 발화한 경우에만**(`FiredThisTick` 게이트) 회복을 적용한다 |

> **①만으로도 동작은 맞는다.** 그럼에도 ②를 함께 넣은 이유는, ①만 두면 규칙 13의 "가장 가까운"이 **"모든 물안개가 같은 타이밍에 발화한다"는 타이밍 전제에 기대게** 되기 때문이다.
> 소유권 판정을 발화 여부와 분리해 두면 **타이밍이 어떤 이유로든 어긋나도 규칙 13이 그대로 성립**한다.

**기존 규칙과 충돌하지 않는다 (되돌림 방지용 근거):**

- **규칙 8-1이 요구하는 것은 "물안개마다 자기 누적기"** 이지 **"시작 위상이 제각각"** 이 아니다. 이 수정으로 **누적기 개수 = 물안개 개수**(규칙 14 표)는 그대로 유지된다.
- **매 틱 대상 재수집 구조를 손대지 않았으므로 규칙 9**(범위 이탈 시 즉시 끊김)도 그대로다.
- 이 불변식은 이번에 **`GameSystemRules_Buildings.md` 규칙 8-1에 명시**했다 — 문서에 없으면 나중에 위상 정렬을 "불필요한 결합"으로 오해해 되돌릴 수 있기 때문이다.

**건물 경로도 함께 고쳐졌다.** 규칙 13은 *"한 대상"* 을 말하므로 유닛만의 문제가 아니었고,
**건물 대상 중첩 해소 코드도 같은 이유로 돌지 못하고 있었다.** 수정이 유닛·건물 양쪽에 동시에 적용된다.

**제거한 기존 로직 3건** → 최상단 「기존 로직 제거 여부」 **③** 참조(`_firingBuffer` · `PickedUnit`/`PickedBuilding`의 `SourceBuildingId`·`ShowText` 필드 · `ActiveMist` 4인자 생성자).

**부수 효과 (사양 범위 밖 — 기록만 남긴다):**
이미 다른 물안개가 깔려 있는 상태에서 새로 시전하면, 그 물안개의 **첫 회복이 1초가 아니라 0~1초 사이**에 들어온다(공유 위상에 합류하기 때문).
**대상 입장에서 본 회복 간격은 1초로 유지**되며 이중 회복도 발생하지 않는다. 물안개가 하나뿐이면 종전과 완전히 동일하다.
...
로그 파일은 `Assets/_Project/Docs/_Logs/2026-08-10/14_12_mistshrine-heal-implementation/RuntimeLog_host.txt`에 **영구 보존**한다(LogRules).
위 증상·원인의 수치는 전부 이 로그의 실측값이며, **추정이 아니다.**

**근거 규칙:**
- **Buildings 규칙 13** — *"한 대상은 물안개 효과를 하나만 받는다 / 여러 범위에 동시에 들어가면 더 가까운 MistShrine의 효과를 받는다."* 수정 전 코드는 **두 조항 모두** 위반하고 있었다.
- **Buildings 규칙 8-1** — 물안개별 독립 누적기 + 매 틱 재수집. 이번에 **활성 물안개들이 같은 위상으로 틱한다**는 불변식을 규칙 8-1에 보강했다.
- **Buildings 규칙 14 표** — 누적 단위는 여전히 **물안개 인스턴스 단위**(개수 불변)임을 확인.
- **Buildings 규칙 9** — 매 틱 재수집 구조 무변경으로 유지.

---

## 10. 완료 판정 (구현 작업) — **2026-08-12 실기 검증 결과 반영**

> **⚠️ 표기 원칙 (과대 표기 금지):** 아래에서 **[x]** 는 **사용자 에디터 실기 또는 런타임 로그 실측으로 확인된 항목만**이다.
> 확인 보고가 없는 항목은 **"통과했을 것"이 아니라 미확인**으로 남긴다.
> **검증 환경은 에디터 싱글플레이뿐이다 — 멀티플레이 실기는 실시하지 않았다(§10-3).**

### 10-1. 실기·로그로 검증 완료 (에디터 싱글플레이)

- [x] **컴파일 오류 0건** — 사용자 에디터에서 확인. 로그가 정상 출력된 것이 그 증거다 (R14 / §9-2 · §9-4)
- [x] **범위 원 경계 = 실제 회복 판정 일치** — 로그 실측으로 **회복된 최대 거리 2.29 / 회복에서 탈락한 최소 거리 3.12**, 설정 반경 **3.00**과 **모순 0건** (§9-3 / Buildings 3 · UI 8·12)
- [x] **범위 이탈 시 즉시 끊김(규칙 9)** — Unit5가 거리 `2.29(Healed)` → `3.77(OutOfRange)`로 이탈한 **다음 틱부터 HP 110 고정**. 시전 시점 스냅샷이 아님이 확인됨
- [x] **회복량 정상** — 매 틱 **+10**, `SpecialAttackConfig`의 `_mistHealPerSecond: 10`과 일치
- [x] **중첩 해소(규칙 13) — 수정 후 정상** — 후보 물안개 **4개** 상황에서도 대상당 **1회만** 적용되고, **거리 동률(0.87)에서 Id가 작은 신전9**가 선택됨(결정적 규칙 성립). **수정 전에는 2배 회복 + 더 먼 신전 적용이었다** → §9-4
- [x] 아군 MistShrine 패널을 열면 **범위 원이 화면에 나타나고**, 닫으면 사라진다 (§9-1 / UI 8)
- [x] 싱글: 탭 = 1회 시전 / 롱프레스 = 자동 토글 / 자동 중 탭 = 해제 (육안 확인)
- [x] 쿨다운 오버레이의 **감소 방향**이 사양대로 동작 (육안 확인 / UI 7)
- [x] **적 MistShrine은 반응하지 않음** — 패널·범위 원 모두 열리지 않음 (육안 확인 / UI 8)
- [x] **연구 패널·스킬 패널의 `ClosedFrame` 가드 보정 확인** — 배경 탭으로 닫은 프레임에 뒤쪽 타일이 선택되지 않음 (육안 확인 / §2-4-2 ②)

### 10-2. 싱글플레이지만 **실기 결과 미보고** (통과로 표기하지 않는다)

- [ ] 싱글: 범위 안 **아군 건물 · 시전 건물 자신 · Castle**의 HP 상승 (유닛 회복은 위에서 확인됨 / 규칙 4)
...
- [ ] `MistShrinePanelUI`에 `OnDestroy` 선언이 없음 (R6 — 정적 확인 항목)
- [ ] **MistShrine 패널**을 배경 탭으로 닫은 프레임에 뒤쪽 타일이 선택되지 않음 (§2-4-2 ①)
- [ ] **회귀 없음 확인** — 패널들을 닫은 **다음 프레임부터는** 타일 클릭이 정상 동작 (R9-1)
- [ ] **`SkillAimReticle` 조준원 재확인** — ① 첫 사용에서도 정상 표시(§9-1) ② **커진 조준원 크기가 실제 착탄 반경과 일치**하고 화면을 과도하게 덮지 않는지(§9-3 / R13) ③ 표시·숨김 흐름 무변화
- [ ] `SpecialAttackConfig.asset`의 반경 값을 바꾸면 **화면의 원 크기가 그대로 따라간다** (§9-2 / Buildings 3·16)
- [ ] 씬에 `_baseDiameter` 잔재가 남아 **원 크기를 되돌리지 않는지** (§9-3)
- [ ] 임시 스프라이트(`UI/Skin/Knob.psd`)를 **정식 아트로 교체했을 때** 코드·Inspector 수정 없이 크기가 자동으로 맞는지 — 교체 시점에 확인 (§9-3)
- [ ] git 명령 실행 0건 (CLAUDE.md 규칙 5)

### 10-3. ⚠️ 멀티플레이 — **전 항목 미검증**

**이번 사이클의 검증은 에디터 싱글플레이로만 이루어졌다. 멀티플레이 실기는 한 번도 실시하지 않았다.**

- [ ] 멀티: 클라이언트 시전이 서버에서 재검증되고, 쿨다운 오버레이가 양쪽에서 동일하게 감소 (규칙 21·22)
- [ ] 멀티: 건물 HP 회복이 클라이언트에 동기화됨 (규칙 24)
- [ ] 멀티: 호스트에서 회복량이 **2배가 되지 않음**(이중 틱 없음 — R3)
- [ ] 멀티: 자동 모드 토글이 양 클라이언트에 브로드캐스트됨 (R12)

> **싱글 검증이 멀티에 어디까지 유효한가 (근거 있는 범위만):**
> **범위 판정·중첩 해소·회복량 계산 코드는 싱글과 멀티가 같은 `MistShrineUseCase` 경로를 공유**하므로,
> §10-1의 해당 항목들은 **판정 로직 자체에 대해서는** 멀티에서도 유효하다.
> 그러나 **HP 동기화(`SyncBuildingHealClientRpc`) · 클라이언트 표시 · RPC 팀 검증 · 쿨다운 로컬 미러 · 이중 틱 여부**는
> **멀티에서만 도는 고유 경로**이며 **이번에 전혀 실행되지 않았다.** 멀티 동작을 완료로 표기해서는 안 된다.

### 10-4. 미제작 / 미확정 (사양 범위 밖 — 완료 조건이 아니다)

- [ ] **물안개 지속 VFX 미제작** (Buildings 26) — 현재 물안개는 눈에 보이지 않는다
- [ ] **사용(시전) 버튼 아이콘 미제작** (UI 15) — 임시 텍스트 라벨 사용 중
- [ ] **밸런싱 수치 미확정** (Buildings 16) — 현재 값은 전부 임시값(§3). 확정 시 `SpecialAttackConfig.asset` 한 곳만 교체
- [x] 밸런싱 수치가 **`SpecialAttackConfig.asset` 한 곳**에만 있고, 코드에는 폴백 기본값만 존재 (§9-2에서 반경 이중화가 제거되어 성립)
- [x] 구현 상태 표기가 **과대 표기 없이** 갱신됨 — 규칙 문서의 "구현 미착수" 표기를 2026-08-12에 정정하고 **멀티 미검증·VFX/아이콘 미제작·밸런싱 미확정**을 함께 명시

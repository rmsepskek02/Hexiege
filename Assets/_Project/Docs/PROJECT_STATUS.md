...
- ⚠️ **해소되지 않은 것** — 🔴 **재경기 맵은 여전히 미구현이다.** 개정은 **만들 것의 범위를 줄였을 뿐 만든 것이 아니다.** 재경기하면 여전히 `Reason=MapHandoffEmpty` 로 실패한다. 아울러 **「재경기 실패 시 안내가 없다」는 별개의 다음 작업으로 [ROADMAP.md](ROADMAP.md) 에 행을 세웠다**(「경기 종료 후 UI·프로세스」 — 2026-09-14 (4차) 등록).
...
**✅ 구현 완료 (2026-09-14) — 검증 상태: 멀티플레이 실기 검증 완료(최초 경기 2판) / 🔴 재경기 맵은 미구현 · 초기 골드는 미적용:** **무작위 맵 3단계 — 멀티플레이 맵 전송 · 해시 대조 · Client 재생성 검증 · 씬 전환 게이트.**
...
- ⚠️ **이 세 줄을 함께 적지 않으면 거짓이 된다** — 완료된 것은 「멀티 맵 전송」이 아니라 **「최초 경기의 맵 전송」**이다.
...
- **전송 관련 예외·경고 0건.**
- 🔴 **바이트 수 공식이 실기 전송으로 확인됐다 — `319 + 4 × (중립 광산 수)`** (광산 3개 → 331, 2개 → 327, 장식 0). 그래서 **조각은 항상 1개**다.

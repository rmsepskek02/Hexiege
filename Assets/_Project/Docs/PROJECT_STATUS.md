...
**⚠️ 부분 구현 (2026-09-24) — 검증 상태: 🔴 실기 검증 0건 (코드 확인까지):** **「경기 종료 후 UI·프로세스」 단계 9(규칙 18 — 재경기 실패 문구 분기) + §8 14-4 개발용 강제 실패 플래그.**

쉽게 말하면, **「왜 재경기가 안 됐는지」를 화면이 구분해 말한다.** 종전에는 이유가 무엇이든 「재경기를 시작할 수 없습니다」 한 문장만 떴는데, 이제 **상대가 나가서 실패한 경우에만** 「상대방이 나가서 재경기를 시작할 수 없습니다」로 바뀐다. 두 경우는 **사용자가 할 수 있는 일이 다르다** — 앞쪽은 다시 시도할 여지가 없고 뒤쪽은 있다. 🔴 **사유를 모르면 기본 문구를 쓴다**(상대가 나간 것처럼 보여도 내 회선 문제일 수 있어, 단정하면 거짓을 말하게 된다). 그리고 **그 화면을 개발 중에 볼 수 있게 하는 에디터 메뉴**가 함께 생겼다 — 맵을 주고받는 구간이 **실측 99~134ms** 라 사람이 그 순간에 네트워크를 끊을 수 없어서, 실패 화면은 지금까지 **한 번도 관측된 적이 없다.**

- **커밋 해시는 적지 않는다** — 규칙 5 로 git 을 실행할 수 없어 확인할 수단이 없다(지어내지 않는다).
- **사양의 단일 소스는 규칙이고 여기에 옮겨 적지 않는다** — **문구 2종**은 [GameSystemRules/GameSystemRules_UI.md](GameSystemRules/GameSystemRules_UI.md) 「공통 UI 규칙」 **규칙 D-7** 의 2026-09-24 확정 블록, **가르는 기준**은 [GameSystemRules/GameSystemRules_RandomMap.md](GameSystemRules/GameSystemRules_RandomMap.md) **규칙 18** 이다. **강제 실패 플래그의 사용 절차**는 [TechnicalDesignDocument.md](TechnicalDesignDocument.md) 「개발용 강제 실패 플래그 (에디터 전용)」 절, **구현 상세**는 `_Tasks/2026-09-16/06_27_post-game-leave-ui/Plan.md` **§14** 가 단일 소스다.

🔴 **완료된 것과 완료되지 않은 것의 경계 — 이 문단이 그 단일 소스다.**
- ✅ **완료 — 실패 문구가 「연결 끊김인가」 하나를 기준으로 둘로 갈린다.** 문구 상수는 각각 코드에 한 곳뿐이고, **화면에 쓰는 자리는 종전과 같은 한 자리(삼항 연쇄)이며 우선순위 「이탈 > 실패 > 평시」도 그대로**다(직접 실측). **코드 확인까지다.**
- ✅ **완료 — 새 이벤트·새 채널 0개.** 규칙 18 이 *"새 이벤트를 만들지 않는다"* 고 정했으므로 **기존 채널에 사유를 실었고 발행처 2곳·구독처 1곳으로 개수가 변하지 않았다**(직접 실측). 사유 값은 **레이어 제약 때문에 Application 쪽에 따로 뒀다.**
- ✅ **완료 — 개발용 강제 실패 플래그.** 신규 파일 2개 · 에디터 메뉴 5개 · 사유 3종. **저장은 `EditorPrefs` 라 씬·프리팹에 남지 않고**(직렬화 필드 선언 0건), **한 번 쓰면 스스로 꺼지며**, **재경기 회차에만 걸린다.** 🔴 **가드가 `#if UNITY_EDITOR` 로 좁혀졌다** — 확정 당시의 `|| DEVELOPMENT_BUILD` 는 조작 수단이 에디터 메뉴뿐이라 죽은 코드가 되기 때문이다(근거는 위 Plan **§8** 의 2026-09-24 블록).
- 🔴 **미검증 — 실기 검증이 0건이고, 강제 실패 플래그는 한 번도 쓰이지 않았다.** 재현하려면 **에디터를 Host 로 두고** 한 판 끝낸 뒤 메뉴에서 사유를 고르고 재경기를 진행해야 한다. ⚠️ **단계 8 의 재현 조건과 다르므로 한 번의 테스트로 둘이 함께 검증되지 않는다.**
- 🔴 **미착수 — 단계 10(싱글 규칙 M-4)뿐이다.** 멀티(규칙 M-3)는 **팝업이 아니라 상태 줄**로 개정됐고 그 구현은 직전 회차에 끝났다. ⚠️ **규칙 M-4 의 「표시 문구·버튼 라벨 미정」은 살아 있고 이번 회차가 건드리지 않았다.** 🔴 **`UIManager.ShowAlert` 실호출처는 여전히 1건**이며 이번 작업으로 늘지 않았다(직접 실측).

**⚠️ 부분 구현 (2026-09-22) — 검증 상태: 🔴 실기 검증 0건 (코드 확인까지):** **「경기 종료 후 UI·프로세스」 단계 8(규칙 D-6 — 재경기 요청자 이탈 처리).**

쉽게 말하면, **상대가 「한 판 더 하자」고 물어 놓고 그대로 사라졌을 때 내 화면이 더 이상 멍하니 기다리지 않는다.** 답할 곳이 없어진 **요청 팝업을 먼저 닫고**, **「상대방이 떠났습니다.」 알림**을 **「로비로」 버튼 하나와 함께** 띄운다. 버튼을 누르지 않아도 이미 30초로 다시 시작된 자동 복귀 카운트다운이 로비로 데려가므로 **어느 쪽이든 갇히지 않는다.**

- **커밋**: `500bbc1`(단계 8 코드 — **2개 파일뿐**: `Presentation/UI/Common/RematchRequestPopup.cs` +60행 · `Presentation/UI/GameEndUI.cs` +124/−3) · `cae1e9c`(docs) · 머지 후 원격 `ab0490f`. ※ **커밋 해시와 증감 행수는 메인 세션 전달값**이다(규칙 5 로 git 을 실행할 수 없다).
- **사양의 단일 소스는 규칙이고 여기에 옮겨 적지 않는다** — [GameSystemRules/GameSystemRules_UI.md](GameSystemRules/GameSystemRules_UI.md) 「공통 UI 규칙」의 **규칙 D-6** 이며 그 아래에 **2026-09-22 구현 결과 블록**이 붙어 있다. **구현 상세·완료 판정 6건·실기 재현 조건은 `_Tasks/2026-09-16/06_27_post-game-leave-ui/Plan.md` §12** 가 단일 소스다.

🔴 **완료된 것과 완료되지 않은 것의 경계 — 이 문단이 그 단일 소스다.**
- ✅ **완료 — 규칙 D-6 의 1항(요청 팝업 닫기) · 2항(알림 팝업) · 3항(별도 타이머 없음).** 셋 다 **코드 확인**까지다. **새 닫기 경로를 만들지 않고 기존 숨김 경로를 재사용**했고, `UIManager.Instance?.ShowAlert(...)` 실호출처는 **1건**이며 null-safe 다(직접 실측).
- ✅ **씬·인스펙터 작업 없음.** 팝업 참조가 직렬화 필드가 아니라 **런타임 1회 탐색·캐시**이고, 그 팝업이 `Game.unity` 에 배치돼 있음을 **guid 검색 1건**으로 확인했다(직접 실측). 🔴 **아래 「`NetworkStatusUI` 미배치」와 같은 상태가 아니다.**
- ✅ **알림 팝업의 실호출처가 처음으로 생겼다 — 0건 → 1건.** 아래 2026-09-21 문단의 「실호출처 0건」 항목은 **이 항목이 대체한다**(그 문단의 원문은 지우지 않고 마커만 덧붙였다).
- 🔴 **미검증 — 실기 검증이 0건이다.** 이 경로를 타려면 **한쪽이 재경기 버튼을 누른 직후 그 누른 쪽 앱을 강제 종료**해야 하고 **역할(Host/Client)은 무관하다**(단계 6 이 양쪽 각자 판정이므로). ⚠️ **「구현했다」와 「실기에서 확인됐다」는 다른 말이고 이 문단은 앞쪽까지만 참이다.** 🔴 **그래서 알림 팝업이 화면에 뜨는 모습은 여전히 아무도 본 적이 없다 — 바뀐 것은 그 이유뿐이다**(띄우는 코드가 없어서 → 코드는 있으나 실기로 관측되지 않아서).
- 🔴 **미착수 — 단계 9·10 과 14-4 강제 실패 플래그.** 맵 전송 중 이탈 문구 분기(단계 9) · 맵 준비 실패 알림 팝업(단계 10)이며, 플래그가 없어 **그 두 단계의 화면은 정상 경로에서 관측되지 않는다.**
  - **[🔴 2026-09-24 갱신 — 위 한 줄은 2026-09-22 시점의 기록이라 한 글자도 지우지 않고 덧붙인다(`.claude/MEMORY.md` B-7).]** **「단계 9 와 14-4 플래그가 미착수」는 더 이상 참이 아니다 — 둘 다 구현됐다**(맨 위 「⚠️ 부분 구현 (2026-09-24)」 문단이 경계의 단일 소스다). ⚠️ **그래도 이 줄의 결론 하나는 살아 있다 — 그 화면은 여전히 관측되지 않았다.** 바뀐 것은 **이유뿐**이다: 플래그가 없어서 → **플래그는 생겼으나 아직 한 번도 쓰이지 않아서.** 🔴 **미착수로 남은 것은 단계 10 이고, 그중에서도 싱글(규칙 M-4)뿐이다.**
- ⚠️ **이 문서가 낡아 있던 자리를 함께 정정했다** — 아래 2026-09-21 문단의 **「🔴 미착수 — 단계 4~10」** 한 줄은 **이 회차 착수 전부터 이미 낡아 있었다**(단계 4·5·6 완료·로그 확인, 단계 7 완료·실기 확인). 🔴 **단계별 상태의 단일 소스는 `_Tasks/2026-09-16/06_27_post-game-leave-ui/Plan.md` §10 실행 기록 표**이며 **여기에 표를 옮겨 적지 않는다.** 그 줄은 지우지 않고 마커를 덧붙였다.

**⚠️ 부분 구현 (2026-09-21) — 검증 상태: 컴파일 정상 · 기존 팝업 회귀 없음(2/3 확인) / 🔴 알림 팝업은 화면에 띄워 본 적이 없다:** **「경기 종료 후 UI·프로세스」 단계 1·2·3 + 프리팹 구성.**

쉽게 말하면, **결과 화면에서 사람이 갇히지 않게 만드는 작업의 앞부분 세 조각이 끝났다.** ① 「다시하기」를 눌러도 **로비로 나가는 버튼은 이제 꺼지지 않는다.** ② 결과 화면이 스스로 로비로 나가기까지의 시간이 **30초에서 60초로** 늘었다. ③ **「제목 + 본문 + 버튼 하나」짜리 알림 팝업을 띄울 수 있는 수단**이 생겼다 — 다만 **그것을 실제로 띄우는 자리는 아직 하나도 만들지 않았다.**

- **커밋 4개**: `d881d23`(단계 1~3 — 5개 파일, +176/−6) · `b336d9c`(에디터 셋업 스크립트 + 에이전트 메모리 2건 + `_baseline.json` — 4개 파일) · `a87e404`(사용자의 로그·meta 커밋 `d6104af` 병합) · `1eba997`(사용자가 Unity 에서 셋업 스크립트를 실행해 저장·푸시한 `ConfirmPopup.prefab`).
- **사양의 단일 소스는 규칙이고 여기에 옮겨 적지 않는다** — [GameSystemRules/GameSystemRules_UI.md](GameSystemRules/GameSystemRules_UI.md) 「공통 UI 규칙」의 **규칙 D-3 · D-4 · D-5** 이며, 각 규칙 아래에 **2026-09-21 구현 결과 블록**이 붙어 있다. 계획·단계 분할은 `_Tasks/2026-09-16/06_27_post-game-leave-ui/Plan.md`.

🔴 **완료된 것과 완료되지 않은 것의 경계 — 이 문단이 그 단일 소스다.**
- ✅ **완료 — 규칙 D-3(로비 복귀 버튼 상시 활성).** `GameEndUI.SetupRematchButton` 의 `_backToLobbyButton.interactable = false` 를 제거했고 **잔존 0건**이다(주석만 남아 제거 이유를 설명한다. 문서 작업 중 직접 실측). ⚠️ **다만 이 규칙의 후반부「상대 이탈 판정 뒤에도 켜 둔다」는 이탈 판정이 미구현이라 확인되지 않았다.**
- ✅ **완료 — 규칙 D-4 의 「결과 화면 표시 60초」.** 🔴 **코드와 씬 두 자리를 모두 고쳐야 성립하는 항목**이라 `GameEndUI._autoReturnSeconds` **30f → 60f** 와 `Game.unity` 직렬화 값 **30 → 60** 을 함께 바꿨다(**Inspector 값이 코드 기본값보다 우선**하므로 코드만 고쳤다면 실제 동작은 30초 그대로였다). 🔴 **같은 규칙의 「이탈 판정 시 30초 재시작」은 미구현이다.**
- ✅ **완료 — 규칙 D-5 의 수단(알림 팝업 API).** 신규 팝업을 만들지 않고 **기존 `ConfirmPopup` 을 확장**했다(사용자 승인). `_titleText` 필드 · `ShowAlert()` · `ApplyTitle()` · `SetCancelButtonVisible()` 추가, `IUIManager.ShowAlert` 선언 + `UIManager` 위임. **`ShowConfirm` 의 시그니처·동작 무변경 · 실호출처도 변경 전과 같은 5건**(직접 실측).
- ✅ **완료 — 프리팹 전제 조건.** `ConfirmPopup.prefab` 의 Panel 에 VerticalLayoutGroup 이 들어갔다. 🔴 **이것이 없으면 제목을 숨겨도 그 자리가 빈 공간으로 남아 제목 없는 기존 팝업의 본문이 아래로 밀린다** — 즉 **코드만으로는 규칙 D-5 가 성립하지 않는다.** 1회성 에디터 스크립트 `Assets/Editor/Setup/SetupConfirmPopupAlertLayout.cs` 로 수행했고(WORKFLOW [5-2]), **그 스크립트는 멱등이라 프리팹이 깨졌을 때 되돌리는 수단으로도 쓸 수 있다.**
- 🔴 **미검증 — 알림 팝업이 화면에 뜨는 모습은 아무도 본 적이 없다.** **`UIManager.ShowAlert` 의 실호출처가 0건**이기 때문이다(직접 실측). 실제로 띄우는 것은 **단계 8**(규칙 D-6 요청자 이탈)과 **단계 10**(규칙 M-3·M-4 맵 실패)이며 둘 다 미착수다. ⚠️ **「구현했다」와 「실기에서 확인됐다」는 다른 말이고 이 항목은 앞쪽까지만 참이다.** **[⚠️ 2026-09-22 갱신 — 위 문장은 2026-09-21 시점의 기록이라 한 글자도 지우지 않고 덧붙인다: **「실호출처가 0건」과 「단계 8·10 둘 다 미착수」는 더 이상 참이 아니다** — **단계 8 이 실호출처 1건을 만들었다**(커밋 `500bbc1`). ⚠️ **그래도 이 항목의 결론은 살아 있다** — **여전히 아무도 이 팝업을 본 적이 없다.** 맨 위 「⚠️ 부분 구현 (2026-09-22)」 문단이 경계의 단일 소스다]**
- ⚠️ **부분 확인 — 기존 확인/취소 팝업의 회귀.** 실기에서 **앱 종료 확인**(짧은 본문)과 **이메일 인증 취소**(긴 본문) 2건이 종전과 같이 표시됐다. **`ProfileView` 의 알림 표시 1건은 임의 재현이 어려워 미확인**이나, `ShowConfirm(cancelLabel: string.Empty)` 로 호출해 **버튼 2개 구조가 확인된 2건과 같고** 버튼 배치를 담는 ButtonRow 가 무변경이다.
- 🔴 **미착수 — 단계 4~10.** 전송 계층 타임아웃 60000(두 씬) · 이탈 채널과 정상 퇴장 통보 · 무반응 30초 자체 감시와 판정 시 연결 종료 · 결과 화면 반영(D-1·D-2·D-4 재시작) · D-6 요청자 이탈 · 맵 전송 중 이탈 문구 분기(규칙 18) · 맵 준비 실패 알림 팝업(규칙 M-3·M-4).
  - **[🔴 2026-09-22 갱신 — 위 한 줄은 2026-09-21 시점의 기록이라 한 글자도 지우지 않고 덧붙인다(`.claude/MEMORY.md` B-7).]** 🔴 **이 줄은 이 회차 착수 전부터 이미 낡아 있었다 — 지금 미착수인 것은 단계 9·10 과 14-4 강제 실패 플래그다.** **단계 4·5·6 은 완료(로그 확인) · 단계 7 은 완료(실기 확인) · 단계 8 은 구현 완료(실기 미검증)** 이며, 🔴 **단계별 상태의 단일 소스는 `_Tasks/2026-09-16/06_27_post-game-leave-ui/Plan.md` §10 실행 기록 표**다(여기에 표를 옮겨 적지 않는다). ⚠️ **「완료」의 등급이 행마다 다르다는 것까지가 그 표의 내용**이므로 이 줄만 읽고 전부 같은 수준으로 끝났다고 읽지 않는다.
- 🔴 **미착수 — 14-4 강제 실패 플래그**(`#if UNITY_EDITOR || DEVELOPMENT_BUILD`, 사유 선택형). 이것 없이는 맵 실패 화면이 **정상 경로에서 관측되지 않는다.**
  - **[🔴 2026-09-24 갱신 — 위 한 줄은 2026-09-21 시점의 기록이라 한 글자도 지우지 않고 덧붙인다(`.claude/MEMORY.md` B-7).]** **구현됐다.** 🔴 **다만 가드는 `#if UNITY_EDITOR` 로 좁혀졌다** — 위 괄호의 `|| DEVELOPMENT_BUILD` 는 **확정 당시의 표기**이며, 조작 수단이 에디터 메뉴뿐이라 개발 빌드에 넣으면 죽은 코드가 된다(근거는 `_Tasks/2026-09-16/06_27_post-game-leave-ui/Plan.md` §8 의 2026-09-24 블록). **사용 절차의 단일 소스는 [TechnicalDesignDocument.md](TechnicalDesignDocument.md) 「개발용 강제 실패 플래그 (에디터 전용)」 절**이다. ⚠️ **이 줄이 말한 「정상 경로에서 관측되지 않는다」는 여전히 참이다 — 도구가 있어도 아직 쓰이지 않았다.**
- ⚠️ **이 체크아웃에서 확인할 수 없었던 것** — `ConfirmPopup.prefab` 의 실제 직렬화 값. **프리팹 변경은 사용자가 Unity 에서 실행해 푸시한 커밋 `1eba997` 에 있고, 문서 작업 시점의 작업 트리에는 반영돼 있지 않다**(이 트리의 프리팹에는 `_titleText` 행도 TitleText 오브젝트도 없다 — 직접 실측). **프리팹 값의 대조는 메인 세션이 수행했고 실측값의 단일 소스는 `.claude/agent-memory/game-programmer/ui-system.md` 「알림 팝업 — 프리팹 전제 조건」이다.**

**✅ 구현 완료 (2026-09-15) — 검증 상태: 멀티플레이 9판 + 싱글플레이 2판 실기 PASS / 🔴 실패 경로·폴백·타임아웃은 실기 미검증:** **맵 테스트 모드 삭제(T1~T3) + 재경기 랜덤 맵(A~D).**

쉽게 말하면, **이제 「한 판 더」를 눌러도 매번 새 전장이 나온다.** 종전에는 재경기를 하면 맵이 구성되지 않고 실패했는데(`Reason=MapHandoffEmpty` — 결함이 아니라 설계대로였다), 이제 결과 화면에서 재경기가 성립하는 순간 Host 가 새 맵을 만들어 상대에게 보내고, 지문(해시)이 맞을 때만 전투 화면으로 넘어간다. 그리고 **쓰지 않던 「맵 테스트 모드」를 코드·설정·문서에서 전부 걷어냈다.** 그 표식이 맵 데이터 안에 들어 있던 값이라, 걷어내면서 **맵 파일의 형식 번호가 1에서 2로 올라갔고 예비 맵(폴백 템플릿) 5개를 다시 만들었다.**

- **커밋 3개**(브랜치 `claude/random-map-phase2-runtime`): `0608f85`(T1 — 규칙·설계 문서 개정, 5개 문서 통독 4,272행 중 24곳) · `8c83317`(T2·T3 — 코드 12개 파일에서 테스트 모드 제거 + `MapVersion` 1→2 + 폴백 템플릿 5개 재생성) · `20ba0ab`(A~D — 재경기 맵. 5개 파일 수정, 신규 파일 0개, 신규 `LogEvent` 키 0개).

🔴 **완료된 것과 완료되지 않은 것의 경계 — 이 문단이 그 단일 소스다.**
- ✅ **완료 — 맵 테스트 모드 삭제.** 코드 12개 파일 · `Resources/Config/GameConfig.asset` 의 `_mapTestModeEnabled` · `_testStartingGold` 두 행 · 규칙 문서·설계 문서에서 전부 제거됐다. 🔴 **`_startingGold: 5000` 은 손대지 않았다** — 그것은 `Economy.StartingGold` 라는 **별개 필드**이며 아래 「미적용」 항목과 위 「🔴 사실 확정 · 결정 기록 (2026-09-14)」 ①②가 단일 소스다.
- ✅ **완료 — canonical 바이트 형식 변경.** **`MapDefinition.CurrentMapVersion` 이 `1` → `2`** 가 됐고, **canonical 바이트 수 공식은 `315 + 4N`**(N = 중립 광산 수)이다. 종전 대비 정확히 **4바이트 감소**(`TestModeFlag` 고정폭 정수 4바이트 제거).
- ✅ **완료 — 폴백 템플릿 5개 재생성(실측값).** Canyon **335 → 331** / FullyOpen · ObstacleOpen · Outer · ThreeLane **각 343 → 339**(전부 −4). `_Reference/MapTemplateSource/*.md` 5개도 같은 도구가 함께 재생성했다. 🔴 **재생성은 Unity 에디터 도구가 아니라 메인 세션이 mono 로 헤드리스 실행했다**(`MapFallbackTemplateFactory` 가 순수 C# 이라 가능했다). 계획서에는 「사용자 작업」으로 적혀 있던 자리다.
- ✅ **완료 — 재경기 맵.** 결과 화면에서 재경기가 성립하면 Host 가 새 맵을 준비·전송·검증한 뒤에야 씬을 재로드한다. 최초 경기와 **같은 전송 본체**를 쓰며, 회차를 가르는 것은 **로그 필드 `Round=`(First / Rematch / Probe) 하나**다 — **새 로그 키는 하나도 만들지 않았다.** 프로토콜 변경도 **`MapPrepareBeginClientRpc` 에 `int roundKind` 를 더한 것 한 건**뿐이다.
- 🔴 **미검증 — 실패 경로(전송 실패 시 재경기 상태 복원).** 실기 11판이 **전부 성공**이라 이 경로는 **한 번도 발화하지 않았다.** 발화시키려면 인위적 불일치(임시 조작) 코드가 필요한데 **사용자 승인 사항이라 이번에 넣지 않았다.** ⚠️ **「구현했다」와 「실기에서 확인됐다」는 다른 말이며, 이 항목은 앞쪽까지만 참이다.**
- 🔴 **미검증 — 폴백 템플릿 사용 경로 · 타임아웃 · 재전송.** 11판 전부 `UsedFallback=False` · `ResendCount=0` 이었다. **검증 누락이 아니라 정상 조건에서 발화하지 않는 경로**다.
- ⚠️ **mono 로 검증할 수 없었던 것** — Unity 의존 코드와 `GameConfig.asset` 의 T2 변경은 헤드리스로 확인이 불가능해 **실기로만 확인됐다.**
- 🔴 **남아 있는 것 — `NetworkGameEndController.cs` 의 `[재경기맵 대체 대기]` 주석 마커 2건.** 실기 통과로 제거 조건은 충족됐으나 **아직 코드에 남아 있다.** 제거는 [ROADMAP.md](ROADMAP.md) 에 행을 세웠다. **[🔴 2026-09-15 정정 — 원문은 그대로 두고 덧붙인다(이 블록이 이 정정의 단일 소스이며 나머지 문서는 여기를 가리킨다): 이 마커 2건은 그 뒤 **실제로 제거됐다.** ⓐ **제거 완료 · 잔존 0건** — `grep -rc "재경기맵 대체 대기" Assets/_Project/Scripts --include=*.cs` 가 **0** 이다. ⓑ **삭제된 것은 주석 9줄뿐이고 동작 코드 diff 는 0줄이다** — `RequestRematchServerRpc` 상호 동의 분기의 마커 주석 4줄 + 비활성화돼 있던 `// StartRematch();` 1줄, `AcceptRematchServerRpc` 의 마커 주석 3줄 + 비활성화돼 있던 `// StartRematch();` 1줄. `git diff --numstat` 은 **0 추가 / 9 삭제**이고 그 9줄은 전부 `//` 로 시작하는 주석이었다(`GameLog.Dev.Info(...)` 2건 · `BeginRematchMapPreparation()` 호출 2건 · `StartRematch()` 본문과 디스폰 루프는 **전부 무변경**). ⓒ **근거 규정은 `WORKFLOW.md` [4]** — *"주석 처리된 기존 로직의 최종 삭제는 [6] 사용자 테스트 통과 후, [7] 문서/메모리 업데이트 전에 수행한다"* — 이고 2026-09-15 실기 통과로 그 조건이 충족됐다. ⓓ 🔴 **한계 — Unity 에디터 컴파일은 확인되지 않았고, 제거 후 재경기 실기 재검증도 하지 않았다.** 이 파일은 `Unity.Netcode` · `UnityEngine` 의존이라 헤드리스로 빌드할 수 없다. **주석만 지웠으므로 동작이 바뀔 수 없다는 것은 판단이지 실행으로 확인한 사실이 아니다.** 그래서 [ROADMAP.md](ROADMAP.md) 의 행은 내리지 않고 **남은 확인 작업(에디터 컴파일 · 제거 후 실기 재검증)으로 좁혔다.**]**

**🔴 실기 검증 (2026-09-15, 사용자 수행 — 에디터 + 실기기 SM-S901N)**

**멀티플레이 9판 전부 성공**(최초 경기 2 + **재경기 7회 연속**). Host 측 기록 기준:

| # | 시각 | Round | Host | Seed | 맵 유형 | 광산 | Bytes |
|---|---|---|---|---|---|---|---|
...
- 9판 전부 `HashMatch=True` · `ResendCount=0` · `ErrorCode=None` · `UsedFallback=False` · `MapVersion=2` · `ChunkCount=1`
- **9개 seed 전부 상이, 9개 해시 전부 상이.** 바이트 수는 9판 전부 **`315 + 4N` 과 정확히 일치**한다.
- 클라이언트 복원(D 방식): 9판 전부 `AttemptIndex=0`, `SearchedCombinations` 1 또는 2(최대 2)
- `Round=First` / `Round=Rematch` 가 Host 「맵 전송 회차 시작」 · Client 「시작 통보 수신」 · 양쪽 `MapTransferSucceeded` **3곳 모두**에 일관되게 기록됐다.
- 팀·종족 유지: 에디터(Client) 8판 전부 `Team=Red, BlueRace=0, RedRace=1`
- 재경기 추가 지연: 맵 준비 `ElapsedMs=0~3`, 전송 객체 스폰부터 전송 완료까지 **99~134ms**
- 신규 빌드 구간(14:52~14:56) `[ERROR]` **0건**(양쪽)
- 🔴 **같은 맵 유형이 연속으로 나온 구간이 있다** — #3·#4 Canyon, #7·#8 ObstacleOpen. **seed·광산 수·해시가 모두 달라 서로 다른 맵이며**, 사용자가 확정한 *「무작위성에 의한 연속은 문제가 아니다 · 회피 검사 불필요」*(위 「🔴 규칙 개정 기록 (2026-09-14)」)에 해당한다. **결함이 아니다.**

**싱글플레이 2판 전부 성공**(19:04 세션, 로그인 화면부터):

| 판 | 시각 | Seed | 맵 유형 | 광산 | ElapsedMs |
|---|---|---|---|---|---|
...
- 경로가 `[Map/GameBootstrapper]` — **싱글은 네트워크 전송을 거치지 않는 로컬 생성 경로를 그대로 유지**한다.
- 19:04 세션 전체 `[ERROR]` 0건 · `[WARN]` 0건

**로그 파일**: `_Logs/_editor/2026-09-15/RuntimeLog.txt` · `_Logs/2026-09-15/14_56_logcat/RuntimeLog_device.txt` · `_Logs/2026-09-15/14_40_logcat/RuntimeLog_device.txt`(**이전 빌드** — 재경기 시 `MapPreparationFailed, Reason=MapHandoffEmpty` 가 찍힌 「before」 기준).

**※ 근거 구분(규칙 10):** 위 커밋 해시 · 실기 표의 모든 수치 · 폴백 템플릿 바이트 증감 · mono 검증 결과는 **메인 세션이 측정·전달한 값**이며, 규칙 5(git 금지)와 런타임 실행 불가로 이 문서 작업에서 재검증하지 않았다. **문서 작업 중 직접 확인한 것**은 `Round=` 필드가 `NetworkMapTransfer.BuildTransferLogData` 본문에 들어 있어 `MapTransfer*` 5개 키 전부에 실린다는 사실과, `[재경기맵 대체 대기]` 마커가 `NetworkGameEndController.cs` 에 **여전히 2건 남아 있다**는 사실이다. **[🔴 2026-09-15 정정 — 원문은 그대로 두고 덧붙인다: 그 2건은 그 뒤 **제거됐고 잔존 0건**이다(`grep -rc "재경기맵 대체 대기" Assets/_Project/Scripts --include=*.cs` = 0, 문서 작업 중 직접 실측). **삭제분은 주석 9줄뿐이고 동작 코드 diff 는 0줄**이며(`git diff --numstat` = 0 추가 / 9 삭제), 근거 규정은 `WORKFLOW.md` [4] 다. 🔴 **다만 Unity 에디터 컴파일과 제거 후 실기 재검증은 하지 않았다** — 이 두 가지는 **전달받은 값도 실측값도 아니고 아직 수행되지 않은 확인**이다. 상세는 위 「✅ 구현 완료 (2026-09-15)」 문단의 「남아 있는 것」 항목에 붙인 정정 블록.]**

---

**🔴 규칙 개정 기록 (2026-09-14 · 같은 날 3차 추가 — 코드 변경 없음, 문서만):** **재경기 맵 선택 방식이 바뀌었다. 「같은 맵 / 새 맵」을 고르는 일이 없어졌다.**

쉽게 말하면, **한 판 더 하기를 누르면 전장이 무조건 새로 뽑힌다.** 방금 싸운 전장을 다시 쓰는 선택지도, 상대에게 「어느 쪽으로 할지」를 보여 주는 표시도 만들지 않는다. 🔴 **사용자 확정**: *"재경기는 「같은 맵 / 새 맵」을 따로 선택하지 않고, 무조건 무작위로 새 맵이 결정된다. 추가 UI나 작업도 필요 없다."* 그리고 *"무작위로 같은 맵이 연속으로 나오는 것은 문제가 아니다."*

- **개정된 문서**: `GameSystemRules/GameSystemRules_RandomMap.md` 규칙 14·16 · `GameSystemRules/GameSystemRules_UI.md` 규칙 M-3·M-4 · `GameSystemRules.md` · `GameDesignDocument.md`(1.17.0) · `TechnicalDesignDocument.md`(0.47.0). **제거한 조항과 그 근거의 단일 소스는 규칙 14 의 개정 블록**이며 이 문서에 옮겨 적지 않는다.
- ✅ **이 개정으로 해소된 것 4가지** — **남은 일로 오해하지 않도록 못 박아 둔다.**
  - **UI 자산 작업 불필요** — `GameEndUI` 세 번째 버튼도 `RematchRequestPopup` 조건 표시 텍스트 슬롯도 **만들 필요가 없다.** 아래 「과대 표기 금지」 ① 이 적은 *"UI 자산도 아직 담지 못한다"* 는 **더 이상 남은 일이 아니다.**
  - **요청·수락 절차 수정 불필요** — 고를 조건이 없으므로 현재 요청/수락/거절 흐름을 그대로 쓴다.
  - **싱글플레이 코드 변경 0건** — 싱글 재경기는 **이미 매 판 새 맵을 만든다**(다시하기 → `GameBootstrapper.LoadMap` → 호출마다 새 root seed. 문서 작업 중 직접 실측). 규칙을 현실에 맞춘 것이지 새 사양이 아니다.
  - 🔴 **「현재 코드가 규칙 14 위반」이 해소됐다** — 종전 규칙의 「자동 시작 금지」 조항 때문에 *두 번째 요청을 받는 즉시 재경기를 시작하는* 현재 코드가 위반이었는데, **그 조항이 사라져 현재 동작이 그대로 규칙이 된다.**
- ⚠️ **해소되지 않은 것** — 🔴 **재경기 맵은 여전히 미구현이다.** 개정은 **만들 것의 범위를 줄였을 뿐 만든 것이 아니다.** 재경기하면 여전히 `Reason=MapHandoffEmpty` 로 실패한다. 아울러 **「재경기 실패 시 안내가 없다」는 별개의 다음 작업으로 [ROADMAP.md](ROADMAP.md) 에 행을 세웠다**(「경기 종료 후 UI·프로세스」 — 2026-09-14 (4차) 등록). **[🔴 2026-09-15 정정 — 원문 유지: **앞 두 문장이 무효가 됐다.** 재경기 맵은 2026-09-15 에 **구현·실기 검증 완료**이며(커밋 `20ba0ab`) **더 이상 `MapHandoffEmpty` 로 실패하지 않는다.** 🔴 **뒤 문장은 그대로 유효하다 — 「재경기 실패 시 안내가 없다」는 여전히 미해결이고 그 로드맵 행도 그대로 살아 있다**(2026-09-15 에 착수 조건만 충족돼 우선순위가 올라갔다). 규칙 M-3 의 「자동 로비 복귀 countdown 전체 길이 재시작」도 **미구현·미충족으로 남는다**]**
- ⚠️ **폴백 확인도 추가 작업이 없다** — 맵 생성기가 폴백으로 넘어가는 경우를 로그로 확인하는 수단(`MapPreparationUsedFallbackTemplate`, Warn)이 **이미 존재한다.**

**🔴 사실 확정 · 결정 기록 (2026-09-14 · 같은 날 추가 — 코드 변경 없음, 문서만):** **초기 골드 5000 의 정체와 맵 테스트 모드의 처분이 확정됐다.**

쉽게 말하면, **지금 초기 골드가 5000 인 것은 고장이 아니라 「테스트하기 편하라고 그렇게 맞춰 둔 것」**이다. 그리고 **맵마다 광산 수에 따라 초기 골드를 다르게 주기로 한 규칙은 아직 게임에 연결되어 있지 않다** — 맵은 그 값을 계산해서 상대에게 보내기까지 하는데 **받아서 쓰는 쪽이 없다.** 두 가지는 **별개의 문제**이며, 앞의 것은 **출시 때 조정**하고 뒤의 것은 **밸런싱과 함께 다시 작업**한다. 아울러 **지금은 쓰지 않는 「맵 테스트 모드」를 걷어내기로 정했다**(이번에는 기록만 하고 지우지 않는다).

- **① 초기 골드 5000 은 버그가 아니라 확정된 개발 설정이다.** 🔴 **사용자 확정**: *"테스트 단계이기 때문에 5000 골드로 설정되어 있고, **출시에는 초기 골드를 조정할 것**이다. **현재 예정은 500이지만 밸런스에 따라 조정될 여지가 있다.** 현재 **맵 테스트 모드는 따로 사용하고 있지 않다.**"* 🔴 **출처는 `Economy.StartingGold` 이며 맵 테스트 모드가 아니다** — `Resources/Config/GameConfig.asset` 의 **`_startingGold: 5000`**(29행) · **`_mapTestModeEnabled: 0`**(34행) · **`_testStartingGold: 5000`**(35행)이고, 소비 지점은 `Bootstrap/GameBootstrapper.Setup.cs` **444행 `new ResourceUseCase(_config.StartingGold)`** 하나다(문서 작업 중 직접 실측). 실기 로그의 `MapPreparationSucceeded` 에도 **`TestMode=False`** 로 찍혔다.
- **② 🔴 5000 이 서로 다른 두 필드에 우연히 같아 생기는 함정이 있다.** `Economy.StartingGold`(5000) 와 `TestStartingGold`(5000) 는 **다른 필드인데 값이 우연히 같다** — `Infrastructure/Config/GameConfig.cs` **160~162행** 주석이 이미 *"**다른 필드**다. **값이 우연히 같을 뿐**이며 … 한쪽을 고칠 때 다른 쪽이 따라 바뀐다고 가정하지 말 것"* 이라고 경고한다(직접 실측). **그래서 맵 테스트 모드를 켜도 화면상 5000 그대로라 효과가 관측되지 않는다** — 다음 사람이 **「테스트 모드가 동작하지 않는다」로 오해하기 가장 쉬운 자리**다. 🔴 **출시 때 `Economy.StartingGold` 를 500 으로 바꾸는 순간 이 우연이 깨져 두 값이 갈라진다.**
- **③ 🔴 광산 수 ↔ 초기 골드 연동은 미구현이며 「추후 밸런싱과 함께」 작업한다(사용자 결정).** 🔴 **사용자 확정**: *"광산 수에 따른 초기 골드 연동은 추후 밸런싱과 함께 조정할 것이고 다시 작업하도록 하겠다."* **`MapPreparationResult.InitialGold` 를 실제 초기 골드로 소비하는 곳은 0곳**이며 `Bootstrap/` 안의 유일한 등장은 `GameBootstrapper.Map.cs` **713행 로그 문자열**이다(전수 grep 실측). 규칙 3 의 표를 강제하는 것은 `Domain/Map/MapDefinitionValidator.cs` **270행 `GetExpectedInitialGold(testModeFlag, neutralMineCount)`**(정상 모드 **광산 1개 700 … 6개 200** / 테스트 모드 **5000 고정**)다. **즉 맵은 이 값을 계산·검증·인코딩·전송·해시 대조까지 마친 뒤 아무도 읽지 않는다.** 🔴 **출시 때 `Economy.StartingGold` 를 500 으로 바꾸는 것만으로는 규칙 3 이 지켜지지 않는다 — 그렇게 하면 광산이 1개든 6개든 매 판 500 고정이 된다. 광산 수 연동은 별개의 배선 작업이다.** 그래서 **규칙 3 과 규칙 16 이 이 항목 때문에 미충족으로 남는다.**
- **④ 🔴 맵 테스트 모드를 삭제하기로 결정했다 — 다음 작업이며 이번에는 기록만 한다.** 🔴 **사용자 확정**: *"맵 테스트 모드는 현재 특별히 사용하고 있지 않아서 불필요한 것으로 보인다. 관련 구현 내용 삭제하고 문서도 삭제하면 될 것 같다."* **영향 범위는 12개 파일**(`MapPreparationUseCase` · `MapVerificationUseCase` · `GameBootstrapper.Map` · `IMapArchetypeGenerator` · `MapArchetypeGeneratorBase` · `MapDefinition` · `MapDefinitionCodec` · `MapDefinitionValidator` · `MapFallbackTemplateFactory` · `GameConfig` · `NetworkGameManager` · `NetworkMapTransfer` — 직접 실측). 🔴 **단순 삭제가 아니다**: ⓐ **`TestModeFlag` 가 canonical 바이트 안에 있어**(`MapDefinitionCodec.cs:67`) **맵의 바이트 형식 자체가 바뀌고** ⓑ 따라서 **`MapVersion` 을 `1 → 2` 로 올려야 하며**(`MapDefinition.cs:96`) ⓒ **폴백 템플릿 5개**(`Resources/MapTemplates/MapTemplate_{FullyOpen,Canyon,ThreeLane,ObstacleOpen,Outer}.bytes`)는 **옛 형식으로 인코딩된 바이너리라 재생성이 필요하고** ⓓ **검증기 자체 점검의 테스트 모드 케이스**(`MapDefinitionValidator.cs` N9b **1532~1543행** · P9c **1545~1558행** · 표 대조 루프 **1572~1576행**)와 ⓔ **D 방식 검증**(`MapVerificationUseCase.cs` **497·518행** + 함정 주석 **31·308~311·478행**)과 ⓕ **`MapPreparationUseCase.Prepare(rootSeed, mapTestModeEnabled)` 시그니처**(**302행**)를 함께 손봐야 한다. ⓖ 🔴 **규칙 문서 개정이 선행돼야 한다 — 규칙 3·12 가 테스트 모드를 정의한다.** ⓗ ⚠️ **`MapVersion` 을 올리면 2026-09-14 에 끝낸 전송 실기 검증이 무효가 되어 재검증이 필요하다**(해시·바이트 수·D 방식 복원이 전부 새 형식 기준이 된다). ✅ **덤**: `NetworkGameManager.ReadMapTestModeEnabled()` 가 사라지므로 아래 ⑦ⓒ 의 **「조합 루트 마찰 2건」 중 신규 1건이 자동 해소**된다(기존 1건은 남는다).
- **⑤ ✅ 확인했고 문제없음 — 멀티 초기 골드 불일치.** `new ResourceUseCase(_config.StartingGold)` 가 **Host·Client 양쪽에서 각자 로컬 설정으로** 만들어지지만, `Infrastructure/Network/NetworkResourceSync.cs` 가 골드를 **서버 권위 `NetworkVariable`**(**53~62행 `readPerm: Everyone` · `writePerm: Server`**)로 내려보내고 **클라이언트가 `OnValueChanged` 에서 자기 `ResourceUseCase` 를 보정**하므로(서버만 구독 **103행**, 콜백 등록 **112~113행**) **Host 값으로 자동 수렴한다. 결함이 아니다.** 🔴 **생성 지점만 보면 양쪽이 각자 만드는 것으로 읽혀 같은 의심이 반복되기 쉬우므로 여기에 못 박아 둔다.** ⚠️ **다만 이것이 ③ 을 해소하지는 않는다** — 수렴하는 값이 **`Economy.StartingGold`** 이지 **맵이 정한 값**이 아니다.

**※ 근거 구분(규칙 10):** 사용자 확정 문장 3건은 **사용자 발언 그대로**이고, 실기 로그의 `TestMode=False` 와 초기 골드 5000 고정은 **메인 세션이 로그 원본에서 측정·전달한 값**이며, **위의 모든 파일 경로·행 번호·필드 값·표 값·12개 파일 목록·템플릿 5개 실재 여부는 문서 작업 중 직접 실측한 값**이다. ⚠️ **인계값과 어긋난 1건**: 테스트 모드 참조 **줄 수**(인계 **89줄** → 실측 **115줄**(주석 포함) / **95줄**(주석 줄 제외)). **파일 수 12개는 일치**한다. 세는 기준이 달랐던 것으로 보이나 **확정할 수 없어 추정하지 않는다.** 대조표는 `_Tasks/2026-09-08/17_29_random-map-phase3-multiplayer/Plan.md` §16-아.

⚠️ **범위 밖 — 이번 회차에 하지 않은 것:** 코드·에셋 수정 **0건**(맵 테스트 모드는 **삭제하지 않았다**). **규칙 문서**(`GameSystemRules_*.md` 등) **미수정** — 규칙 3·12 개정은 삭제 작업 착수 시 함께 한다.

---

**✅ 구현 완료 (2026-09-14) — 검증 상태: 멀티플레이 실기 검증 완료(최초 경기 2판) / 🔴 재경기 맵은 미구현 · 초기 골드는 미적용:** **무작위 맵 3단계 — 멀티플레이 맵 전송 · 해시 대조 · Client 재생성 검증 · 씬 전환 게이트.** **[🔴 2026-09-15 정정 — 원문 유지: 머리의 두 조건 중 **앞의 하나만 무효가 됐다.** **「재경기 맵은 미구현」은 더 이상 사실이 아니다** — 2026-09-15 에 구현·실기 검증까지 끝났다(멀티 재경기 **7회 연속 PASS**). 🔴 **「초기 골드는 미적용」은 그대로 유효하다 — 함께 부정하지 말 것.** 이 블록의 나머지 서술은 **`MapVersion=1` 시절의 기록으로서 유효**하며, 현재 상태의 단일 소스는 맨 위 「✅ 구현 완료 (2026-09-15)」 문단이다]**

쉽게 말하면, **이제 둘이 하는 경기에서도 매 판 다른 전장이 나온다.** 방을 만든 사람(Host)이 맵을 만드는 유일한 주인이 되고, 만든 맵을 통째로 상대에게 보내 준다. 받은 쪽은 **지문(해시)을 맞춰 보고**, 한 걸음 더 나아가 **받은 맵이 정말 그 씨앗값에서 나올 수 있는 맵인지 스스로 다시 만들어 대조**한다. **지문이 같을 때만 전투 화면으로 넘어간다.** 그 대가로 있던 임시 조치 — **양쪽이 똑같은 고정값으로 맵을 만들게 해 두었던 것** — 이 사라졌다.

🔴 **완료된 것과 완료되지 않은 것의 경계 — 이 문단이 그 단일 소스다.**
- ✅ **완료** — **최초 경기의 맵 전송.** 매 판 다른 seed · Host→Client 조각 전송 · SHA-256 해시 대조 · Client 재생성(D 방식) 검증 · 해시가 같을 때만 씬 전환. **싱글플레이 회귀도 없다.**
- 🔴 **미구현** — **재경기(Rematch) 맵.** 재경기하면 맵이 구성되지 않고 `Reason=MapHandoffEmpty` 로 실패한다. **이것은 결함이 아니라 설계대로의 동작이며**(아래 「버그로 오해하기 쉬운 항목」), 규칙 14 의 `SameMap`/`NewMap` 은 **다음 범위**다. **[🔴 2026-09-14 (3차) 갱신 — 원문은 그대로 두고 덧붙인다: 규칙 14 가 개정돼 **`SameMap`/`NewMap` 조건 선택 자체가 없어졌다.** 다음 범위인 것은 **「재경기 맵」 한 갈래**이며, **미구현이라는 사실과 `MapHandoffEmpty` 로 실패한다는 사실은 그대로다.** 맨 위 「🔴 규칙 개정 기록 (2026-09-14)」 문단 참조]** **[🔴 2026-09-15 정정 — 위 원문과 2026-09-14 (3차) 갱신을 모두 그대로 두고 덧붙인다: **이 줄 전체가 무효가 됐다.** **재경기 맵은 구현·실기 검증 완료**이며(커밋 `20ba0ab`, 멀티 재경기 7회 연속 PASS), **재경기해도 더 이상 `MapHandoffEmpty` 로 실패하지 않는다.** 결과 화면에서 재경기가 성립하면 Host 가 새 맵을 준비·전송·검증한 뒤에 씬을 재로드한다. 🔴 **`SameMap`/`NewMap` 은 「다음 범위」였던 것이 아니라 2026-09-14 규칙 14 개정으로 폐지됐고**(사용자 확정), T1 커밋 `0608f85` 에서 규칙 문서 개정까지 끝났다. 상세는 맨 위 「✅ 구현 완료 (2026-09-15)」 문단]**
- 🔴 **미적용** — **맵이 정한 초기 골드.** 규칙 16 의 「양쪽은 그 값을 그대로 적용한다」가 여전히 비어 있어 **규칙 16 이 일부 미충족으로 남는다**(아래 ④). **[🔴 2026-09-14 같은 날 정정 — 원문은 그대로 두고 덧붙인다: 「미적용」은 맞지만 **원인이 특정됐다.** 미구현인 것은 **광산 수 ↔ 초기 골드 연동**이고, 화면의 **5000 은 고장이 아니라 `Economy.StartingGold` 의 테스트 개발값**이다(맵 테스트 모드 때문이 **아니다** — `_mapTestModeEnabled: 0`). **규칙 3 도 함께 미충족**이며 **사용자가 「추후 밸런싱과 함께 다시 작업」으로 확정**했다. 맨 위 「🔴 사실 확정 · 결정 기록 (2026-09-14)」 문단 ①②③ 이 단일 소스다]**
- ⚠️ **이 세 줄을 함께 적지 않으면 거짓이 된다** — 완료된 것은 「멀티 맵 전송」이 아니라 **「최초 경기의 맵 전송」**이다. **[🔴 2026-09-15 정정 — 원문 유지: **이 한정이 풀렸다.** 재경기 전송도 2026-09-15 에 완료·실기 검증됐으므로 **이 시점 이후로 「최초 경기의」라는 한정은 더 이상 필요하지 않다.** 🔴 **다만 세 줄 중 「초기 골드 미적용」은 그대로 유효하므로 이 문단이 여전히 경계 문장을 요구한다는 취지 자체는 살아 있다.** 현재의 경계 문장은 맨 위 「✅ 구현 완료 (2026-09-15)」 문단이 갖는다]**

**🔴 실기 검증 (2026-09-14, 에디터 1 + 실기기 1) — 최초 경기 2판 모두 PASS**

| | 1판 (19:59:16) | 2판 (19:59:34) |
|---|---|---|
| Seed | `3905793782056422338` | `10943744334431763949` |
| 맵 유형 / 중립 광산 | FullyOpen / 3 | ObstacleOpen / 2 |
| 전송 바이트 / 조각 수 | **331** / **1** | **327** / **1** |
| 해시 일치 | **True** | **True** |
| 전송 / 재전송 횟수 | 1 / **0** | 1 / **0** |
| 맵 준비 시간 (시도 횟수) | 27ms (1) | 1ms (1) |
| 씬 전환 게이트 | 통과 | 통과 |

- **Client 쪽에서 D 방식 재생성 검증 통과** 확인 — `AttemptIndex=0, SearchedCombinations=2, VerifyStage=None, VerifyError=None`.
- **로비 스폰 성공** — `맵 전송 객체 스폰 완료 | Role=Host/Client, NetworkObjectId=1`. 🔴 **이 프로젝트에서 로비 씬에 네트워크 오브젝트가 존재한 것은 이번이 처음이다**(`Lobby.unity` 의 `NetworkObject` 개수는 종전 **0개**, 기존 `NetworkBehaviour` **14개는 전부 Game 씬**). 즉 **「로비에서 NGO 가 실제로 도는가」 자체가 이번에 처음 확인됐다.**
- **싱글 회귀 없음** — `20:00:04`·`20:00:07`·`20:00:10` 세 판이 각기 다른 seed(FullyOpen / ThreeLane / Outer)로 정상 동작.
- **전송 관련 예외·경고 0건.**
- 🔴 **바이트 수 공식이 실기 전송으로 확인됐다 — `319 + 4 × (중립 광산 수)`** (광산 3개 → 331, 2개 → 327, 장식 0). 그래서 **조각은 항상 1개**다. **[🔴 2026-09-15 공식 정정 — 원문 유지. 다음 작업자가 이 줄을 근거로 크기를 계산하므로 네 칸을 모두 적는다: ⓐ **옛 공식이 유효했던 시점** — `319 + 4N` 은 **`MapVersion = 1`** 의 canonical 형식에 대해 참이었고, 위 표의 **331 / 327 도 그 형식에서 실제로 측정된 값**이다(그래서 표를 고치지 않는다). ⓑ **현재 공식은 `315 + 4N`** (N = 중립 광산 수). ⓒ **바뀐 이유** — 맵 테스트 모드 삭제로 `TestModeFlag` 고정폭 정수 **4바이트가 canonical 바이트열에서 빠졌고**, 그래서 `MapVersion` 이 **`1` → `2`** 로 올라갔다(커밋 `8c83317`). ⓓ **2026-09-15 실기 9판이 새 공식과 전부 일치**했다(광산 1개 → 319 · 2개 → 323 · 3개 → 327 · 4개 → 331 · 5개 → 335). 🔴 **`MapVersion = 2` 인 지금은 `315 + 4N` 만 쓴다. 옛 공식으로 계산하면 매번 4바이트씩 크게 나온다**]**
- **D 방식 대량 재측정(시드 1~5000)**: 준비 성공 5000/5000(폴백 0) · **복원 성공 5000/5000 = 100.00%** · 평균 생성 시도 **1.51회**(최대 3) · 복원값 불일치 0건. **2단계의 「시드 40개 · 평균 1.4회」 기록을 125배 넓게 다시 잰 값이며, 옛 기록을 지우지 않는다** — 표본 크기가 다른 두 측정이다.
- **`MapChunkAssembler` 자체 검증**: 6가지 입력 **79 단언 전원 PASS**, 그리고 **모든 `Accept` 직전에 「불완전 상태에서 바이트가 나오는 일」이 0건**임을 확인(규칙 16 「부분 데이터 사용 금지」).

**단계별 결과(커밋)**: **B·E** 전송 골격 + 로그 키 5종 `df392da` / **C·D** 인계 통로 + 준비/투영 분리 `771e5e4` / **F·G·H** 전송·수신 검증 `1ffe0f2` / **I** 씬 전환 게이트 + 고정 seed 해제 `cc3a14b`. **A(NGO 씬 재로드 생존 조건)는 착수하지 않았고 「차단 요인」에서 「관찰 항목」으로 강등**됐다 — 전송이 **씬 로드 전 로비에서 전부 끝나고** 확정 맵은 객체가 아니라 정적 홀더로 씬을 건너므로 이번 범위에 그 조건이 걸리지 않는다. **생존이 실제로 물리는 곳은 재경기(다음 범위)다.**

🔴 **버그로 오해하기 쉬운 항목 — 재경기하면 맵이 구성되지 않는다. 결함이 아니라 설계대로다.** 실기 로그(`19:59:44~45`)에 `StartRematch: 동적 NetworkObject Despawn | ObjectName=NetworkMapTransfer(Clone)` → `StartRematch: Game 씬 재로드` → `[ERROR] 멀티플레이인데 로비에서 확정된 맵이 전투 씬으로 인계되지 않았다 | Event=MapPreparationFailed, Reason=MapHandoffEmpty` 가 남았다. `NetworkGameEndController.StartRematch()`(`:447~497` 실측)는 **로비로 돌아가지 않고 Game 씬만 재로드**하며 그 과정에서 전송 객체를 despawn 하고, 확정 맵 인계 통로는 첫 경기에서 이미 비워졌다(`TryTake` 는 읽고 비운다). 🔴 **`GameBootstrapper` 가 지난 판 맵으로 조용히 때우지 않고 실패로 남긴 것이 규칙 16 이 의도한 동작이다** — 이 가드가 없었다면 **양쪽이 서로 다른 맵으로 재경기를 시작했을 것**이다. **다음 사람이 버그로 접수하지 않도록 못 박아 둔다.**

**[🔴 2026-09-15 정정 — 위 문단은 한 글자도 고치지 않고 그대로 두고 덧붙인다]** 🔴 **이 문단이 설명하는 「설계대로의 동작」이 2026-09-15 에 대체됐다 — 이제 재경기해도 맵이 정상 구성된다.** 결과 화면에서 재경기가 성립하면 Host 가 **새 맵을 준비·전송·검증한 뒤에야** `StartRematch()` 로 넘어가므로 인계 통로가 비어 있는 채 씬이 재로드되지 않는다(커밋 `20ba0ab`). ✅ **`StartRematch()` 본문은 주석 외 무변경이며 위 문단이 적은 despawn 루프도 그대로다** — 전송 객체를 **결과 화면 위에서 새로 스폰**해 그 루프가 **동적 스폰이라는 이유 하나로** 정리하게 했다. 🔴 **위 문단이 인용한 `19:59:44~45` 의 `Reason=MapHandoffEmpty` 로그는 이제 「이전 빌드의 before 기준」으로 읽는다** — 같은 성격의 최신 before 기록은 `_Logs/2026-09-15/14_40_logcat/RuntimeLog_device.txt` 다. ⚠️ **그럼에도 「`GameBootstrapper` 가 지난 판 맵으로 조용히 때우지 않고 실패로 남긴다」는 가드 자체는 살아 있다** — 없어진 것은 그 가드에 걸리던 원인이지 가드가 아니다.

⚠️ **과대 표기 금지 — 아직 완료가 아닌 것:** ① 🔴 **재경기 맵은 미구현이다**(위 항목). 규칙 14 의 `SameMap`/`NewMap` 전부가 다음 범위이며, **UI 자산도 아직 담지 못한다**(`GameEndUI` 버튼 2개인데 3개 필요 · `RematchRequestPopup` 에 조건 표시 텍스트 슬롯 없음). **[🔴 2026-09-14 (3차) 정정 — 원문은 그대로 두고 덧붙인다: **뒤 절반(「UI 자산도 아직 담지 못한다」 괄호 포함)은 무효가 됐다.** 규칙 14 개정으로 **조건 선택이 없어져 세 번째 버튼도 조건 표시 텍스트 슬롯도 필요 없다.** 🔴 **앞 절반은 그대로 유효하다 — 재경기 맵은 여전히 미구현**이고, 남은 범위가 「재경기 맵」 한 갈래로 좁아졌을 뿐이다. 맨 위 「🔴 규칙 개정 기록 (2026-09-14)」 문단 참조]** **[🔴 2026-09-15 정정 — 위 원문과 (3차) 정정을 모두 그대로 두고 덧붙인다: **이제 ① 항목 전체가 무효다.** (3차) 정정이 *"앞 절반은 그대로 유효하다"* 라고 남겨 둔 **「재경기 맵은 미구현」이 2026-09-15 에 해소**됐다 — 구현·실기 검증 완료(커밋 `20ba0ab`, 멀티 재경기 7회 연속 PASS). 🔴 **원문의 「규칙 14 의 `SameMap`/`NewMap` 전부가 다음 범위」도 사실이 아니다 — 그 조건 선택 자체가 폐지됐고**(사용자 확정) **T1 커밋 `0608f85` 에서 규칙 문서 개정까지 끝났다.** ⚠️ **②③④⑤⑥⑦⑧⑨ 는 이 정정의 대상이 아니다** — 그중 ②(실기 미발화 경로)와 ③(전송 한도 실측 2건)과 ④(초기 골드 미적용)는 **2026-09-15 회차에도 그대로 미해결**이다]** ② ⚠️ **실기에서 발화하지 않은 경로가 있다** — 실패 키 4종(`MapTransferRetried`/`Failed`/`MapHashMismatch`/`MapClientVerificationFailed`) · timeout 감시 · 1회 재전송 · **게이트가 막는 쪽**. 🔴 **검증 누락이 아니다 — 2판 모두 성공이었기 때문이며, 발화시키려면 인위적 불일치 코드를 넣어야 하는데 그 여부가 사용자 미확정이다**(Plan §15 표 3번). ③ 🔴 **전송 한도 실측 2건이 남았다** — ⓐ NGO RPC 한 번의 **실효** 페이로드 상한 ⓑ **Relay 경유** 실효 MTU. 그래서 조각 크기가 아직 `IsChunkSizeMeasured = false` · `ProvisionalChunkSizeBytes = 1024`(근거 없음)이며, **규칙 16 의 「값은 NGO 실측으로 확정하고 근거와 함께 TDD 에 기록한다」가 미충족**이다. **실전에서 조각이 항상 1개인 것은 이 미충족을 없애 주지 않는다.** ④ 🔴 **맵이 정한 초기 골드는 여전히 적용되지 않는다 — 규칙 16 이 일부 미충족으로 남는다.** 로그의 `InitialGold` 는 매 판 다르게 찍히지만(**500·600·200·300**) 실제 초기 골드는 **`BlueGold=5000, RedGold=5000` 고정**(`19:59:25.110` 실측). 전송·해시·검증은 값을 **실어 나르기만 하고** 「양쪽은 그 값을 그대로 적용한다」가 비어 있다. ⚠️ **사용자가 범위로 고르지 않았고 「하지 말라」인지 「이번엔 아니다」인지 확정되지 않았으므로 사실만 적는다.** **[🔴 2026-09-14 같은 날 정정·보강 — 원문은 그대로 두고 덧붙인다: ⓐ **「5000 고정」의 출처가 `Economy.StartingGold` 임이 확정**됐다 — **맵 테스트 모드 때문이 아니다**(`_mapTestModeEnabled: 0` · 실기 로그 `TestMode=False`). ⓑ **미구현인 것은 광산 수 ↔ 초기 골드 연동**이고 이것 때문에 **규칙 16 뿐 아니라 규칙 3 도 미충족**이다. ⓒ 🔴 **출시 때 `Economy.StartingGold` 를 500 으로 바꾸는 것만으로는 규칙 3 이 지켜지지 않는다 — 매 판 500 고정이 될 뿐이다.** ⓓ **「확정되지 않았다」는 부분은 해소됐다** — 사용자가 **「추후 밸런싱과 함께 다시 작업한다」**로 확정했다. 상세는 맨 위 「🔴 사실 확정 · 결정 기록 (2026-09-14)」 문단]** ⑤ 🔴 **로비 씬에서는 로그 파일이 써지지 않는다.** `LogSessionOwner.EnsureInitialized()` 호출부가 **`LoginBootstrapper.cs:121` 과 `GameBootstrapper.cs:529` 둘뿐**이라(전수 grep 실측) **`Lobby.unity` 에서 직접 Play 하면** sink 가 0개라 콘솔 폴백(`GameLog.cs:493`)을 타고 **파일에 아무것도 안 남는다**(실제로 그 회차에 맵 전송 로그가 통째로 누락됐다). **Login 씬부터 시작하면 정상**임은 재테스트로 확인됐다. **그럼에도 맵 전송은 로비에서 일어나는 일**이라 규칙 12·16 이 요구하는 기록이 「로비 직접 Play」에서는 남지 않는 구조다. ⑥ ⚠️ **인게임 패널 백그라운드가 에디터에서만 나타나지 않는다**(사용자 보고). 실기기에서는 백그라운드를 터치하면 UI 가 닫히는데 에디터에서는 백그라운드가 안 나온다. **재현 조건·원인 미조사이며, 맵 전송과 관계있는지조차 확인되지 않았다** — 추정하지 않고 보고 내용만 적는다. ⑦ ⚠️ **구현 중 나온 판단 3건이 사용자 확인 대기다** — ⓐ `MapRootSeed` 를 `Domain/Map/` 에 둔 것(Domain 이 시계·`Guid` 를 읽는다. **Application 으로 옮기자는 판단도 성립**한다) ⓑ **범위 밖 수정 1줄**(`NetworkMapTransfer.StartHostRound` 의 `PackageTooLarge` 분기에 `_hostIsProbe = isProbe;` 추가 — 게이트에 직결돼 불가피했으나 **범위를 넘은 것은 사실**이다) ⓒ **조합 루트 마찰이 1건 → 2건**(`NetworkMapTransfer` 의 자체 조립 + `NetworkGameManager.ReadMapTestModeEnabled()` 의 `Resources.Load<GameConfig>` 직접 읽기. 둘 다 **로비에 `GameBootstrapper` 가 없다**는 같은 원인). **[✅ 2026-09-14 같은 날 추가 — 원문은 그대로 둔다: **맵 테스트 모드 삭제(맨 위 ④)를 착수하면 `ReadMapTestModeEnabled()` 자체가 사라져 신규 1건은 자동 해소**되고 마찰은 다시 1건이 된다. ⚠️ **기존 1건은 남는다** — 원인인 *"로비에는 `GameBootstrapper` 가 없다"* 가 해소되는 것이 아니므로 **사용자 확정이 필요하다는 질문은 살아 있다**]** ⑧ **`UnityEngine`·`Unity.Netcode` 참조 파일의 컴파일 통과는 Unity 에서만 확인된다** — `NetworkMapTransfer` · `GameBootstrapper` 계열 · `NetworkGameManager` 가 그렇다. **헤드리스에서 실제 컴파일·실행된 것은 순수 C# 3개**(`MapChunkAssembler` · `MapHandoff` · `MapVerificationUseCase`)뿐이다. ⑨ **보류 항목**은 2단계 §13 의 8건 중 **1번(멀티 seed 고정)이 닫히고 2번(초기 골드)이 열린 채 남았으며**, 3단계에서 **6건이 새로 생겼다** — 목록은 `_Tasks/2026-09-08/17_29_random-map-phase3-multiplayer/Plan.md` **§16**. **[🔴 2026-09-14 같은 날 갱신 — 원문은 그대로 둔다: 같은 날 **§16 에 2건이 더해져 8건**이 됐다 — **(아) 맵 테스트 모드 삭제**(🔴 **사용자 확정 결정 · 다음 작업**)와 **(자) 멀티 초기 골드 불일치**(✅ **확인 완료 · 문제없음**). **번호는 재배열하지 않았다.** 그리고 **(마) 초기 골드 항목은 원인이 특정되어 갱신**됐다]**

**※ 근거 구분(규칙 10):** 인게임 패널 백그라운드 보고와 화면에서 본 경기 동작은 **사용자 실기 관찰**, 위 실기 표·로그 지문·초기 골드 고정값·싱글 3판 seed·`MapChunkAssembler` 79 단언·D 방식 시드 1~5000 재측정·**커밋 해시 4개**·로비 `NetworkObject` 0개는 **메인 세션이 로그 원본과 `mono` 실행으로 측정·전달한 값**이며, `LogEvent` 46개·`IsChunkSizeMeasured`/`ProvisionalChunkSizeBytes`·`EnsureInitialized` 호출부 2곳·`GameLog.cs:493`·`StartRematch` 447~497행·`ReadMapTestModeEnabled` 의 `Resources.Load`·신설 파일 행수는 **문서 작업 중 직접 실측한 값**이다. ⚠️ **인계값과 어긋난 2건은 실측값으로 적었다** — `NetworkMapTransfer.cs` 행수(인계 1500 → 실측 **1607**) · `StartRematch()` 범위(인계 `:447~492` → 실측 **`:447~497`**). 대조표는 Plan §16-사.

---

**⚠️ 멀티플레이 실기 검증 (2026-09-09) — 검증 상태: 2단계 결과의 멀티플레이 실기가 처음 이뤄졌다 / 부분 검증:** **무작위 맵 2단계 — 멀티플레이(랜덤 매칭) 실기 2회.** **[🔴 2026-09-14 — 원문은 그대로 두고 덧붙인다: 이 문단이 「검증 경계의 단일 소스」였던 것은 2026-09-09 시점의 사실이다. 아래 ⚠️ 항목이 적은 **두 가지 미검증이 3단계로 전부 해소**됐다 — ㉠ 「멀티에서 매 판 다른 맵이 나오는가」(3단계 I 가 고정 seed 를 없앴고 실기에서 매 판 다른 seed 확인) ㉡ 「칸 단위 대조가 아니다」(해시 대조 = 바이트 단위 대조로 대체됨). **현재 경계의 단일 소스는 맨 위 「✅ 구현 완료 (2026-09-14)」 문단이다.**]**
...
- 초기 골드 전용 `MapTestModeEnabled` 확정: ON=5000, OFF=광산 수 표. 멀티플레이는 Host 표식·실제 골드 권위 **[🔴 2026-09-15 갱신 — 원문은 2026-07-19 기획 확정 시점의 기록이라 그대로 두고 덧붙인다: **이 항목은 2026-09-14 규칙 3 개정으로 기획에서 제거됐고, 2026-09-15 커밋 `8c83317` 로 코드·`GameConfig.asset` 에서도 실제로 삭제됐다.** 초기 골드 갈래는 **광산 수 표 하나**만 남는다. 권위 규정은 「Host 가 생성·확정한 `MapDefinition` 이 권위」로 대체됐다]**
- 국소 건설 불가 구역, 완전 차단 지형, 결정적 seed·독립 PRNG 스트림·100회 재시도·검증된 폴백 규칙 확정
- canonical binary + SHA-256, persistent chunk 전송, `SameMap`/`NewMap`, 임시 `MapVersion=1` 계약 확정 **[🔴 2026-09-14 (3차) 갱신 — 원문은 그대로 두고 덧붙인다: 이 줄은 **2026-07-19 기획 확정 시점의 계약 목록**이다. 그중 **`SameMap`/`NewMap` 은 2026-09-14 규칙 14 개정으로 폐기**되어 현재 계약은 **「재경기 맵 — 조건 선택 없이 매 판 새 맵」** 한 갈래다. 같은 줄의 나머지 셋(canonical binary + SHA-256 · persistent chunk 전송 · 임시 `MapVersion=1`)은 **그대로 유효하다.**]** **[🔴 2026-09-15 갱신 — 위 두 기록을 그대로 두고 덧붙인다: **`MapVersion` 은 더 이상 `1` 이 아니다.** 테스트 모드 표식이 canonical 바이트열에서 빠지면서 **`1` → `2`** 로 올라갔고(`MapDefinition.CurrentMapVersion = 2`), 그 결과 **canonical 바이트 수 공식이 `315 + 4N`**(N = 중립 광산 수)이 됐다. canonical binary + SHA-256 · persistent chunk 전송 계약 자체는 그대로다]**
...
- **상태 (2026-09-03 갱신):** 문서 설계 완료. **1단계(타일 상태 계약 전환)만 구현·에디터 실기 검증 완료** — 상세는 위 「구현 완료 (2026-09-03)」 문단과 「완료된 시스템 > 코어 게임플레이」 표의 **타일 상태 계약** 행. **런타임 생성기·검증기·전송·테스트 모드·건설 불가/차단 지형·경로 완전 차단 대응은 여전히 미구현**이다(2·3단계).
  - **[🔴 2026-09-08 갱신 — 위 문장은 그대로 두고 덧붙인다]** **2단계 A~J 가 구현·싱글 실기 검증까지 끝났다.** 위 문장이 「여전히 미구현」이라고 적은 것 중 **런타임 생성기 5종 · 검증기 · 폴백 5개 · 테스트 모드 설정 필드 · 건설 불가/차단 지형 · 맵 준비 조정자 · `MapDefinition` → `HexGrid` 투영 · 렌더러 · 건설/점령/AI 판정 조건**은 **구현됐다.** **여전히 미구현인 것은 ① 2단계 K(맵 준비 로그 키) ② 맵 전송·해시 대조·재경기(3단계) ③ 경로 완전 차단 대응**이다. 상세는 위 「⚠️ 진행 중 (2026-09-08)」 문단. **[🔴 2026-09-08 갱신 — 원문은 그대로 두고 덧붙인다]** **①(2단계 K)은 커밋 `2e1b305` 로 완료**되어 2단계는 A~K 전부 끝났다. **남은 미구현은 ②와 ③뿐**이며, 상세는 위 「✅ 구현 완료 (2026-09-08)」 문단이다. **[🔴 2026-09-14 갱신 — 원문은 그대로 두고 덧붙인다]** **②(맵 전송·해시 대조) 중 「맵 전송·해시 대조」가 3단계로 구현·실기 검증까지 끝났다.** ②에 남은 것은 **재경기(`SameMap`/`NewMap`)** 이고(**[🔴 2026-09-14 (3차) 갱신: 규칙 14 개정으로 조건 선택이 없어져 남은 것은 **「재경기 맵」 한 갈래**다. **미구현이라는 사실은 그대로**]**), **③(경로 완전 차단 대응)은 그대로 미구현**이다(그 항목은 맵이 아니라 유닛 행동이라 `GameSystemRules_Units.md` 규칙 45 가 단일 소스이며, 조사조차 되지 않았다). 상세는 위 「✅ 구현 완료 (2026-09-14)」 문단. **[🔴 2026-09-15 갱신 — 위 기록 전부를 그대로 두고 덧붙인다]** **②에 남아 있던 「재경기 맵」이 구현·실기 검증까지 끝났다**(멀티 재경기 7회 연속 성공). 함께 **「테스트 모드 설정 필드」는 구현됐다가 삭제됐다** — 위 2026-09-08 갱신이 「구현됐다」고 적은 그 항목이며, **2026-09-15 커밋 `8c83317` 로 코드·`GameConfig.asset` 에서 제거되고 `MapVersion` 이 `1` → `2` 가 됐다.** 🔴 **그래서 ②는 닫혔고, 남은 것은 ③(경로 완전 차단 대응) 과 「맵이 정한 초기 골드 적용」, 그리고 「맵 준비·전송 실패 표시 UI」 셋이다.** 상세는 맨 위 「✅ 구현 완료 (2026-09-15)」 문단.
  - **이 절이 「확정 설계 — 구현 예정」에 남아 있는 것은 여전히 맞다** — ~~2단계가 K 를 남겨 미완이고~~ **[🔴 2026-09-08 갱신: 2단계는 완료됐다]** **3단계(맵 전송·해시 대조·재경기)가 미착수**이기 때문이다. **[🔴 2026-09-14 갱신 — 원문은 그대로 두고 덧붙인다: 3단계 중 **맵 전송·해시 대조는 끝났고 재경기가 남았다.** 이 절이 여기 남아 있는 것은 **여전히 맞다** — 재경기(`SameMap`/`NewMap`)와 경로 완전 차단 대응이 미구현이고, 초기 골드 적용은 규칙 16 이 요구하는데 비어 있기 때문이다]** **[🔴 2026-09-14 (3차) 갱신 — 위 문장도 그대로 두고 덧붙인다: 규칙 14 개정으로 `SameMap`/`NewMap` 조건 선택이 없어져 남은 것은 **「재경기 맵」 한 갈래**다. **이 절이 「확정 설계 — 구현 예정」에 남아 있는 것은 여전히 맞다** — 범위가 좁아졌을 뿐 **재경기 맵은 미구현**이기 때문이다]** **[🔴 2026-09-15 갱신 — 위 문장들을 그대로 두고 덧붙인다: **그 근거가 소진됐다. 재경기 맵은 구현·실기 검증 완료이며 무작위 맵 본체(1~3단계)는 전부 끝났다.** 이 절이 아직 「확정 설계 — 구현 예정」 아래에 있는 이유는 **잔여 3건**뿐이다 — ① **경로 완전 차단 대응**(맵이 아니라 유닛 행동. `GameSystemRules_Units.md` 규칙 45) ② **맵이 정한 초기 골드 적용**(규칙 3·16 미충족. [ROADMAP.md](ROADMAP.md) 에 별도 행) ③ **맵 준비·전송 실패 표시 UI**(규칙 M-3·M-4 의 팝업 여부가 미정. [ROADMAP.md](ROADMAP.md) 에 별도 행). ⚠️ **이 절을 「✅ 완료된 시스템」으로 옮길지는 문서 구조 변경이라 이번에 하지 않았다 — 사용자 판단 대상이다**(CLAUDE.md 규칙 12)]**

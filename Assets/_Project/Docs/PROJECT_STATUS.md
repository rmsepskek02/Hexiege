...
- 🔴 **미검증 — 알림 팝업이 화면에 뜨는 모습은 아무도 본 적이 없다.** **`UIManager.ShowAlert` 의 실호출처가 0건**이기 때문이다(직접 실측). 실제로 띄우는 것은 **단계 8**(규칙 D-6 요청자 이탈)과 **단계 10**(규칙 M-3·M-4 맵 실패)이며 둘 다 미착수다. ⚠️ **「구현했다」와 「실기에서 확인됐다」는 다른 말이고 이 항목은 앞쪽까지만 참이다.**

...
#### 스킬 건물 시스템 Phase 1 (타입 A·B + 조준/UI/좌표화/렌더링/버그수정) ✅ 구현 완료 · 실기기 테스트 PASS (2026-08-04)

스킬 건물(FlightFacility / MagicSpirit / WillowShrine)로 액티브 스킬을 발동하는 프레임워크와 타입 A(즉발 범위 피해)·B(장판 DoT) 실행기, 모바일 탭 조준, 전용 UI를 구현하고 실기기 테스트로 검증. SSoT는 `GameSystemRules/GameSystemRules_Skills.md`(규칙 1~26). task: `_Tasks/2026-07-28/12_14_skill-building-system-design/`(프레임워크 설계·Phase 1) + `_Tasks/2026-08-04/04_46_skill-aim-coordinate-based/`(조준 좌표화·렌더링·버그수정).

| 항목 | 상태 | 비고 |
|------|------|------|
| 프레임워크 골격 (데이터 주도) | ✅ 완료 | Domain `Skill/SkillMechanicType.cs`(A/B/C, **C는 선언만**)·`Skill/SkillAimType.cs`(Instant/PointTarget). Application `Skill/{SkillData,ISkillExecutor,SkillActivationContext,SkillExecutorRegistry}`·`UseCases/SkillActivationUseCase.cs`(발동 단일 진입점 `Activate`, 플레이어/AI 공유). 건물 글로벌 쿨다운=UseCase `Dictionary<int,float>`(BuildingData 무변경). 로드아웃=RaceId 키(`SkillLoadoutConfig:ISkillDataProvider` — MagicBuilding enum 공유라 종족 분기). 규칙 1~7 |
...
| 문서 | ✅ 완료 | `GameSystemRules/GameSystemRules_Skills.md`(규칙 1~26, 조준 좌표화 정정 17·19·22·22-1·24·26 반영·구현 상태 갱신) |

> **⏳ 후속 / 미완 (별도 작업 — 과대 표기 금지):** ① **타입 C(전역 상태변경: 버프/디버프/CC/힐) — Phase 2 미구현.** `SkillMechanicType.GlobalStatusChange`는 **enum 멤버만 선언**, 실행기 미구현(`SkillExecutorRegistry` TODO). 업그레이드 연동 결정(규칙 9-x)도 C와 함께 예정. ② **건물 파괴 시 패널/조준 UI 원복 미구현** — 별도 작업으로 분리(스킬 포함 4개 건물 패널 공통 갭: `BuildingPanelBase`에서 `OnBuildingDied` 구독→Close 방식 제안됨, 미착수). ③ **구체 스킬 목록·수치(기획)** — 보류(각 슬롯 1~5의 스킬·쿨다운·반경·지속·피해·상태효과는 추후 ScriptableObject로 확정).
...
> **⏳ 후속 / 보류 (별도 작업 — 과대 표기 금지):** ① 연구 패널 UI 레이아웃 다듬기(현재 에디터 스크립트 자동생성 골격, 사용자가 Unity에서 직접 다듬을 예정) ② 매트릭스 헤더 아이콘(공/방/속·그룹, 텍스트 라벨 대체 중) ③ AI 시나리오 연구 사용(코드는 있으나 실기 미검증) ④ MistShrine(HealShrine) 힐 미구현 ⑤ 싱글플레이 자연회복 실기 미검증(코드상 정상 예상). ×10 적용·배선·디버그에 쓰였던 에디터·디버그 스크립트는 config `.asset` 커밋·씬 배선 반영 후 제거됨.

...
- 🔴 **미구현** — **재경기(Rematch) 맵.** 재경기하면 맵이 구성되지 않고 `Reason=MapHandoffEmpty` 로 실패한다. **이것은 결함이 아니라 설계대로의 동작이며**(아래 「버그로 오해하기 쉬운 항목」), 규칙 14 의 `SameMap`/`NewMap` 은 **다음 범위**다.

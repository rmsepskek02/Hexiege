...
**실기 검증 완료 (2026-08-24) — ✅ 멀티 3경기 회귀 없음 · ⚠️ 8곳 중 6곳은 발화 확인 불가:** **네트워크 종료 시점 가드 8곳(커밋 `bcf45ec1`)의 실기 검증** — 코드 변경 없음, 문서/메모리만. **[쉬운 말로]** 2026-08-20 에 네트워크가 꺼지는 순간의 안전장치 8곳을 넣어 두고 **실제로 게임을 돌려 보지는 못한 상태**였다. 이번에 멀티플레이 **연속 3경기**를 플레이해 그 기록을 받았고, **13,003줄 전체에 오류가 한 건도 없으며 세 경기 모두 끝까지 정상 진행돼 승패가 났다.** 안전장치는 원래 *"아무 일도 안 일어나게 하는 것"* 이 목적이라 고쳐진 것을 보여 줄 화면이 없다 — 그래서 볼 것은 **「멀쩡하던 것이 망가지지 않았는가」** 였고, 망가지지 않았다. **[⭐ 이번 판이 특별한 이유 — 클라이언트 쪽 기록을 처음 남겼다]** 무작위 매칭이라 **1경기는 호스트, 2·3경기는 클라이언트**가 됐다(`IsServer=True, IsHost=True` 33행 → `IsServer=False` 481·5541행). 과거 실기 로그는 **전부 호스트 쪽**이었다 — 2026-08-19 로그(1,408행) 재실측 결과 `네트워크 스폰 | IsServer=False` **0건** · `IsServer=True` **30건**으로 **3경기 내내 호스트**였다. **[핵심 지표 — 전부 직접 재실측]** `[ERROR]` **0건**(13,003행) · 승자 발표 **3회**(447·5536·12993행) · `게임 종료 — 전투 틱 정지` **1경기에 정확히 1회**(446행) · `NetworkControllerSpawnedWithoutGameServices` **0건**(스폰 레이스 미재현) · 사용자 육안 확인 *"특별히 문제없어보였어"*. **[🔴 `[WARN]` 1,421건은 새로 생긴 문제가 아니다 — 「알려진 현상(정상 동작)」]** 내역은 `InitializeUnitView: 해당 UnitId 의 GameObject 가 아직 없다` **1,099건** + `SpawnUnitClientRpc — UnitView 초기화 지연. 재시도 대기` **319건** + 연결 끊김/재접속 **3건**(경기 전환 시점)이다. 근거 4가지 — ① **재시도가 전부 성공한다**: `재시도 대기` 319건 ↔ `RetryInitializeUnitView — 초기화 성공` **319건**, **재시도 소진·초기화 실패 0건**, 대기 시간 실측 분포 **0.01초 1 · 0.02초 51 · 0.03초 192 · 0.04초 70 · 0.05초 3 · 0.06초 2**(최빈 0.03초, 최대 0.06초) ② **1,099건 전부가 클라 전환 뒤**다 — 첫 발생 937행 `12:17:35.694`, **1경기 구간(1~473행)에는 0건** ③ `SpawnUnitClientRpc` 수신부는 **클라이언트 전용 경로**이고 `bcf45ec1` 의 가드는 **전부 서버 측**이며 `ClientRpc` 수신부의 `if (IsServer) return;` 10곳은 손대지 않았다 ④ 경고 코드 자체는 **2026-07-19부터 존재**(※ 이 항목만 `git log -S` 결과 = **호출 세션이 측정한 값**이며 규칙 5로 재검증하지 않았다). **⚠️ 다음 사람을 위한 함정 경고:** 2026-08-19 로그에 이 경고가 **0건**인 것만 보고 *"이번에 생겼다"* 고 결론내면 **틀린다** — 그 세션은 3경기 내내 호스트였다. **두 로그의 차이는 "코드가 바뀐 것"이 아니라 "기록한 쪽이 바뀐 것"이다.** 남길 가치가 있는 사실은 **NGO 스폰 순서 경합이 실재하며 재시도로 100% 흡수되고 있다**는 것이고, 숫자가 커 보이는 것은 경합이 심해서가 아니라 **한 유닛당 여러 번 찍히기 때문**이다(1,099 대 319). **[⚠️ 과대 표기 금지 — 해소되지 않은 것 4가지]** ① **8곳 중 서버 발화가 로그로 확인된 것은 2곳뿐** — ③`OnGameEndServer`(447행 1회)·⑧`OnUnitProduced`(1경기 189회). 나머지 6곳은 **가드 아래 본문에 호출당 로그가 없어 셀 수 없다**(Plan §6-4 *"가드에 로그 0건 추가"* 결정의 대가) ② **`_combatStopped` 재경기 리셋 미검증** — 가드가 `NetworkCombatController.cs` 380행 `if (!IsSpawned || !IsServer || _combatStopped) return;` 이라 **2·3경기 클라이언트에서는 `!IsServer` 에서 먼저 반환**된다. 2·3경기 사망 로그는 전부 `EntityDiedClientRpc 수신 → 클라 처리` 경로(`서버: 유닛 사망` **0건**)로 **상대 호스트의 틱이 돈 것**이다. 확인하려면 **에디터가 호스트로 연속 2경기**를 해야 한다 ③ **B(엔진 오류 수집) 여전히 미검증** — 이번에도 `[ERROR]` 0건이라 **잡을 대상이 없었다**(*수정이 잘 돼서 검증 기회가 사라지는 역설*이 계속된다) ④ **스폰 레이스 미재현**. **[인계 수치 3건이 실측과 어긋나 실측값을 썼다 (규칙 10)]** 경기별 유닛 사망 인계 `135/128/358` → 실측 **1경기 서버 유닛 129+건물 5 · 2경기 클라 유닛 62+건물 2(수신 64) · 3경기 클라 유닛 175+건물 4(수신 179)** / 재시도 대기 인계 *"30ms 안에"* → 실측 **0.01~0.06초** / `서버 모드로 골드 동기화 시작` 에 인계문은 `| IsServer=True` 를 붙였으나 **37행 실제 문자열에는 없다**(그 접미가 붙은 것은 43행 `NetworkTileSync` 쪽). `NetworkCombatController` 총 건수 **137/129/359** 와 나머지 지표는 인계값과 일치. 상세: task `_Tasks/2026-08-20/12_24_network-guard-sweep/Plan.md` **§14**, `LogRules.md` **1.13**「2026-08-24」.

**문서/메모리 정비 (2026-08-24, 후속 2건) — 죽은 윈도우 경로 마지막 잔존 제거 + QA 기록 중복 해소 (문서/메모리만, 코드 변경 없음):** **① `AGENTS.md` 2건.** 2026-08-20 사고 원인 ②(죽은 윈도우 절대경로)의 **마지막 잔존**이 `AGENTS.md` **116행**(「에이전트 메모리」 표 마지막 행 — 행 내용이 `C:\Users\rmsep\.claude\projects\...\memory\` 하나뿐이고 「✅ 항상」으로 표시돼 **존재하지 않는 파일을 매번 갱신하라고 지시**하고 있었다)과 **155행**(「완료 후 업데이트 체크리스트」 마지막 행)에 있었다. 116행은 **삭제**, 155행은 조건(「모든 작업 완료 후」)이 유효하므로 경로만 **`.claude/MEMORY.md`(에이전트 공용)로 교체**했다. **⚠️ 왜 두 번이나 놓쳤나 — 이번 라운드의 요점:** 종전 정리는 `Dmain`·`D:/dev` 문자열로 **`.claude/` 하위만** 훑었는데, 이 둘은 문자열이 `C:\Users\rmsep`(`C:/Users` 변형 포함)였고 위치도 **리포지토리 루트**였다. *"`.md` 만 훑고 정리 끝이라고 쓰지 말 것"* 이라는 2026-08-20 교훈에 **"찾는 문자열과 찾는 위치를 둘 다 넓혀야 한다"** 가 더해진다. 이번엔 확장자·디렉터리 제한 없이 전수 조회했고(빌드 산출물 폴더 제외) 잔존은 **`WORK_HISTORY.md`·`ROADMAP.md`·`_Tasks/` 의 「이 경로를 지웠다」는 기록 3건뿐**임을 확인했다. **② `qa_history.md` 중복 절 제거.** 「종족 인게임 적용 QA (2026-04-07 완료)」가 **143행과 522행에 두 벌** 있었다(직전 라운드 이관 때 원본이 `qa-tester/MEMORY.md` 와 이 파일 양쪽에 있어 겹쳤다). **파이썬으로 11행씩 대조해 소제목 한 줄(`정적 분석 발견 사항` ↔ `핵심 발견 사항`) 외 완전 일치**임을 확인한 뒤 522행 사본을 제거하고, **삭제 사유를 남은 절 아래에 명시**했다(`.claude/MEMORY.md` 「🔴 에이전트 메모리 갱신 규칙」 3 — 삭제는 틀렸다고 확인했을 때만 + 이유 명기). 낡아진 203행 경고 주석도 함께 현행화. **[⭐ 검사 `[7]` 이 설계대로 작동한 첫 사례]** 이 삭제로 qa-tester 폴더 총합이 줄자 검사 `[7]` 이 **756 → 754(-2행), 파일 3개 → 3개**로 즉시 **실패(EXIT=1)** 시켰다 — *"파일 수 그대로 + 총합 감소 = 이동이 아니라 삭제"*. 기준값은 **`--update-baseline` 플래그로만** 내려가므로 **"이 삭제는 의도된 것"이라는 판단을 사람이 하도록 강제**된다. **자동 갱신이었다면 이 감소가 조용히 새 기준이 됐을 것이다.** ⚠️ 다만 기준값이 2026-08-21 값(756)에 머물러 직전 라운드의 증가(760)가 반영돼 있지 않아, **실제 삭제 -6행이 -2행으로 보고**됐다 — **증가 방향 갱신을 미루면 감소 폭이 왜곡된다.** 갱신 후 재검사 **0건 · 종료 코드 0**. `--update-baseline` 은 `change_log` 에 사유를 **자동으로 남기지 않으며**, `measured_at` 도 2026-08-21 그대로다(문서 에이전트는 `_baseline.json` 을 직접 편집하지 않는다 — 사용자 판단 대기). `known_orphans` 의 `project-orchestrator/roadmap-3d.md` 는 직전 라운드에 링크돼 **제거 가능한 상태이나 이번 범위가 아니라 손대지 않았다.**

**문서/메모리 정비 (2026-08-24) — 에이전트가 과거 상태를 현재로 오해하던 구조를 제거 (문서만, 코드 변경 없음):** **[가장 중요하게 기록할 것]** `project-orchestrator/MEMORY.md` 에 **「프로젝트 현재 상태」라는 똑같은 제목의 절이 7개** 있었다 — 2026-08-08 / 07-31 / 06-23 / 04-13 / 04-06 / 03-26 + 「이전 상태 (2026-03-19)」(77행). 전부 같은 제목이라 **에이전트가 이 파일을 읽고 2026-03-19 상태를 현재로 착각할 수 있었다.** 총괄 에이전트는 위임할 때 「현재 프로젝트 상태 요약」을 필수 컨텍스트로 넘기므로, 그 오해는 다른 에이전트로 그대로 전파된다. **[조치]** 2026-08-08 절만 **「프로젝트 현재 상태 — 기준일 2026-08-08 (이 절만이 현재 상태다)」** 로 제목을 고쳐 남기고, 나머지 6개 + 「2026-03-14 완료 작업(비-네트워크)」 + 「3D 전환 시 수정된 파일」 + 2026-07-16/07-18 인증 플로우 진행 기록을 신규 토픽 `project-history.md`(253행, 서두에 *"이것은 과거 스냅샷 아카이브다. 현재 상태는 `MEMORY.md` 를 보라"* 명시, 날짜 역순)로 옮겼다. 같은 정비를 **`qa-tester`**(지나간 QA 기록 18건, 2026-04-07~07-20 → 기존 `qa_history.md`, 193 → 532행)와 **`game-design-lead`**(「핵심 시스템별 확정 설계」 → 신규 `core-systems.md` 116행 / 「싱글플레이 AI 시스템」 → 신규 `ai-scenarios.md` 72행)에도 적용하고 **세 폴더 모두에 「토픽 파일 인덱스」 절을 신설**했다(링크 없는 토픽은 존재하지 않는 것과 같다 — `.claude/MEMORY.md` 「🔴 에이전트 메모리 갱신 규칙」 5). **[성격 — 과대 표기 금지]** 이 작업은 **손실 방지가 아니라 품질 개선(오해 방지 + 매 작업 읽는 비용 절감)** 이다. 2026-08-21 프로브로 *"200행 초과분이 에이전트에게 안 보인다"* 는 전제가 거짓임이 확인됐으므로(메모리는 잘리는 게 아니라 **애초에 0행이 주입**되고 `Read` 에는 행 수 제한이 없다), 행수 자체는 문제가 아니었다. **[손실이 아님을 증명한 방법]** 2026-08-17 `-378행` 사고가 *"옮긴다면서 지운 것"* 이었으므로 모든 이동을 **`Read`→`Edit`**(`Write` 금지)로 수행하고 **폴더 총합 행수가 줄지 않았음**을 검사 `[7]` 로 확인했다 — **qa-tester 756 → 760 · project-orchestrator 549 → 592 · game-design-lead 254 → 285**(`MEMORY.md` 본체는 502 → 167 · 325 → 115 · 254 → 97). 옮긴 본문은 한 글자도 고치지 않았고 바뀐 것은 **현재 상태로 오독되던 절 제목 6개뿐**이다. 작업 전/후 `python3 Tools/check_docs.py` 모두 **0건 · 종료 코드 0**. **[함께 해소]** 고아로 예외 등록돼 있던 `project-orchestrator/roadmap-3d.md`(224행)를 새 인덱스에 링크해 검사 `[6]` 의 「알려진 예외」가 **0건**이 되었다(`_baseline.json` 의 `known_orphans` 정리 여부는 사용자 판단 대기 — 문서 에이전트는 그 파일을 직접 편집하지 않는다). 검사기가 **5종 → 7종**이 되면서 낡아진 「검사 5종」 문구는 `WORKFLOW.md` [11]③ · `AGENTS.md` 도구 표 · `.claude/agents/document-manager.md` 에서 갱신했다. `CLAUDE.md` 는 **사용자와 메인 세션의 소통 전용 문서로 확정**되어 열지 않았다(낡은 문구도 발견되지 않았다 — 100행은 검사 항목 수를 적지 않은 일반 서술이라 여전히 유효).

**전제 정정 (2026-08-21) — 「200행 초과분은 에이전트에게 보이지 않는다」는 거짓이었다 · 끊긴 도달 경로 연결 (문서/설정만, 코드 변경 없음):** **[어떻게 확인했나]** qa-tester · game-design-lead · asset-prompt-crafter · project-orchestrator **4개 에이전트에게 「도구를 하나도 쓰지 말고 시스템 프롬프트 내용만으로 답하라」** 고 지시한 프로브를 돌렸고 **4건 결과가 일치**했다. **[층별 자동 주입 표]** `CLAUDE.md` **✅ 예**(규칙 5 번호 · `2026-03-03` 날짜 · 체크리스트 [1][3]까지 정확 인용) · 에이전트 정의 `.claude/agents/<이름>.md` **✅ 예**(단 **자기 것만**, 6개가 각각 별도 파일) · `AGENTS.md` **❌ 아니오** · `.claude/MEMORY.md`(공용) **❌ 아니오** · 각 에이전트 `MEMORY.md` **❌ 아니오**. **[결론]** 메모리는 **200행에서 잘리는 것이 아니라 애초에 0행이 실린다.** 에이전트가 메모리를 아는 유일한 방법은 **`Read` 로 직접 여는 것**이고 **`Read` 에는 200행 제한이 없다.** 따라서 종전의 **「200행 절삭 = 손실 경로」 서술은 폐기**되고, 손실 경로는 **①덮어쓰기 · ③고아 토픽 2개**만 남는다. **[함께 드러난 사실]** **`AGENTS.md` 도 자동 주입되지 않는다** — 문서 인덱스인데 도달 경로가 `.claude/MEMORY.md` 경유뿐이라, 그 `.claude/MEMORY.md` 마저 자동 주입이 없어 **인덱스 전체가 사실상 죽은 경로**였다. **[조치 — 완료]** ① **에이전트 정의 6개 전부**(`asset-prompt-crafter` · `document-manager` · `game-design-lead` · `game-programmer` · `project-orchestrator` · `qa-tester`)에서 거짓 문장 *"`MEMORY.md` is always loaded into your system prompt — lines after 200 will be truncated"* 를 **삭제하고** *"자동으로 실리지 않는다 — 작업 시작 전 `Read` 로 직접 열어야 한다"* 로 교체(`document-manager` 는 *"keep it under 200 lines"* 변형이었고 같은 전제라 함께 교체). 잔존 **0건** 확인. ② 같은 6개 파일에 **포인터 2줄**을 넣어 죽은 경로를 이었다 — **`.claude/MEMORY.md` 를 `Read` 하라** + **자기 `.claude/agent-memory/<이름>/MEMORY.md` 가 인덱스다**. **규칙 본문은 복사하지 않고 경로 포인터만** 넣었다(2026-08-20 사고 원인 ①이 *"정의 파일에 박아둔 내용이 시간이 지나 거짓이 된 것"* 이었으므로, 규칙이 바뀌어도 6곳을 고칠 필요가 없게 한다). 2026-08-20 `405538c7` 의 `Read`→`Edit`·`Write` 금지 경고문은 **프로브로 실제 도달이 확인되어 그대로 유지**했다. ③ `.claude/MEMORY.md` 의 「🔴 에이전트 메모리 갱신 규칙」 4번을 **「인덱스 + 매 작업마다 필요한 것만 담는다 / 경고선 250행 / 토픽 파일은 상한 없음」** 으로 교체(250행 근거는 실측 — 인덱스형 game-programmer 132 · document-manager 131 · asset-prompt-crafter 166 vs 이력 누적형 game-design-lead 254 · project-orchestrator 325 · qa-tester 502), 5번은 **「자동 주입이 없으므로 인덱스 링크가 유일한 발견 경로」** 로 보강, 6번은 유지. **자동 주입 경계 표**와 **도달 경로**를 파일 상단에 명시했고, **자동 주입되는 `CLAUDE.md` 를 중복 기재하던 「절대 규칙 참조」 절은 제거**(사본은 원본이 바뀌면 조용히 거짓이 되므로), **`WORKFLOW.md` 는 자동 주입되지 않아 경로를 유지**했다. **[⚠️ 가장 중요한 결론]** 이 발견으로 **「3개 에이전트 1,081행 분산」이 불필요한 작업이었음이 드러났다.** 없는 문제를 고치려고 **1,081행을 옮길 뻔했고, 그 이동 자체가 2026-08-17 `-378행` 과 같은 모양의 실제 손실 위험**이었다. **[⚠️ 교훈]** 2026-08-20 사고의 두 원인(**거짓 문장** / **죽은 경로**)과 **같은 종류의 세 번째**가 `lines after 200 will be truncated` 였다 — **에이전트 프롬프트에 적힌 사실 주장은 검증 없이 인용하면 안 된다.** 이번엔 그 한 줄이 `PROJECT_STATUS.md` · `WORK_HISTORY.md` · `ROADMAP.md` · Task Plan 까지 **여러 문서로 퍼졌다.** **※ 근거 구분(규칙 10):** 자동 주입 표는 **프로브 4건 실측**, 행수는 **실측**, 커밋 해시·증감 수치는 **호출 세션이 측정한 값**이며 규칙 5(git 명령 금지)로 이 작업에서 재검증하지 않았다.

**구현 완료 (2026-08-20) — ~~⚠️ 코드 적용 완료 · 실기 미검증~~ → 🔴 2026-08-24 갱신: ✅ 멀티 실기 3경기 회귀 없음 확인 (단, 8곳 중 6곳은 발화 횟수 확인 불가 — 아래 「[🔴 2026-08-24 실기 검증 결과]」 참조):** **네트워크 종료 시점 가드 전수 보강 8곳** (커밋 `bcf45ec1`, Plan 커밋 `2f255b33`). **[무엇을 했는가]** 직전 커밋 `55d24d83` 이 `NetworkCombatController` **한 파일**의 가드 구멍 6곳을 고친 뒤, **같은 성질의 구멍이 다른 파일에도 있는지 전수 조사**해 **나머지 5파일 8곳**을 보강했다. 전부 **`if (!IsSpawned || !IsServer) return;` 한 가지 형태**다 — ① `NetworkResourceSync.OnResourceChangedOnServer`(188행, **가드 전무 → 신설**) ② `NetworkTileSync.OnTileOwnerChangedOnServer`(164행, **가드 전무 → 신설**) ③ `NetworkGameEndController.OnGameEndServer`(178행) ④ `NetworkHealthSync.OnEntityDamaged`(131행) ⑤ `NetworkHealthSync.OnEntityHealed`(186행) ⑥ `NetworkProductionController.OnProductionStarted`(209행) ⑦ 〃 `OnProductionQueueChanged`(253행) ⑧ 〃 `OnUnitProduced`(319행) — ③~⑧ 은 `!IsServer` 만 있던 줄을 **대체**한 것이라 줄이 늘지 않는다. 각 진입점은 **`ClientRpc` 를 보내거나(7곳) `NetworkVariable` 을 쓴다(①)** — 전부 "네트워크가 살아 있음" 을 전제하는 동작이다. **[가드가 없던 ①②에 `IsServer` 가 새로 붙지만 동작은 바뀌지 않는다]** 두 구독 모두 `OnNetworkSpawn` 의 `if (IsServer)` 블록 안에서만 이루어져 **원래도 서버에서만 호출되던 자리**다. 목적은 **프로젝트 전체에서 가드 모양을 한 가지로 통일**하는 것이다. **[⭐ 8번째를 포함한 근거]** `OnProductionQueueChanged` 는 **사전 조사 목록에 없었다.** Plan 작성 중 `SubscribeToProductionEvents` 를 읽다가 이것이 `OnProductionStarted`·`OnUnitProduced` 와 **같은 블록에서 나란히 구독되는 세 번째 형제**임을 확인했다. **셋 중 둘만 막으면 같은 파일 안에서 가드 모양이 갈린다.** **[위험 구간은 「정지 화면」이 아니다]** `GameEndUI.cs` **343행 `Time.timeScale = 1f`** 가 **354행 `BackToLobby`** 보다 **먼저** 실행된다. 승리 팝업 동안은 `timeScale=0` 이지만 **[로비로] 를 누르는 순간 정상 속도로 되돌린 뒤 Shutdown** 하므로, 그 사이 **생산 틱 · 수입 틱 · 타일 점령 틱이 정상 속도로 돈다.** **[🔴 수치 정정 — 27ms 가 아니라 6~41ms]** 이 위험 구간은 기존 문서·코드 주석이 적어 온 **27ms 단일값이 아니다.** `_Logs/_editor/2026-08-19/RuntimeLog.txt` **255·692·874·1398행 부근**의 **4회 실측**은 **25 / 27 / 41 / 6 ms** 다. **최댓값 41ms 는 60fps 기준 2~3 프레임**이며, **`27ms` 는 그중 한 표본**일 뿐 대표값도 최댓값도 아니다. 코드 주석의 `27ms` 표기(`NetworkCombatController.cs` 880·920·1001·1078행)는 **이번 범위 밖이라 고치지 않았다**(규칙 6). **[건드리지 않은 것 — 부호가 반대인 가드 10곳]** `if (IsServer) return;` 은 **`ClientRpc` 수신부에서 서버의 중복 처리를 막는 정반대 목적**이다. Resource 2 · Tile 1 · Health 3 · Production 4 로 **총 10곳 전부 무변경**임을 확인했다. **특히 `NetworkTileSync.BroadcastTileChangeClientRpc` 의 것을 잘못 고치면 클라이언트 타일 색이 통째로 죽는다.** **[로그 0건]** 가드에 걸리는 것은 **정상 종료 흐름**이고 상태 **전이** 지점이 아니라 `LogRules` **1.14 금지 8**(매 틱 로깅 금지)에 걸린다. **`LogEvent` 37개 무변경 · 신설 0건.** **[검증 — 정적 확인만]** 8곳 적용 확인 · **부호 반대 가드 10곳 무변경 확인**(변경분의 삭제 6줄이 전부 `!IsServer` 형태) · 중괄호 균형 5파일 전후 동일(20·23·47·52·137) · 대체 3파일의 `return` 문 수 전후 동일 · `LogEvent` 37개 무변경. **⚠️ `NetworkTileSync` 의 `return` 문이 2→4 로 세어지는 것은 오탐이다** — 신설 가드 1줄과 **새 주석이 혼동 방지용으로 인용한 `` `if (IsServer) return;` `` 문자열 1줄**(162행)이 함께 잡힌 것이다. **[⚠️ 완료 옆에 붙는 단서 — 과대 표기 금지 (규칙 10)]** ① **실기 재현 0회 · 실기 검증도 아직 안 했다.** 이 8곳은 **이벤트 발행 시에만** 실행되어 매 프레임 도는 `NetworkCombatController.Update` 와 위험도가 다르다. `NetworkCombatController` 는 실기에서 **2회 재현**됐지만 **이번 8곳은 0회**다. **그럼에도 고친 이유는 「터진 적 없으니 놔둔다」 가 `NetworkCombatController` 를 그 상태로 방치했던 판단과 같기 때문이고, 그것은 실제로 터졌다.** ~~**현재 표기는 「코드 적용 완료 · 실기 미검증」 이며 「실기 PASS」 가 아니다**~~ **[🔴 2026-08-24: 「멀티 실기 3경기 회귀 없음」으로 갱신. 다만 「실기 PASS」 라고는 여전히 적지 않는다 — 8곳 중 6곳은 발화 횟수 확인 불가]** ② **고쳐졌음을 적극적으로 보일 수 없다** — 오류가 0회라 *"오류가 사라졌다"* 는 확인이 성립하지 않는다. 실기에서 볼 것은 **「멀쩡하던 것이 망가지지 않았는가」**(골드·타일 색·체력·생산·큐·승패가 양쪽 화면에서 정상인지)다 ③ **범위 밖 5건은 전부 무변경** — `NetworkUnit.SetAnimState`(`NetworkUnit.cs` 173행, 여전히 `IsServer` 만. 유일한 호출부가 이미 막혀 **중복**) · `NetworkGameEndController` 의 `_localRematch*` 3종(`ServerRpc` · `IsServer` 블록 **밖** 구독이라 `!IsSpawned` **만** 필요 — 형태가 달라 별건) · `ServerRpc` 계열 전반(호출 주체가 **UI 입력**이라 성격이 다름) · **`ProductionTicker.Update`**(`Presentation/Production/` 238행, **종료 가드 없음.** 길목으로는 더 근본적이나 틱 전체를 멈추는 **동작 변경**이라 별도 설계 판단 필요 — **제안만**) · `NetworkBuildingController`·`NetworkUpgradeController`(`.Subscribe(` **0건**이라 이번 전수 대상에서 제외. **다른 형태의 구멍 유무는 확인하지 않았다** — 있다/없다를 말할 수 없다). **코드 변경은 5개 파일**이고 프리팹·씬·`.asset` 변경은 없다. 상세: task `_Tasks/2026-08-20/12_24_network-guard-sweep/Plan.md` **§13**, `LogRules.md` **1.13**「2026-08-20」. **[🔴 2026-08-24 실기 검증 결과 — 원문은 그대로 두고 덧붙인다]** **멀티 실기 연속 3경기로 검증했고 회귀는 없다.** 근거 `_Logs/_editor/2026-08-24/RuntimeLog.txt`(13,003행) — **`[ERROR]` 0건 · 3경기 전부 정상 종료(승자 발표 3회: 447·5536·12993행) · 전투 틱 정지 1경기에 정확히 1회(446행) · 스폰 레이스(`NetworkControllerSpawnedWithoutGameServices`) 0건**, 사용자 육안 확인 *"특별히 문제없어보였어"*. **[⭐ 이 세션이 특별한 이유 — 클라이언트 쪽을 처음 기록했다]** 무작위 매칭이라 **역할이 도중에 바뀌었다**: **1경기 호스트**(`IsServer=True, IsHost=True`, 33행) → **2·3경기 클라이언트**(481·5541행). 과거 실기 로그는 **전부 호스트 쪽**이었다(2026-08-19 로그 1,408행을 재실측하면 `네트워크 스폰 | IsServer=False` **0건** · `IsServer=True` **30건**). 결과적으로 **호스트 1경기 + 클라이언트 2경기**를 한 세션에서 검증했다. **[⚠️ 과대 표기 금지 — 해소된 것과 안 된 것을 가른다]** ① **8곳 중 서버 측 발화가 로그로 확인된 것은 2곳뿐**이다 — ③`NetworkGameEndController.OnGameEndServer`(447행 1회) · ⑧`NetworkProductionController.OnUnitProduced`(1경기 189회). **나머지 6곳(①②④⑤⑥⑦)은 가드 아래 본문에 호출당 로그가 없어 발화 횟수를 셀 수 없다** — 이는 *"가드에 로그를 0건 추가한다"*(Plan §6-4)는 결정의 대가이지 결함이 아니다. ② **가장 위험했던 ①`NetworkResourceSync`·②`NetworkTileSync`(`IsServer` 조건이 새로 붙은 두 곳)의 근거를 섞지 말 것** — 1경기(에디터=서버)에 남은 것은 `서버 모드로 골드 동기화 시작`(37행)·`서버 모드로 타일 소유권 동기화 시작 | IsServer=True`(43행) 즉 **서버 분기 진입·구독 성립까지**이고, **골드가 실제로 흘렀다는 근거는 `서버 유닛 생산 완료` 189회 + 2분 43초 정상 종료**(생산은 골드를 소모하므로 가드가 서버 트래픽을 막았다면 성립 불가)다. 반면 2·3경기의 `클라이언트 골드를 서버 값으로 보정` **6,168건**(516행~)·`타일 동기화 수신` **733건**(492행~)은 **상대 호스트가 보낸 것을 받은 기록**이라 **에디터의 서버 측 가드를 통과한 트래픽이 아니다**(상대 빌드의 커밋을 이 로그로 확인할 수 없다 → 가드의 근거로 쓰지 않는다, 규칙 10). ③ 따라서 **「고쳐졌음을 적극적으로 보일 수 없다」는 여전히 해소되지 않았고**, 이번에 확인된 것은 **「멀쩡하던 것이 망가지지 않았는가」** 쪽이다. ④ **`_combatStopped` 재경기 리셋은 검증되지 않았다** — 아래 별도 항목 참조. 상세: Plan **§14**, `LogRules.md` **1.13**「2026-08-24」.

**문서/설정 정비 (2026-08-20) — 에이전트 메모리 손실 사고와 그 원인 제거:** **[증상]** `game-programmer` 에이전트가 메모리를 갱신하면서 `.claude/agent-memory/game-programmer/MEMORY.md` 를 **`Write` 로 통째로 재작성**해 **46행 → 28행**으로 줄었고, 아래가 **소실**됐다 — `logging.md` 토픽 파일 참조 링크 · `LogLevel` 이 `Hexiege.Application`·`Hexiege.Infrastructure` **양쪽에 있다는 함정** · `LogSessionOwner.cs` 는 **의도적으로 `using` 이 없다**는 관례 · **host 는 서버이자 클라이언트라 `ClientRpc` 안의 로그는 `if (IsServer) return;` 뒤에** 둔다(`LogRules` 1.14 금지 9) · 선례 파일:행 목록(`NetworkUnit:291`·`NetworkCombatController:310`·`NetworkGameEndController:457`·`UnitFactory:533`) · **「알려진 잔존 구멍」 섹션 전체** — 특히 **아직 미해결인 `GameBootstrapper.Update`(530~590행)의 싱글플레이 틱 낭비**. **[⭐ 원인 — 에이전트 정의 파일에 하드코딩된 두 줄]** ① **거짓 문장** `Your MEMORY.md is currently empty.` — 파일이 실제로 비어 있던 시점에 자동 생성돼 **그대로 굳었다.** 에이전트는 매 호출마다 *"내 MEMORY.md 는 비어 있다"* 고 **믿은 채 시작**하므로, 갱신 지시를 받으면 `Edit`(덧붙이기)이 아니라 **`Write`(전체 덮어쓰기)** 를 쓴다 ② **존재하지 않는 윈도우 절대경로** `D:\Dmain\...\.claude\agent-memory\<agent>\` — 원격 리눅스 세션에 그런 경로가 없어 **안내받은 경로로는 기존 파일을 찾아 ①의 오해를 확인할 방법조차 없었다.** 두 줄은 **6개 에이전트 정의 전부**에 있었다(`asset-prompt-crafter:112/121` · `document-manager:210` · `game-design-lead:104/134` · `game-programmer:126/156` · `project-orchestrator:89/119` · `qa-tester:134/164`). **[조치 — 완료]** 6개 정의 전부에서 **윈도우 절대경로 → `.claude/agent-memory/<agent>/` 상대경로**로 교체(`.claude/agents/` 하위 `Dmain` 잔존 **0건** 확인), **거짓 문장 → *"MEMORY.md 는 이미 내용이 있을 수 있다. `Read` 로 먼저 확인하고 `Edit` 로 해당 부분만 고쳐라. `Write` 로 전체를 다시 쓰지 마라"* 경고로 교체**(6개 파일 전부 반영 확인). `game-programmer/MEMORY.md` **복원**(구버전 전 항목 + 이번 배치 학습 병합, **92행**), `game-programmer/network-infra.md` 는 **손실 없이 추가만** 됨(27ms → 6~41ms 정정 · 전수 보강 결과 · 반대 부호 가드 주의 · 남은 후보). 원칙은 `AGENTS.md` 「에이전트 메모리」 절에 **문서로 못 박았다.** **⚠️ 남은 것:** `.claude/MEMORY.md`(공용) **33·36·44~49행**에 같은 성질의 **윈도우 절대경로 `d:/Dmain/...` 가 아직 남아 있다** — 이번 조치 범위 밖이라 손대지 않았고, **별도 승인 후 정리 대상**이다. **[✅ 2026-08-20 해소 — 원문은 그대로 두고 덧붙인다: 사용자 승인(*"이제 로컬에서 작업하는 경우가 없으니 필요 없을 것 같다"*)을 받아 정리했다. ① `.claude/MEMORY.md` 8행 교체(33 → `CLAUDE.md`, 36 → `Assets/_Project/Docs/WORKFLOW.md`, 44~49 에이전트 메모리 경로 표 → `.claude/agent-memory/<에이전트명>/MEMORY.md`) — **`Read`→`Edit` 만 사용해 파일 108행 그대로 유지**(이 사고의 재발 방지 원칙을 이번 작업 자신에게도 적용). ② **🔴 이번에 새로 드러난 사실 — `.claude/settings.json` 8행 `SessionStart` 훅이 `cat "d:/Dmain/dev/Portfolio/Hexiege/Hexiege/CLAUDE.md"` 로 존재하지 않는 윈도우 경로를 읽고 있었다.** 원격 리눅스 세션에 그 경로가 없으므로 **이 훅은 매 세션 시작마다 계속 실패해 왔고, 절대 규칙 파일을 자동으로 읽어 들이는 장치가 사실상 죽어 있었다.** 사고 원인 ②(죽은 윈도우 경로)와 **같은 뿌리**이며, **위 조치에서 `.claude/agents/*.md` 6개만 고쳤을 때 설정 파일의 이 한 줄이 그대로 남아 있었다는 점이 요점**이다. `cat CLAUDE.md` 로 교체했고, 같은 파일의 윈도우 전용 죽은 허용 항목 4개(`Bash("D:/dev/Node/npm.cmd" ...)` 3개 · `Read(//c/Users/rmsep/**)`)와 `additionalDirectories` 의 윈도우 경로 2개도 정리했다(`PreToolUse` 훅·나머지 permissions 무변경, JSON 파싱 성공 확인). ③ `.claude/agent-memory/project-orchestrator/MEMORY.md` **239행**은 *"위임 시 파일 경로를 절대 경로(`d:/Dmain/...`)로 전달하라"* 며 **서브에이전트에게 죽은 경로를 넘기라고 지시하고 있어** 상대경로 + 경고로 교체(나머지 324행 무변경). **⚠️ 과대 표기 금지 — 정적 확인(파일 내용 · JSON 파싱)까지이며, `SessionStart` 훅이 실제로 성공하는지는 다음 세션 시작 때 확인해야 한다.**]** **[🔴 2026-08-21 추가 조사 — 원문은 그대로 두고 덧붙인다.** 위 항목이 *"46행 → 28행"* 한 건으로 적은 사고는 **더 크고 더 이른 사고의 재발이었다.** ① **2026-08-17 커밋 `675203ae` 에서 `game-programmer/MEMORY.md` 가 -450행 / +72행 = 실질 `-378행` 으로 덮어써졌다.** 08-20에 발견한 **-18행보다 21배 크고 3일 앞선 사고**이며, **아직 복구되지 않았다.** 같은 커밋에서 `agent-memory/game-programmer/` 폴더의 **파일 수는 19 → 19 로 그대로**, 폴더 총 행수는 **2,503 → 2,125 로 정확히 -378** 이었다 — **토픽 파일로 옮긴 것이 아니라 소실**이라는 뜻이다. 그때 지워진 것: `## CRITICAL — GIT 명령 절대 금지` 절 · 레이어 제약 · NGO API 제약 · DontDestroyOnLoad 관례 · **런타임 로그는 GameLog 경유** · 「최근 작업」 약 20건. 커밋 제목은 `feat(log): 감사 누락 PlayerId 마스킹…` 으로 **메모리 정리와 무관한 작업이었다**(= 다른 작업 도중 곁다리로 덮어써졌다). ② ~~**200행 절삭 — 별개의 손실 경로.** 에이전트 정의 파일이 *"lines after 200 will be truncated"* 라고 명시하는데, 2026-08-21 실측 초과가 **3개** 다 — **qa-tester 502행 · project-orchestrator 325행 · game-design-lead 254행.** 초과분은 파일에 남아 있어도 **에이전트에게 보이지 않으며, 커밋 이력에 안 남아 사고가 났다는 것조차 모른다**(①보다 나쁜 점).~~ **[✅ 2026-08-21 정정 — 원문은 그대로 두고 덧붙인다: ②는 **손실 경로가 아니다. 전제 자체가 거짓이었다.** 아래 「전제 정정」 항목 참조. 세 파일의 행수(502 · 325 · 254)는 사실이지만 그 의미가 **손실 → 세분화 권장(품질)** 으로 바뀐다]** ③ **고아 토픽 16개(1,839행).** `game-programmer` 토픽 **18개 중 16개**가 `MEMORY.md` 인덱스에서 링크가 끊겨 있다(살아 있는 링크는 `logging.md`·`network-infra.md` 둘뿐) — `work-history.md` 819 · `unit-building.md` 166 · `ui-system.md` 137 · `3d-transition.md` 104 · `network.md` 94 · `architecture.md` 91 · `hex-grid.md` 88 · `skill-aim-coordinate.md` 60 · `rendering-and-animation.md` 50 · `gameplay-systems.md` 47 · `unit-stats-and-combat.md` 46 · `camera-and-view.md` 45 · `attack-direction-refactor.md` 35 · `random-matching-bugfix.md` 34 · `combat-fixes.md` 17 · `network-todo.md` 6. **원인은 ①이다** — 링크를 담고 있던 인덱스가 덮어써지면 토픽 파일이 통째로 미아가 된다. ④ **✅ 대조 결과 프로젝트 문서(`Assets/_Project/Docs/`)는 정상이다.** 소실된 것은 **에이전트 캐시**이지 프로젝트 지식이 아니다 — 커밋 수 기준 `ROADMAP.md` 35 · `PROJECT_STATUS.md` 20 · `WORK_HISTORY.md` 19 · `LogRules.md` 15(6월 이후)로 꾸준히 갱신돼 왔고, ①에서 지워진 내용의 **원본 대부분은 `CLAUDE.md` · `.claude/MEMORY.md` · `GameDesignDocument.md` 에 살아 있다.** ⑤ **조치 — 규칙 신설.** 손실 경로가 ①②③으로 **서로 달라 `Write` 금지만으로는 ②③이 막히지 않으므로**, `.claude/MEMORY.md`(모든 에이전트가 작업 전 읽는 공용 파일) 상단에 **「🔴 에이전트 메모리 갱신 규칙」 6개 항목**을 신설했다(Read→Edit / 비어 있다고 가정 금지 / 삭제는 틀렸을 때만+이유 명기 / 200행 이내 인덱스+링크 필수 / 링크 없는 토픽은 없는 것 / **폴더 총 행수 합이 줄지 않을 것** = 이동과 삭제를 구분하는 유일한 검증법). 이 신설 작업 자체도 **`Read`→`Edit` 순수 삽입**으로 수행해 **기존 108행을 한 줄도 건드리지 않았다**(108 → 121행, 삽입 13행 제외 시 원문 108행 완전 일치 확인). `CLAUDE.md` 는 **사용자와 메인 세션의 소통 전용 문서로 확정**되어 손대지 않았다. **⚠️ 미복구 · 후속 4건은 `ROADMAP.md` 우선순위 표 참조.** **[✅ 2026-08-21 해소(부분) — 원문은 그대로 두고 덧붙인다: 후속 4건 중 **2건 완료**. ① **고아 토픽 16개 재링크 완료** — `game-programmer/MEMORY.md` 의 토픽 인덱스를 복원해 **18/18 전부 링크**(누락 0건 실측), 92 → **132행**. ② **2026-08-17 `-378행` 소실분 대조 복구 완료** — 전량 되돌리기가 아니라 **항목별 전수 grep 판정**으로 **유일본 7건만** 골라 성격에 맞는 토픽 파일 4개(`ui-system.md`·`architecture.md`·`rendering-and-animation.md`·`gameplay-systems.md`)에 복구했다. 나머지는 원본이 `CLAUDE.md`·`.claude/MEMORY.md`·`WORK_HISTORY.md` 등에 살아 있어 **의도적으로 복구하지 않았다**(중복 분산이 다음 정리 때 또 지워지는 원인이기 때문). **폴더 전체 행수 2,411 → 2,517행(+106)** 으로 늘어 이동/삭제가 아님을 확인했다. **남은 2건은 미착수 그대로** — `Tools/check_docs.py` 메모리 검사 3종 추가 · 200행 초과 3개(qa-tester 502 · project-orchestrator 325 · game-design-lead 254) 분산. 항목별 판정 근거는 `ROADMAP.md` 해당 행 참조]** **[✅ 2026-08-21 정정 — 원문은 그대로 두고 덧붙인다: 남은 2건의 **내용이 바뀐다.** 메모리 검사는 **3종 → 2종**(고아 토픽 검출 · 행수 급감 감지. 「200행 초과 경고」는 폐기), 「200행 초과 3개 분산」은 **「세분화 권장 3개 분산」** 으로 성격이 바뀌어 **🟡 중간 → 🟢 낮음**으로 내려간다. 근거는 아래 「전제 정정」 항목]** **※ 근거 구분(규칙 10):** ②③④의 행수·파일수·링크 상태는 **이 세션에서 직접 실측**했고, ①의 커밋 해시·증감 행수와 ④의 커밋 수는 **호출 세션이 git 이력을 대조해 전달한 값**이다(document-manager 는 규칙 5에 따라 git 명령을 실행하지 않는다).**]**

**구현 완료 (2026-08-19) — ✅ 실기 검증 통과 (재경기 2회 포함):** **게임 종료 · 서버 종료 시점의 전투 뒷정리** (커밋 `55d24d83`). **[①  가드 구멍 6곳 — `OnUnitDied` 하나가 아니었다]** 종료 시점에 네트워크가 이미 꺼진 뒤 호출될 수 있는 서버 핸들러 전수를 확인해 **6곳**을 막았다 — `OnUnitDied`·`OnBuildingDied` 는 `!IsServer` 만 있던 가드를 **`if (!IsSpawned || !IsServer) return;`** 로, `OnUnitEnteredCombatHandler` 는 **가드가 아예 없던 자리**에 같은 형태로, Walk·HealCast·FreezeChanged 3건은 **공통 길목인 `SetUnitAnimState` 한 곳**에 `if (!IsSpawned) return;` 을 두어 **호출 지점 5곳을 한 번에 덮었다**(호출부마다 붙이는 것보다 **새 호출부가 생겨도 자동으로 덮인다**). **⭐ `OnUnitDied` 만 고쳤으면 성 파괴 경로(`OnBuildingDied`)로 그대로 재발했을 것이다 — 게임을 끝내는 바로 그 경로다.** `OnUnitEnteredCombatHandler` 에 새로 붙인 `IsServer` 는 **동작 변화가 없다**(그 구독이 `OnNetworkSpawn` 의 `if (IsServer)` 블록 안에서만 이루어져 원래도 서버 전용). **[② 게임 종료 시 전투 틱 정지]** `GameEvents.OnGameEnd` 를 **서버 전용**으로 구독해 `_combatStopped` 를 세우고 `Update` 진입부에서 검사한다(`if (!IsSpawned || !IsServer || _combatStopped) return;`). 진행 중이던 `DelayedAttackDamage` 는 `StopAllCoroutines()` 로 정리. **구독 해제 방식은 기각했다** — `GameEndUseCase` 가 `OnBuildingDied` **디스패치 도중 동기적으로** `OnGameEnd` 를 발행하므로, 그 안에서 `Dispose()` 하면 디스패치 중 구독자 목록을 바꾸게 되고 구독 순서에 따라 **게임을 끝낸 성의 `EntityDiedClientRpc` 가 영영 안 나갈 수 있다.** **[🔴 최우선 위험이었던 것 — 플래그 리셋]** `TickCombat` 에는 전투 외에 **타워·파도·HoT·자연회복·연구·스킬 쿨다운·물안개·상태효과**가 함께 들어 있다. 재경기에서 `_combatStopped` 가 `true` 로 남으면 그 전부가 멈춰 **성이 파괴될 수 없고 게임이 영원히 끝나지 않는다** — 고치려던 것보다 훨씬 나쁜 회귀다. 그래서 `OnNetworkSpawn`(IsServer 블록)과 `OnNetworkDespawn` **양쪽**에서 `false` 로 되돌리며, 같은 파일의 `_attackTimer`·`_lastCarry` 가 정확히 그 두 자리에서 리셋되는 **선례 옆에** 붙였다. **[✅ 실기 — `_Logs/_editor/2026-08-19/RuntimeLog.txt` 1408줄, 세 번째 세션 23:26:46~]** 사용자가 **3경기를 연속 진행 = 재경기 2회**. **① ✅ 해결** — 3경기 내내 `[ERROR]` **0건** · `"Rpc methods can only be invoked after starting the NetworkManager!"` **0건**, 게임 종료 → 로비 → 재경기를 두 번 반복. **② ✅ 동작** — `게임 종료 — 전투 틱 정지` 가 **872·1090·1394행**에 **경기당 정확히 1회**. **재경기 ✅ 2회 연속 통과** — 1경기 정지 후 2경기 구간에 `NetworkCombatController` 로그 **40건**(`서버: 유닛 사망` 34), 2경기 정지 후 3경기 구간에 **68건**(사망 63)으로 **전투가 정상 재개됐다.** **⭐ 재경기 검증이 이 작업의 핵심이다** — 플래그 리셋이 잘못됐다면 2경기에서 유닛이 아예 싸우지 못했을 것이고, 그 시나리오가 **두 번 연속 통과**했다. **[⚠️ 완료 옆에 붙는 단서 — 과대 표기 금지]** ① **B(전역 훅의 엔진 오류 수집, 커밋 `cc864054`)는 여전히 미검증** — 3경기를 더 돌렸으나 엔진 오류가 나지 않았다. **역설이지만 ①·② 수정이 잘 돼서 B 를 검증할 기회가 사라졌다**(B 로 잡으려던 대상이 바로 그 RPC 에러였다) ② **`IsSpawned` 전이 시점 미확정** — `Shutdown()` 직후인지 `OnNetworkDespawn` 과 동시인지는 **NGO 소스를 열 수 없어 확정하지 못했다.** **27ms 창을 완전히 닫는다고 단정하지 않는다** — 실기에서 에러가 나지 않았다는 관측만 있다 **[🔴 2026-08-20 수치 정정 — 원문은 그대로 두고 덧붙인다: 이 구간은 `27ms` 단일값이 아니다. 같은 로그 파일 4회 실측이 25 / 27 / 41 / 6 ms 이므로 정확한 표기는 `6~41ms` 이고, `27ms` 는 그중 한 표본일 뿐 대표값도 최댓값도 아니다. 최댓값 41ms 는 60fps 기준 2~3 프레임이다. 근거: `_Logs/_editor/2026-08-19/RuntimeLog.txt` 255·692·874·1398행 부근]** ③ **Walk·HealCast·FreezeChanged 3건은 「예방」** — 디스폰 후 NetworkVariable 쓰기가 실제로 오류를 내는지 미확정 ④ **`NetworkUnit.SetAnimState` 는 손대지 않았다** — 여전히 `IsServer` 만 보고 `IsSpawned` 를 안 본다. 더 근본적인 자리지만 다른 파일이라 `NetworkCombatController.SetUnitAnimState` 쪽에서 막았다 ⑤ ~~**다른 네트워크 컨트롤러의 동일 구멍은 점검하지 않았다** — `NetworkProductionController`·`NetworkBuildingController`·`NetworkUpgradeController` 등을 열지 않았으므로 **있다/없다를 말할 수 없다**(규칙 10).~~ **[2026-08-20 해소 — 커밋 `bcf45ec1`]** 전수 점검 후 **5파일 8곳** 보강(위 2026-08-20 항목). 단 **`NetworkBuildingController`·`NetworkUpgradeController` 는 `.Subscribe(` 0건이라 대상에서 빠진 것**이고 다른 형태의 구멍 유무는 **여전히 미확인**이다. **코드 변경은 `NetworkCombatController.cs` 1개 파일**이고 프리팹·씬 변경은 없다. 상세: task `_Tasks/2026-08-19/10_16_combat-shutdown-cleanup/Plan.md` §10, `LogRules.md` **1.13**「2026-08-19 (2차)」.

**구현 완료 (2026-08-19) — ⚠️ 부분 검증 (A ✅ / C ✅ / B 미검증):** **전역 로그 훅의 `Error` 수집 확대 + 전투 컨트롤러 RPC 가드 + 연구 흐름 로그 보강** (커밋 `cc864054`). **[B]** `LogSessionOwner` 의 전역 훅이 이제 **`Exception` + `Error`** 를 수집한다 — 엔진·NGO 가 내는 `Debug.LogError` 가 로그 파일에 한 줄도 남지 않던 구멍을 메웠다. 방어 1겹을 푸는 변경이라 **되먹임 방어를 3겹 → 4겹**으로 올렸다(`RuntimeLogger.IsEmittingToConsole` 신설 — `GameLog` 를 우회하는 `RuntimeLogger` 직접 호출 경로까지 덮는다. **기존 「직전 동일 줄 무시」 방어는 우리 출력의 밀리초 타임스탬프 때문에 애초에 작동한 적이 없다**). 스팸 대비로 `condition` 단위 **1초 스로틀 + `Suppressed=n`**(딕셔너리 상한 32). `LogEvent` **36 → 37**(`UnhandledEngineError` 신설 — 엔진 오류를 `UnhandledException` 에 섞으면 **크래시 건수 지표가 크래시 아닌 사건으로 부풀고 조치도 다르다**). **[A]** `NetworkCombatController.Update` 가드를 `if (!IsSpawned || !IsServer) return;` 로 확장 — `IsServer` 는 *"내가 서버 역할인가"* 이지 *"이 오브젝트가 아직 살아 있는가"* 가 아니다. **[C]** `NetworkUpgradeController` 의 성공 경로가 **무로그**였다 → 착수·완료 브로드캐스트·클라 반영을 `개발`, 클라 측 서비스 부재 분기만 `운영`(기존 키 재사용)으로 4줄 추가. **[+]** `NetworkProductionController` 의 `Pos={unit.Position}` 이 `Pos=(4, 15)` 를 만들어 **값 안에 구분자가 들어가 있던 `key=value` 위반 1건**을 `Q=`/`R=` 로 수정 — **2026-08-18 에 `LogRules.md` 1.4 로 명문화한 규정을 우리 코드가 어기고 있었다.** **[실기 — `_Logs/_editor/2026-08-19/RuntimeLog.txt` 704줄]** **A ✅ 해결**(Shutdown 13:34:01.467 이후 `StopCombatClientRpc` 0건. 에디터가 host 인 회차라 조건 성립) · **C ✅ 정상**(착수 6건 ↔ 완료 브로드캐스트 6건 1:1, `Level=1` → `Level=2`) · **되먹임·폭주 ✅ 없음**. **⚠️ B 는 미검증** — `[ERROR]` 0건 · `UnhandledEngineError` 0건 · `Suppressed=` 0건으로 **이번 세션에 엔진 오류가 나지 않아 잡을 것이 없었다.** 「동작 확인」이 아니라 **「확인할 기회가 없었음」**이다(규칙 10). **⚠️ 그 밖의 단서:** **A 의 원인인 스폰 레이스는 이번에도 미발생**(`NetworkControllerSpawnedWithoutGameServices` 0건 — 정상 경로만 확인됨) · **`SetCameraStartPositionForTeam` 최종 삭제 여전히 잔여**  · ~~**범위 밖 2건**(`OnUnitDied` → `EntityDiedClientRpc` 의 같은 성질 가드 구멍 / `NetworkCombatController` 에 게임 종료 구독 부재 — 로그상 종료 감지 후 **약 3초간 전투 루프 지속**)~~ → **둘 다 해소됨**(커밋 `55d24d83` — 위 항목). 상세: `LogRules.md` **1.5 / 1.9 / 1.11 / 1.13**, task `_Tasks/2026-08-18/20_07_log-coverage-and-rpc-guard/Plan.md` §11.

**구현 완료 (2026-08-18) — 컴파일 통과 · ⚠️ 실기 미검증:** **로그 체계(`GameLog`) 전환 — 나머지 계층 이관 완료.** `Assets/_Project/Scripts/` 의 게임 런타임 코드에서 raw `Debug.Log` 이관이 끝나, **누적 386건**(선행 205 + 이번 **181**)이 `GameLog` 를 경유한다. 배치별: 1-A **65**(`3151d0df`) · 1-B **23**(`793e584f`) · 2 **50**(신규 로그 3건 포함, `3141bc61`) · 3·4 **43**(`5a554bc7`) · `system` 문자열 교정 **16건**(`589c2fcf`). `LogEvent` enum 은 **36개**(배치 2 에서 4개 신설), **`LogEvent.Unknown` 사용처 0건**. **[최종 잔존 46건 — 전부 남을 이유가 있다]** `GameLog.cs` **9**(sink 0개일 때의 콘솔 폴백 — `LogRules.md` **1.8** 이 요구하는 동작) · `ILogSink.cs` **2**(주석) · `Infrastructure/Debug/` **19**(로그 시스템 본체) · `Assets/Editor/` **16**(**미적용** — 에디터 도구는 플레이 모드 밖이라 sink 가 없어 이관 효과가 원리적으로 0. **규칙 예외가 아니라 현행 구현의 한계**로 기재). `grep` 총계 54 = 실제 호출 46 + **주석 8**. **[⭐ 확정된 해석 — `system` 은 폴더가 아니라 기능 기준]** 배치 3 이 원본 `[Network]` 태그의 UI 4파일(`LobbyUI` 10 · `NetworkStatusUI` 4 · `GameEndUI` 1 · `LobbyRootView` 1)을 폴더 기준 `"UI"` 로 바꿨다가 되돌렸다(`589c2fcf`) — 폴더로 통일하면 **`System=Network` 필터에 `LobbyUI` 의 방 참가 실패가 안 잡힌다.** 이 해석을 `LogRules.md` **1.4 본문에 한 문장으로 명문화**했다(**규칙 번호는 신설하지 않음** — 코드 주석의 기존 참조가 끊긴다). **[⭐ 추정이 두 번 연속 빗나갔다]** *"같은 계층을 호출하니 중복일 것"* 이라는 판정이 두 번 틀렸다 — `RelaySetupFailed` 는 **3곳 중 2곳만**(`Stage=CodeMissing` 은 하위를 호출하지 않고 `return`), `PlayerProfileService` 호출 View 는 **4곳 중 3곳만**(`ProfileView` 로그아웃은 `SignOutAsync` 에 `catch` 가 없다) 중복이었다. **호출 경로를 끝까지 따라가야 한다** — `LogRules.md` **1.14 금지 9** 에 보강. **[함께 수정한 버그 3건 (커밋 `da5eeaab`)]** ① **`NetworkUpgradeController` 완료 훅 구독 누락** — `OnNetworkSpawn` 이 조기 `return` 하면 `OnResearchCompleted` 구독이 세션 내내 복구되지 않던 문제. `NetworkSkillController` 의 검증된 `EnsureStatusSubscription` 패턴을 이식(`EnsureUpgradeSubscription`, 호출 2곳). **증상은 클라이언트 한정** — 서버 레벨은 정상 상승하고 전투 판정도 서버 권위라 정상이며, 죽는 것은 **양 클라 브로드캐스트뿐**이다. ② **`SetCameraStartPositionForTeam` 비활성화**(호출부 0곳, `ViewConverter.Setup` 의 뷰 반전으로 대체됨) ③ **유닛 스폰 실패 운영 로그에 진단 필드 추가**(`UnitType`·`Team`·`Q`·`R`). **[⚠️ 과대 표기 금지 — 완료 옆에 붙는 단서]** **컴파일은 통과했으나 실기 동작 테스트를 하지 않았다** — 특히 **①은 회선 상태에 좌우되는 간헐 버그**라 실기로 확인되지 않았고, **"안 났으니 고쳐졌다"고 결론 내리지 않는다** · **`SetCameraStartPositionForTeam` 주석 처리분의 최종 삭제가 남아 있다**(WORKFLOW [6] 통과 후 · [7] 전) · **`Assets/Editor/` 16건 미적용**(위) · **`_services` 가 이전 경기 인스턴스를 가리킬 위험**은 `NetworkSkillController` 포함 **구조 공통 사항**이며 이번 변경이 만든 위험이 아니다(재경기 후 강화가 이상하면 여기를 먼저 의심) · **`LobbyUI` 가 현재 쓰이지 않을 가능성** — 로비가 `LobbyRootView` 계열로 재구성됐는데 `LobbyUI` 는 옛 구조 그대로다. **확인 없이 판단하지 않고 10건 전부 이관했다.** 상세: `LogRules.md` **1.4**·**1.7**·**1.13**·**1.14**, task `_Tasks/2026-08-17/17_19_remaining-layers-log-migration/Plan.md` §11 · `_Tasks/2026-08-18/04_02_upgrade-subscription-fix/Plan.md` §11.

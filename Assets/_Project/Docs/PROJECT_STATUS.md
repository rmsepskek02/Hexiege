...
**⚠️ 진행 중 (2026-09-08) — 검증 상태: 싱글플레이 에디터 실기 검증 완료 / 멀티 미검증 / 🔴 2단계는 아직 끝나지 않았다(K 단계 남음):** **무작위 맵 2단계 — A~J 단계 구현 완료.**

쉽게 말하면, **이제 싱글 경기에 들어가면 매 판 다른 전장이 나온다.** 성과 광산 자리가 코드에 박혀 있던 것을 걷어내고, 맵을 만들어 내는 기계 · 만든 맵을 검사하는 기계 · 검사에 계속 실패할 때 쓸 예비 맵 · 그 맵을 실제 전장으로 옮기는 일 · 막힌 곳을 빈 공간으로 그리고 지을 수 없는 곳에 빗금을 얹는 일 · 그 빗금 규칙을 사람과 AI 양쪽에 똑같이 적용하는 일까지가 들어갔다. 사용자 실기 테스트로 **매 판 광산 위치·시작 광산 방향·초기 골드가 바뀌는 것**과 **기존 기능이 그대로 도는 것**을 확인했다.

단계별 결과(커밋): **A~E** 결정적 난수·대칭 builder·초기 상태 평가·생성기 5종·검증기 / **F** 폴백 템플릿 5개 + 제작 도구 `996cc0a` / **G** 맵 준비 조정자 `7eea8f0` / **H** `GameConfig` 11×21 + 투영 `f432fb5`(**여기서 처음 화면이 바뀐다**) / **J** 렌더러 + 클릭 판정 `a2a8c60`·후속 `a19fe07` / **I** 판정 조건 전환 `c47a067`. 🔴 **I 와 J 는 계획의 I → J 순서를 J → I 로 바꿔 실행했다**(사용자 승인) — 렌더링이 먼저 서야 「빗금 친 곳에 건설이 막히는가」를 눈으로 확인할 수 있기 때문이다.

⚠️ **과대 표기 금지 — 아직 완료가 아닌 것:** ① 🔴 **2단계는 미완이다. K 단계(맵 준비 로그 키 신설)가 남아 있다** — 규칙 12 가 요구하는 로그 항목이 아직 로그로 나가지 않는다(현재는 결과 객체에 값으로만 담겨 있다). ② 🔴 **멀티플레이 seed 가 임시 고정값(1)이다.** `LoadMap` 이 싱글·멀티 공용이라 그대로 두면 Host 와 Client 가 각자 seed 를 뽑아 서로 다른 맵을 보게 되므로 임시로 고정했다. **3단계에서 Host 가 seed 를 전송하면 사라지며, 그때까지 멀티는 매 판 같은 맵이 나온다.** ③ 🔴 **「AI 가 빗금 타일에 건물을 짓지 않는가」는 현재 실기로 확인할 수 없다 — 검증 누락이 아니라 도달 불가다.** 각 유형 200시드(외곽형만 100판) **900판 전부**에서 경기 시작 시점 Red 보유 10칸 중 빗금 타일이 **0개**였다(규칙상 양 팀 초기 보호 10칸이 일반 타일로 보장된다). AI 가 빗금 타일을 건설 후보로 만나려면 유닛으로 빗금 구역까지 진출해 점령을 넓혀야 하는데 **AI 시나리오가 이 맵을 전제로 짜인 것이 아니라 그 상황이 만들어지지 않는다.** 맵 전체 빗금 평균은 완전개방형 0.0 · 장애물개방형 0.0 · 협곡형 17.6 · 외곽형 26.5 · **3라인형 75.8**(225 재생 칸 중)이라, **AI 시나리오를 이 맵에 맞추는 순간 이 판정이 곧바로 실전에서 쓰인다. 지금 닿지 않을 뿐 죽은 코드가 아니다.** ④ **멀티플레이 실기 미검증** — 위 검증은 전부 에디터 싱글플레이 세션이다. ⑤ **유스케이스·서비스·네트워크 컨트롤러의 컴파일 통과는 Unity 에서만 확인된다**(`UnityEngine` 참조라 헤드리스 컴파일이 불가하다). ⑥ **보류 항목 8건**(멀티 seed 임시 고정 · `InitialGold` 미사용 · 맵 준비/투영 실패 처리 미명세 · 규칙 13 임계값 3·2 기획 확인 · 생성기 주입 이음매 없음 · `AGENTS.md` 에 `_Reference` 누락 · `PlaceMiningPostDirect` 막힌 타일 가드 없음 · `NoBuildHatch.mat` 공유 머티리얼 오염 가능)은 **미해결 결함이 아니라 사용자가 이번 범위 밖으로 확정한 항목**이며 목록은 `_Tasks/2026-09-03/03_14_random-map-phase2-generator/Plan.md` §13. 근거: 같은 Plan §9·§12.
...
- **상태 (2026-09-03 갱신):** 문서 설계 완료. **1단계(타일 상태 계약 전환)만 구현·에디터 실기 검증 완료** — 상세는 위 「구현 완료 (2026-09-03)」 문단과 「완료된 시스템 > 코어 게임플레이」 표의 **타일 상태 계약** 행. **런타임 생성기·검증기·전송·테스트 모드·건설 불가/차단 지형·경로 완전 차단 대응은 여전히 미구현**이다(2·3단계).
  - **[🔴 2026-09-08 갱신 — 위 문장은 그대로 두고 덧붙인다]** **2단계 A~J 가 구현·싱글 실기 검증까지 끝났다.** 위 문장이 「여전히 미구현」이라고 적은 것 중 **런타임 생성기 5종 · 검증기 · 폴백 5개 · 테스트 모드 설정 필드 · 건설 불가/차단 지형 · 맵 준비 조정자 · `MapDefinition` → `HexGrid` 투영 · 렌더러 · 건설/점령/AI 판정 조건**은 **구현됐다.** **여전히 미구현인 것은 ① 2단계 K(맵 준비 로그 키) ② 맵 전송·해시 대조·재경기(3단계) ③ 경로 완전 차단 대응**이다. 상세는 위 「⚠️ 진행 중 (2026-09-08)」 문단.
  - **이 절이 「확정 설계 — 구현 예정」에 남아 있는 것은 여전히 맞다** — 2단계가 K 를 남겨 미완이고 3단계는 미착수이기 때문이다.

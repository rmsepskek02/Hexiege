...
#### MistShrine 물안개 힐 시스템 (2026-08-10 재설계 기획 확정)

초월(Transcendence) 종족의 **공격하지 않는 힐 건물** MistShrine(`BuildingType.HealShrine` = 6)의 재설계를 확정했다.
**⚠️ 구현·테스트는 전혀 진행되지 않았다 — 문서 확정만 완료된 상태다(과대 표기 금지).**
...
- **신규 필요:** 아군 유닛·건물 수집 헬퍼(기존 원형 반경 헬퍼는 **전부 적 대상 전용**), **건물 회복 경로**(기존 힐 서브시스템은 유닛 전용, `BuildingData`에 회복 메서드 없음), 파괴·철거 시 물안개/자동모드/쿨다운 정리 경로, **물안개 VFX 신규 제작**(현재 MistShrine VFX는 destroy/upgrade뿐)
...
- **문서:** `GameSystemRules/GameSystemRules_Buildings.md`(MistShrine 물안개 힐 시스템 규칙 1~27 — 특수 건물 2종 이상 시 `GameSystemRules_SpecialBuildings.md`로 분리한다는 분기 시점 명시), `GameSystemRules/GameSystemRules_UI.md`(MistShrine 패널 UI 규칙 1~9), GDD §5, TDD, `StatsReference.md`, `GameSystemRules_Upgrade.md`
- **상태:** **기획 확정 / 구현 미착수.** 코드·프리팹·전용 패널·범위 표시 UI·물안개 VFX 전부 없음. task: `_Tasks/2026-08-10/09_34_mistshrine-heal-redesign/`
...
> **⏳ 후속 / 보류 (별도 작업 — 과대 표기 금지):** ① 연구 패널 UI 레이아웃 다듬기(현재 에디터 스크립트 자동생성 골격, 사용자가 Unity에서 직접 다듬을 예정) ② 매트릭스 헤더 아이콘(공/방/속·그룹, 텍스트 라벨 대체 중) ③ AI 시나리오 연구 사용(코드는 있으나 실기 미검증) ④ **MistShrine(HealShrine) 힐 — 2026-08-10 재설계 기획 확정, 구현은 미착수**(아래 "📐 확정 설계 — 구현 예정 > MistShrine 물안개 힐 시스템" 참조. 자연회복과는 별개 효과로 중첩 적용) ⑤ 싱글플레이 자연회복 실기 미검증(코드상 정상 예상). ×10 적용·배선·디버그에 쓰였던 에디터·디버그 스크립트는 config `.asset` 커밋·씬 배선 반영 후 제거됨.

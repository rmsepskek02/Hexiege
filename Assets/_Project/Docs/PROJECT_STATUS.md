...
- B3 v9 코드 게이트: typed status/offending tile/index와 후보 verdict, corridor probe/final-stage 동일 seam, recoverable `WaitingRepath`, actual commit attempt 직전 `planned`/성공 뒤 `committed` 및 END equality, 양수 전이 성공 commit에서만 recoverable 이력 clear, lifecycle retire/reuse를 구현했다. observer `b3-movement-authority-v9`, Runtime/Editor Roslyn PASS, 독립 정적 QA P0~P2 없음. Unity 메뉴 self-validation·Android v9 미실행이므로 **B3 FAIL / OPEN**이다. Android는 recoverable repath 0이 아니라 resolved/nonrepeat/no-cleanup coverage를 요구한다. 규칙 문서는 기존 계약과 일치해 변경하지 않았다.

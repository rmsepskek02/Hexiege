...
#### 연구소 유닛 강화 시스템 + 전투 스탯 ×10 스케일 개편 (2026-07-23 밸런스 최종 확정)

- **강화 스탯**: 공격력 / 방어력(신규 스탯, 전 유닛 기본 0) / 이동속도 3종 + 초월계 전용 자연회복(3~15 HP/s). 종족별 그룹×스탯 트랙(인간 9·정령 9·초월 7).
- **적용 방식 (B)**: 팀별 연구 레벨 배율을 데미지·이동 사용 지점에서 곱함 → 이미 전장에 나온 유닛도 연구 완료 즉시 소급 강화(유닛 재계산·재동기화 불필요).
- **방어력 감쇄**: `데미지 = Max(1, Round(공격력 × (1 − 방어력/(방어력+120))))`, K=120, 방어 0/8/16/24/32/40, floor 1, 하드캡 60~65%. direct·스플래시·DoT 모든 최종 데미지 지점 일괄 적용.
- **전투 스탯 ×10 스케일**: 유닛 HP·공격력, 건물 HP, 타워 HP·공격력, DoT 틱값, 힐량 전부 ×10. HP·공격력 동일 배율로 TTK(처치 타수) 불변 → 상성·매치업 불변, 강화 그리드만 규칙적으로 개선. 사거리·이동속도·쿨다운·생산/연구 시간·모든 골드 비용·채굴 수입·비율은 불변.
- **연구소 운영**: 복수 건설·연구 시간·진행 중 트랙 잠금(중복 차단)·파괴 시 진행 연구 100% 환불·건물 업그레이드 없음(단일 등급 Lv1~5)·상대 비공개·서버 권위. 표준 트랙 1,000골드(80/120/180/260/360)/15~70초, 그룹 배율(초월 동물 ×2.0·자연회복 ×2.5·초월 식물 포함 그 외 ×1.0), 연구소 건설비 200골드.
- **미확정 쟁점 3건 확정**: Tank vs Fox/Rabbit 원샷 관계 유지(HP 미변경) / 방어력 8~40·K=120 / 초월 식물 그룹 비용 ×1.0.
- **문서**: `GameSystemRules/GameSystemRules_Upgrade.md`(구현 계약), `StatsReference.md`(×10 값·업그레이드 표·배너), `GameSystemRules_Units.md` 규칙 44(방어력 감쇄), GDD 연구소 섹션. 밸런스 확정 근거·old/new 대조: `_Tasks/2026-07-22/10_08_unit-upgrade-system/BalanceReview.md`·`Research.md`·`Plan.md`.
- **상태**: 밸런스 최종 확정, 문서 반영 완료. Domain 방어력 필드·그룹 매핑, Application `UnitUpgradeUseCase`·데미지 공식·(B) 배율·자연회복 HoT, Infrastructure 연구 RPC·서버 동기화, Presentation 연구 패널 UI, AI 시나리오 스텝, ×10 스케일 코드 반영은 **미구현(구현 예정)**.

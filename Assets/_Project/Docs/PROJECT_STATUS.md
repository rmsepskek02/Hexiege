...
>
> **⏳ 후속 / 미완 (별도 작업 — 과대 표기 금지):** ① **건물 파괴 시 패널/조준 UI 원복 미구현** — 별도 작업으로 분리(스킬 포함 4개 건물 패널 공통 갭: `BuildingPanelBase`에서 `OnBuildingDied` 구독→Close 방식 제안됨, 미착수). ② **구체 스킬 목록·수치·아이콘(기획)** — 보류(각 슬롯 1~5의 스킬·쿨다운·반경·지속·피해·상태효과는 추후 ScriptableObject로 확정). **현재는 종족별 타입 A/C 플레이스홀더 5슬롯(1 폭탄A/2 빙결/3 공격버프/4 둔화/5 회복, 임시 텍스트 라벨) = 최종 기획·아트 전까지 유지되는 테스트용.** ③ 둔화 전투종료 정렬 Lerp 구간은 캡처값 사용(미세 잔여, 필요 시 후속).
>

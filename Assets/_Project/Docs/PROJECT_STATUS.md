...
- **문서:** `GameSystemRules/GameSystemRules_Buildings.md`(MistShrine 물안개 힐 시스템 규칙 1~27 — 특수 건물 2종 이상 시 `GameSystemRules_SpecialBuildings.md`로 분리한다는 분기 시점 명시), `GameSystemRules/GameSystemRules_UI.md`(MistShrine 패널 UI 규칙 1~15 — 2026-08-10 레이아웃/범위 원 시각 사양/물안개 지속 중 버튼 상태/자동 모드 표시 확정으로 규칙 10~14 신설, 규칙 15는 아이콘 미제작), GDD §5, TDD, `StatsReference.md`, `GameSystemRules_Upgrade.md`

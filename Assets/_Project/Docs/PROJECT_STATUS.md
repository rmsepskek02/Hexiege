...
> **이 문서의 역할: 현재 프로젝트 상태.** 무엇이 되어 있고 무엇이 안 되어 있는지만 담는다.
> 「언제 무엇을 했나」는 [WORK_HISTORY.md](WORK_HISTORY.md), 「앞으로 할 일」은 [ROADMAP.md](ROADMAP.md).

**⚠️ 멀티플레이 실기 검증 (2026-09-09) — 검증 상태: 2단계 결과의 멀티플레이 실기가 처음 이뤄졌다 / 부분 검증:** **무작위 맵 2단계 — 멀티플레이(랜덤 매칭) 실기 2회.**

쉽게 말하면, 지금까지 무작위 맵은 **혼자 하는 경기(에디터 싱글플레이)에서만** 확인됐고 **둘이 붙는 경기에서는 한 번도 확인된 적이 없었다.** 이번에 랜덤 매칭으로 두 차례 붙어, 무작위 맵이 만든 전장이 멀티에서도 실제로 그려지고 규칙대로 동작하는지를 눈으로 확인했다. 다만 **전부 확인된 것은 아니다** — 멀티는 아직 seed 가 고정이라 매 판 같은 맵이 나오고, 그래서 「멀티에서 매 판 다른 맵이 나오는가」는 **이번에도 확인할 수 없었다.**

🔴 **검증된 것과 검증되지 않은 것의 경계 — 이 문단이 그 단일 소스다.**
- ✅ **검증됐다** — 멀티 경로에서 **무작위 맵 시스템이 실제로 돈다**: 격자 11×21 과 투영 · 성/광산 배치 · 빗금과 구멍 렌더링 · 빗금 타일 건설 차단 · 빗금 위 광산의 채굴소 예외 · 맵 준비 로그 · **Host 와 Client 가 같은 맵을 본다.**
- ⚠️ **검증되지 않았다** — **「멀티에서 매 판 다른 맵이 나오는가」는 이번에도 확인 불가다. seed 가 고정이라 구조적으로 확인할 수 없으며 3단계가 해결한다.** 그리고 **Host/Client 대조는 두 화면이 같은 맵임을 눈으로 본 것이지 칸 단위 대조가 아니다** — 사용자 본인이 **「빗금 105칸을 세어본 건 아니고 같은 맵이 나온 것만 확인했다」**고 적었다. **「AI 가 빗금 타일에 일반 건물을 짓지 않는가」는 여전히 도달 불가**다(위 2026-09-08 항목 ③ 그대로 유효).

**1차 (고정 seed `1`, 커밋 `2cb0867` 시점)** — 사용자 보고는 「여러 번 시도했으나 맵이 변경되지 않고 하나뿐」이었고 **이것은 정상이다**(멀티는 임시 고정 seed 를 쓰므로 매 판 같은 맵이 나온다). 메인 세션이 맵 준비 코드를 실제로 실행해 대조한 결과 **스크린샷과 일치**했다 — `seed=1 → FullyOpen · 중립 광산 3개(좌표 (5,10)(8,10)(2,11)) · 성 Red(5,1) Blue(5,19) · 시작 광산 Red(7,1) Blue(3,19) · 타일 일반 225 / 빗금 0 / 막힘 6 · 격자 11×21`. **옛 고정 맵은 10×29 였으므로, 이 대조 자체가 「멀티에서 무작위 맵 시스템이 실제로 돌고 있다」의 직접 증거다.**

**2차 (고정 seed `11`, 커밋 `b515995`)** — 빗금 렌더링 · 구멍(막힌 칸) 표시 · 빗금 타일 건설 차단 · 빗금 위 광산의 채굴소 예외 · 맵 준비 로그가 **전부 정상 동작**했고 **Host 와 Client 가 같은 맵**을 봤다. 단 **칸 수를 대조한 정밀 검증은 아니다**(위 경계 참조).

🔴 **버그로 오해하기 쉬운 항목 — 빗금 타일을 탭해도 토스트는 「그 타일을 점령한 뒤」에만 뜬다.** 2차 테스트에서 사용자가 처음 「토스트가 안 뜬다」고 보고했으나 **규정대로의 동작이었다.** `GameSystemRules_UI.md` 「건물 배치 패널 UI」 **규칙 7·8** 이 **자기 팀 소유** 건설 불가 타일에만 토스트를 띄우고 **중립·적 소유면 선택만** 하도록 정하는데, **경기 시작 시점에 빗금 타일을 소유한 팀은 없다** — seed 11 실측(메인 세션)으로 **Blue 초기 소유 12칸 중 빗금 0 · Red 초기 소유 12칸 중 빗금 0 · 맵 전체 빗금 105칸이 전부 중립에서 시작**이다. 사용자가 **빗금 타일을 점령한 뒤 탭하니 토스트가 정상 표시**됐다. 🔴 **이것은 위 ③「AI 가 빗금 타일에 닿을 수 없다」와 정확히 같은 구조다** — 초기 보유 칸에 빗금이 0개라 점령 전에는 **사람도 AI 도** 빗금 타일을 소유하지 않는다. **결함이 아니라 규칙대로의 동작이므로, 다음 사람이 버그로 접수하지 않도록 여기에 못 박아 둔다.**

⚠️ **곁가지 — 「맵이 정한 초기 골드가 적용되지 않는다」가 실기로 드러났다.** 1차 스크린샷의 골드가 **5135** 였다. `seed=1` 맵은 중립 광산이 **3개**라 규칙 3 의 표대로면 초기 골드가 **500** 이어야 하는데, 실제로는 `GameConfig.StartingGold`(**5000**)가 그대로 쓰인다(문서 작업 중 직접 실측 — `Bootstrap/GameBootstrapper.Setup.cs` 가 `new ResourceUseCase(_config.StartingGold)` 로 만들고, `Resources/Config/GameConfig.asset` 의 `_startingGold` 가 `5000` · `_mapTestModeEnabled` 는 `0` · 광산 수→골드 표는 `Domain/Map/MapDefinitionValidator.cs` 의 `{ 0, 700, 600, 500, 400, 300, 200 }` 배열 한 곳뿐이다). **이것은 아래 ⑥ 보류 8건 중 「`InitialGold` 미사용」이 실기로 드러난 것**이며, `GameSystemRules_RandomMap.md` **규칙 16** 이 Host 가 「실제 초기 골드」를 보내고 Client 가 **그대로 로드**하도록 정하므로 **3단계에서 다루지 않으면 규칙 16 이 미충족으로 남는다.** ⚠️ **다만 사용자는 3단계 범위 선택에서 이 항목을 고르지 않았다 — 「하지 말라」인지 「이번엔 아니다」인지 확정되지 않았으므로 범위에 넣지 않고 사실만 적는다.**

**테스트용 임시 seed 변경 (커밋 `b515995`)** — 멀티 임시 고정 seed 를 `1` → `11` 로 바꿨다. 바뀐 것은 `Bootstrap/GameBootstrapper.cs` 의 상수 `NetworkInterimRootSeed` **값 하나와 주석뿐**이고 **seed 분기 구조와 싱글플레이 경로는 무변경**이다(문서 작업 중 직접 실측 — 240행 `private const ulong NetworkInterimRootSeed = 11UL;`). 🔴 **숫자 자체에 의미는 없다** — 밸런스가 검증된 맵이 아니라 **테스트에 쓸모 있어서** 고른 값이다(`seed=11 → ThreeLane · 빗금 105칸 · 막힘 28칸 · 중립 광산 5개(그중 1개가 빗금 위)`. 종전 값 `1` 은 빗금이 0칸이라 빗금·구멍·채굴소 예외를 멀티에서 볼 방법이 아예 없었다). **당분간 11 로 유지한다** — 3단계 전송을 만드는 동안 빗금·구멍이 있는 맵이 계속 나와야 전송이 그 정보를 제대로 싣는지 화면으로 확인된다. **3단계가 이 분기 자체를 없앤다.** 🔴 **싱글플레이는 원래부터 매 판 랜덤이다** — `Bootstrap/GameBootstrapper.Map.cs` 의 `CreateRootSeed()` 가 싱글에서는 `Guid` + `UtcNow` 로 매 판 새로 뽑는다(문서 작업 중 직접 실측). **고정된 것은 멀티뿐이며, 이 구분이 흐려지면 「무작위 맵인데 왜 고정이냐」는 오해가 생긴다.**

**※ 근거 구분(규칙 10):** 스크린샷·화면에서 본 동작·「같은 맵이 나온 것만 확인했다」는 **사용자 실기 관찰**이고, `seed=1`·`seed=11` 이 만드는 맵의 값(유형 · 빗금/막힘 칸 수 · 광산·성 좌표 · 초기 보유 12칸 중 빗금 0개)과 커밋 해시는 **메인 세션이 맵 준비 코드를 실행해 측정·전달한 값**이며, `NetworkInterimRootSeed = 11UL` · `CreateRootSeed()` 의 `Guid`+`UtcNow` · `_config.StartingGold` 소비 지점 · `_startingGold: 5000` · 초기 골드 표 배열 · UI 규칙 7·8 본문 · 규칙 16 본문은 **문서 작업 중 직접 실측한 값**이다.

**✅ 구현 완료 (2026-09-08) — 검증 상태: 싱글플레이 에디터 실기 검증 완료 / 멀티 미검증:** **무작위 맵 2단계 — A~K 전 단계 완료.** **[🔴 2026-09-09 — 원문은 그대로 두고 덧붙인다: 머리의 「멀티 미검증」은 2026-09-08 시점의 사실이며, 2026-09-09 멀티 실기 2회로 **부분 해소**됐다. 무엇이 검증되고 무엇이 여전히 안 됐는지의 경계는 맨 위 「⚠️ 멀티플레이 실기 검증 (2026-09-09)」 문단이 단일 소스다]**

> **[🔴 2026-09-08 갱신 — 원문은 그대로 두고 표기만 바꾼다]** 이 문단의 머리는 같은 날 **「⚠️ 진행 중 … 🔴 2단계는 아직 끝나지 않았다(K 단계 남음)」**으로 적혀 있었다. **K 단계(맵 준비 로그 키)가 커밋 `2e1b305` 로 끝나고 싱글플레이 실기 검증까지 마쳐** 완료로 바꾼다. 아래 「과대 표기 금지」 ①의 원문도 지우지 않고 해소 표시를 덧붙였다.

쉽게 말하면, **이제 싱글 경기에 들어가면 매 판 다른 전장이 나온다.** 성과 광산 자리가 코드에 박혀 있던 것을 걷어내고, 맵을 만들어 내는 기계 · 만든 맵을 검사하는 기계 · 검사에 계속 실패할 때 쓸 예비 맵 · 그 맵을 실제 전장으로 옮기는 일 · 막힌 곳을 빈 공간으로 그리고 지을 수 없는 곳에 빗금을 얹는 일 · 그 빗금 규칙을 사람과 AI 양쪽에 똑같이 적용하는 일까지가 들어갔다. 사용자 실기 테스트로 **매 판 광산 위치·시작 광산 방향·초기 골드가 바뀌는 것**과 **기존 기능이 그대로 도는 것**을 확인했다. 마지막으로 **어떤 맵이 왜 그렇게 만들어졌는지를 기록으로 남기는 일**(K)까지 끝냈다 — 그 값들은 이미 계산되어 있었지만 **꺼내 쓰는 쪽이 없어 매 판 그대로 버려지고 있었다.** 사용자가 경기를 여러 판 돌려 **맵이 매번 다르게 나오는 것을 육안으로 확인**했고(2단계의 주 판정), 경기당 맵 준비 로그가 **한 줄만** 제 위치에 찍히는 것과 **빗금 타일 위 광산에 채굴소가 지어지고 주변 타일 색상이 바뀌는 것**도 실기로 확인했다.

단계별 결과(커밋): **A~E** 결정적 난수·대칭 builder·초기 상태 평가·생성기 5종·검증기 / **F** 폴백 템플릿 5개 + 제작 도구 `996cc0a` / **G** 맵 준비 조정자 `7eea8f0` / **H** `GameConfig` 11×21 + 투영 `f432fb5`(**여기서 처음 화면이 바뀐다**) / **J** 렌더러 + 클릭 판정 `a2a8c60`·후속 `a19fe07` / **I** 판정 조건 전환 `c47a067` / **K** 맵 준비 로그 키 `2e1b305`. 🔴 **I 와 J 는 계획의 I → J 순서를 J → I 로 바꿔 실행했다**(사용자 승인) — 렌더링이 먼저 서야 「빗금 친 곳에 건설이 막히는가」를 눈으로 확인할 수 있기 때문이다.

⚠️ **과대 표기 금지 — 아직 완료가 아닌 것:** ① ~~🔴 **2단계는 미완이다. K 단계(맵 준비 로그 키 신설)가 남아 있다** — 규칙 12 가 요구하는 로그 항목이 아직 로그로 나가지 않는다(현재는 결과 객체에 값으로만 담겨 있다).~~ **[🔴 2026-09-08 해소 — 원문은 그대로 둔다]** 커밋 `2e1b305` 으로 `LogEvent` 키 4개(`MapPreparationSucceeded`/`UsedFallbackTemplate`/`Failed`/`MapProjectionFailed`)를 신설해 **enum 멤버 37개 → 41개**가 됐고, 발신은 호출부 `GameBootstrapper.Map.cs` 에서만 한다. ⚠️ 다만 **`GameBootstrapper` 계열은 `UnityEngine` 참조라 헤드리스 컴파일 검증이 불가하며 컴파일 통과는 Unity 에서만 확인된다**(아래 ⑤와 같은 사유). ② 🔴 **멀티플레이 seed 가 임시 고정값(1)이다.** `LoadMap` 이 싱글·멀티 공용이라 그대로 두면 Host 와 Client 가 각자 seed 를 뽑아 서로 다른 맵을 보게 되므로 임시로 고정했다. **3단계에서 Host 가 seed 를 전송하면 사라지며, 그때까지 멀티는 매 판 같은 맵이 나온다.** ③ 🔴 **「AI 가 빗금 타일에 건물을 짓지 않는가」는 현재 실기로 확인할 수 없다 — 검증 누락이 아니라 도달 불가다.** **[2026-09-08 — 갈라 적는다]** 같은 빗금 규칙 중 **「빗금 타일에서도 채굴소는 예외로 허용」 경로는 실기로 닫혔다**(사용자가 빗금 타일 위 광산에 채굴소를 짓고 주변 타일 색상 변경까지 확인). **여전히 닫히지 않은 것은 AI 의 일반 건물 경로 하나뿐이며 그것이 아래 사유로 도달 불가다.** 각 유형 200시드(외곽형만 100판) **900판 전부**에서 경기 시작 시점 Red 보유 10칸 중 빗금 타일이 **0개**였다(규칙상 양 팀 초기 보호 10칸이 일반 타일로 보장된다). AI 가 빗금 타일을 건설 후보로 만나려면 유닛으로 빗금 구역까지 진출해 점령을 넓혀야 하는데 **AI 시나리오가 이 맵을 전제로 짜인 것이 아니라 그 상황이 만들어지지 않는다.** 맵 전체 빗금 평균은 완전개방형 0.0 · 장애물개방형 0.0 · 협곡형 17.6 · 외곽형 26.5 · **3라인형 75.8**(225 재생 칸 중)이라, **AI 시나리오를 이 맵에 맞추는 순간 이 판정이 곧바로 실전에서 쓰인다. 지금 닿지 않을 뿐 죽은 코드가 아니다.** ④ ~~**멀티플레이 실기 미검증** — 위 검증은 전부 에디터 싱글플레이 세션이다.~~ **[🔴 2026-09-09 부분 해소 — 원문은 그대로 둔다]** 멀티플레이(랜덤 매칭) 실기 2회로 **무작위 맵 시스템이 멀티 경로에서 실제로 도는 것**이 확인됐다(격자 11×21 과 투영 · 성/광산 배치 · 빗금과 구멍 렌더링 · 빗금 타일 건설 차단 · 빗금 위 광산의 채굴소 예외 · 맵 준비 로그 · **Host 와 Client 가 같은 맵을 본다**). ⚠️ **그럼에도 「멀티에서 매 판 다른 맵이 나오는가」는 여전히 확인 불가다 — seed 가 고정이라 구조적으로 확인할 수 없으며 3단계가 해결한다.** 그리고 **Host/Client 대조는 두 화면이 같은 맵임을 눈으로 본 것이지 칸 단위 대조가 아니다.** 경계의 단일 소스는 맨 위 「⚠️ 멀티플레이 실기 검증 (2026-09-09)」 문단. ⑤ **유스케이스·서비스·네트워크 컨트롤러의 컴파일 통과는 Unity 에서만 확인된다**(`UnityEngine` 참조라 헤드리스 컴파일이 불가하다). ⑥ **보류 항목 8건**(멀티 seed 임시 고정 · `InitialGold` 미사용 · 맵 준비/투영 실패 처리 미명세 · 규칙 13 임계값 3·2 기획 확인 · 생성기 주입 이음매 없음 · `AGENTS.md` 에 `_Reference` 누락 · `PlaceMiningPostDirect` 막힌 타일 가드 없음 · `NoBuildHatch.mat` 공유 머티리얼 오염 가능)은 **미해결 결함이 아니라 사용자가 이번 범위 밖으로 확정한 항목**이며 목록은 `_Tasks/2026-09-03/03_14_random-map-phase2-generator/Plan.md` §13. ⑦ ⚠️ **확인되지 않은 것 — K 실기 당시 콘솔에 오류 5건·경고 4건이 잡혀 있었으나 정체를 확인하지 않았다.** 맵 로그가 `MapPreparationSucceeded` 였고 지형·채굴소가 정상 동작했으므로 **준비 실패·투영 실패 두 키는 배제되지만**, 이는 **「맵과 무관함이 확인됐다」가 아니라 「맵 경로는 배제되지만 정체는 미확인」**이다. 근거: 같은 Plan §9·§12·「K 단계 상세」.

**구현 완료 (2026-09-03) — 검증 상태: 에디터 실기 검증 완료 / 멀티 미검증:** **무작위 맵 1단계 — 타일 상태 계약 전환.**

쉽게 말하면, 타일이 「지나갈 수 있는가」를 **저장해 두고 여기저기서 켜고 끄던 방식**에서 **그때그때 계산해서 답하는 방식**으로 바꿨다. 예전에는 건물이 생겼을 때·건물이 없어졌을 때·광산이라 원래 막힌 칸일 때가 전부 같은 스위치 하나를 건드렸고, 그래서 「채굴소를 부쉈는데 광산이라 계속 막아야 한다」 같은 조건을 한 줄에 뒤섞어 적어야 했다. 이제는 **막힌 지형인가 · 광산이 있는가 · 건물이 서 있는가** 세 가지를 각각 따로 기록하고, 「지나갈 수 있는가」는 그 셋으로부터 계산한다. 계산 결과라서 손으로 덮어쓸 수 없다.

에디터 실기(2026-09-03)에서 확인한 것: 초기 배치 **성 2 · 시작 채굴소 2 · 광산 타일 4** 정상 / 광산 타일 4칸 전부 이동 불가이며 **그중 중립 광산 2칸은 건물이 없는데도 이동 불가**(계산 방식이 실제로 동작한다는 직접 증거) / **채굴소를 철거해도 이동 불가가 유지**(이번 전환의 최대 위험 지점이 통과했다는 뜻) / 일반 건물 철거 후에는 이동 가능으로 복구 / 발급된 경로 **43건 전부 중간에 이동 불가 칸 0개** / **AI 건물 배치 2건 전부 성사**.

⚠️ **과대 표기 금지 — 아직 완료가 아닌 것:** ① **무작위 맵 자체는 미구현이다.** 지금 경기는 여전히 고정 맵을 쓴다. **2단계(2026-09-03 범위 재조정)** 는 생성기 5종·검증기·폴백에 더해 **만든 맵을 실제 전장으로 옮기는 것까지**다 — 완료 판정이 「싱글 경기에서 매번 다른 맵이 나오고 정상 플레이된다」로 바뀌었다. ② 이번에 생긴 타입 6종 중 **`MapType`·`DecorationDefinition`·`MapDefinition`·`MapDefinitionCodec` 4종은 아직 어느 코드에서도 호출되지 않는다.** ③ **AI 건물 배치 후보 판정**(과 그 XML 주석)은 여전히 이동 가능 여부로 판정하며 **2단계** 몫이다(2026-09-03 재조정 — 종전 3단계) — 같은 파일의 광산 타일 조회는 전환됐으므로 한 파일 안에 전환된 자리와 안 된 자리가 섞여 있다. ④ **건설·점령의 전용 조건 전환도 미완**(일반 건물 배치가 아직 이동 가능 여부를 그대로 쓴다) — 이것도 **2단계** 몫이다. 현재 고정 맵에는 「이동은 되지만 건설만 막히는 타일」이 하나도 없어 결과가 갈리지 않을 뿐이다. ⑤ **멀티플레이 실기 미검증** — 위 검증은 전부 에디터 싱글플레이 세션이다. 근거: `_Tasks/2026-09-01/19_49_random-map-phase1-tilekind/`.

**현재 단계:** **무작위 맵 2단계 결과의 멀티플레이 실기 검증이 2026-09-09 에 처음 이뤄졌다 — 부분 검증이며, 다음은 여전히 3단계(맵 전송·해시 대조·재경기)다**(맨 위 「⚠️ 멀티플레이 실기 검증 (2026-09-09)」 문단 참조. **멀티 seed 는 `1` → `11` 로 바뀌었을 뿐 여전히 고정이라 「매 판 다른 맵」은 멀티에서 확인 불가 · Host/Client 대조는 칸 단위가 아님 · 빗금 토스트는 점령 후에만 뜬다(규정대로) · 초기 골드 미적용이 실기로 드러났으나 범위 밖**). 직전 **무작위 맵 2단계가 A~K 전 단계 구현·싱글플레이 실기 검증까지 끝났다 — 다음은 3단계(맵 전송·해시 대조·재경기)다**(2026-09-08 — 위 「✅ 구현 완료 (2026-09-08)」 문단 참조. **멀티플레이 실기 미검증 · 멀티 seed 임시 고정(1) · AI 빗금 판정은 도달 불가로 실기 확인 불가 · `GameBootstrapper` 계열은 Unity 에서만 컴파일 확인 가능 · Plan §13 보류 8건**). **[🔴 2026-09-09 갱신 — 원문은 그대로 두고 덧붙인다: 「멀티플레이 실기 미검증」은 멀티 실기 2회로 **부분 해소**됐고, 「멀티 seed 임시 고정(1)」의 값은 커밋 `b515995` 로 **1 → 11** 이 됐다(여전히 고정이다). 「AI 빗금 판정 도달 불가」와 나머지 항목은 그대로 유효하다. 경계는 맨 위 「⚠️ 멀티플레이 실기 검증 (2026-09-09)」 문단 참조]** ~~**무작위 맵 2단계가 A~J 까지 구현·싱글 실기 검증까지 끝났고, 남은 것은 K(로그 키) 하나다 — 즉 2단계는 아직 완료가 아니다**~~ **[🔴 2026-09-08 갱신 — 원문은 그대로 두고 덧붙인다: K 가 커밋 `2e1b305` 로 완료됐다]**. 직전 **무작위 맵 1단계(타일 상태 계약 전환)가 구현·에디터 실기 검증까지 끝났고 다음은 2단계 — 싱글플레이에서 무작위 맵이 실제로 도는 것까지다**(2026-09-03 — 위 항목 참조. **멀티 미검증 · 무작위 맵 본체 미구현**. 같은 날 단계 경계를 「싱글 = 2단계 / 멀티 = 3단계」로 재조정해 **AI 배치 후보 판정 전환과 건설·점령 전용 조건이 2단계로 내려왔다** — 범위표의 단일 소스는 `_Tasks/2026-09-03/03_14_random-map-phase2-generator/Research.md` §0). 직전 **네트워크 종료 시점 가드가 `Infrastructure/Network/` 의 이벤트 구독 경로 전체에서 한 가지 형태(`!IsSpawned || !IsServer`)로 통일되었다**(2026-08-20 — 위 항목 참조. 직전 6곳 + 이번 8곳 = **6파일 14곳** · ~~**⚠️ 이번 8곳은 실기 재현 0회이자 실기 미검증**~~ → **🔴 2026-08-24: 재현 0회는 그대로이나 멀티 실기 3경기(`[ERROR]` 0건 · 3경기 정상 종료)로 회귀 없음을 확인. 단 8곳 중 서버 발화가 로그로 확인된 것은 ③⑧ 2곳뿐이고 나머지 6곳은 호출당 로그가 없어 셀 수 없다 · `_combatStopped` 재경기 리셋은 여전히 미검증(2·3경기 에디터가 클라이언트였다)** · 위험 구간 실측 **6~41ms**(27ms 단일값 정정) · `ProductionTicker` 종료 가드는 **제안만** · `NetworkBuildingController`·`NetworkUpgradeController` 는 **구독 0건이라 미점검**). 그리고 **로그 체계(`GameLog`) 전환이 코드 전 계층에서 완료되었고, 전역 훅의 수집 범위가 엔진 오류까지 넓어졌다**(2026-08-19 — 위 항목 참조. 누적 386건 이관 · 최종 잔존 46건 · `LogEvent` **37개** · **A/C 실기 확인, B(엔진 오류 수집) 미검증** · `SetCameraStartPositionForTeam` 최종 삭제 잔여). ~~*"…컴파일 통과, 실기 미검증"*(2026-08-18 시점)~~ 직전 MistShrine(HealShrine) 물안개 힐 **구현 완료 · 에디터 싱글플레이 실기 검증 완료 / 멀티 미검증**(2026-08-12 — 위 항목 참조. 범위 판정·아우라 끊김·회복량·중첩 해소를 로그 실측으로 확인. **멀티 실기·물안개 VFX·버튼 아이콘·밸런싱 수치는 미완**). 직전 랠리포인트 조준 시 반투명 오버레이 잔존 버그 수정·실기 테스트 PASS(2026-08-08, `ProductionPanelUI` 2줄 추가 — 조준 진입 시 `HideBlockingOverlay()` + `Close()`를 우회하는 랠리 완료 경로의 참조 카운터 반납). 직전 건물 파괴 시 열린 패널/조준 UI 원복 구현·실기 테스트 PASS(2026-08-08, `BuildingPanelBase`가 `OnBuildingDied` 구독→현재 건물 매칭 시 `Close()`, 4개 건물 패널 공통 커버·`.AddTo(this)` 해제). 직전 스킬 건물 시스템 Phase 1(타입 A·B + 프레임워크 + 3×3 패널 UI/쿨다운 오버레이 + 모바일 탭 조준 + 조준 지점 연속 좌표화 + 조준원 지면 데칼 + 취소 버그·쿨다운 토스트) 구현·실기기 테스트 PASS(2026-08-04)에 이어 타입 C(전역 상태변경: 버프/디버프/CC/힐) Phase 2도 구현·실기+멀티(클라) PASS(2026-08-05) — 스킬 관련 잔여는 구체 스킬 목록/수치/아이콘(기획)뿐. 직전 연구소 유닛 강화 시스템 + 전투 스탯 ×10 개편 구현·멀티 실기 완료(2026-07-31). 직전 QuakeSpirit(대지의 정령) 착탄형 즉발 범위 딜러 구현 완료(2026-07-20, QA PASS·**멀티플레이 실기 로그 검증 완료**) — 마지막 남은 특수 유닛으로, 이로써 **특수 유닛 5종(BattleAxe/TorrentSpirit/BloomFairy/MushroomBomber/QuakeSpirit) 전량 + InfernoSpirit DoT까지 전부 완료**. 착탄형 **즉발** AoE(DoT 아님): 주 타깃 1마리 **직접 100%(공격력 200)**(스플래시 제외, 건물이 주 타깃이면 100%로 공성) + 착탄 **월드 좌표 원형 반경**(`_quakeRadius` 기본 1.0=인접 1칸) 내 **다른 적 유닛·적 건물 50%**(올림 `CeilToInt(200×0.5)`=100, 주 타깃 제외). ⚠️ **MushroomBomber/BattleAxe와 달리 스플래시가 건물도 포함**(그쪽 두 유닛의 AoE는 유닛만 — 차별점). 아군 무피해, 서버 권위. 구현: 핸들러 `QuakeAttackBehavior`(`ReplacesPrimaryAttack=false`) + 레지스트리 `QuakeSpirit → QuakeAttackBehavior` 1줄, MushroomBomber 원형 반경 헬퍼(`CollectEnemyUnitsInRadius`)를 `internal static`으로 공용화 재사용(MushroomBomber 유닛만 로직 무변경) + 건물 순회 `CollectEnemyBuildingsInRadius` 신설 + 유닛/건물 hit-set 분리(규칙 29 계승), `SpecialAttackConfig` `_quakeRadius`/`_quakeSplashRatio`(1.0/0.5) GameBootstrapper 주입(폴백 1.0/0.5). **멀티플레이(Host/Client) 실기 로그로 100%=20·스플래시=10·건물 스플래시·주 타깃 제외·반경 이내·아군 무피해·서버↔클라 동기화 전부 정상 검증**(로그 `_Logs/2026-07-20/11_00_quakespirit-aoe-verify/`). 규칙 43 등재. **알려진 이슈(보류, 별도 task 분리)**: ① QuakeSpirit 타격 애니↔데미지 텍스트 타이밍 어긋남 — Attack 클립 `OnAttackHit` 미주입 + placeholder `hitFrameTimes`(1.0s)로 스플래시 텍스트가 `HitPresentationQueue` 타임아웃(쿨다운×1.5=7.5s)까지 지연(피해·판정·동기화 자체는 정상), ② 멀티 원거리 공격 facing 버그(InfernoSpirit 작업에서 진단) — 둘 다 사용자 결정으로 이번 미수정. 직전 InfernoSpirit(지옥불 정령) 단일 대상 지속 피해(DoT) 구현 완료(2026-07-20, QA PASS·사용자 실기 테스트 완료) — 특수 유닛 5종 외 추가 DoT 유닛. 이미 완성된 원거리 유닛(에셋·VFX·생산·`OnAttackHit` 완비)에 DoT 특수만 추가. 원거리 공격이 주 타깃 1마리에 **직접 250**(기존 단일 피해 `ReplacesPrimaryAttack=false`, 건물 공성 포함) + 특수 핸들러 `InfernoAttackBehavior`가 그 **주 타깃 1마리에게만** DoT 50/초×3초(총 150) 부여(AoE 아님·반경 없음). DoT는 **적 유닛만**(건물 제외 — 건물은 직접 250만), 주 타깃 유닛은 직접+DoT, 아군 무피해, 갱신=리셋. DoT는 MushroomBomber에서 만든 **규칙 40 초 단위 discrete 틱 시스템을 단일 대상으로 재사용**(1초 간격·틱당 올림·총량 150 클램프·매초 남은 체력 데미지 텍스트·서버 권위). InfernoSpirit(50/3)은 MushroomBomber(20/3)와 **별도 필드·델리게이트·진입점**(`_infernoDot*`/`ApplyInfernoDot`)으로 분리 — MushroomBomber 회귀 없음. 튜닝값 `SpecialAttackConfig`(`_infernoDotPerSecond`/`_infernoDotDuration`), GameBootstrapper 주입(폴백 50/3). 레지스트리 `InfernoSpirit → InfernoAttackBehavior` 1줄. 규칙 41~42 등재. **잔여 특수 유닛(레지스트리 확인 결과 QuakeSpirit이 유일 미구현 — BattleAxe/TorrentSpirit/MushroomBomber/InfernoSpirit 등록됨, BloomFairy는 힐러 전용 경로로 의도적 미등록)**: QuakeSpirit(착탄형 — MushroomBomber 원형 반경 헬퍼 재사용 예정). **알려진 이슈(보류)**: 멀티플레이 원거리 공격 시 유닛이 타겟을 정확히 안 바라보는 facing 버그 — 진단상 InfernoSpirit 에셋은 정상 유닛(FlameSpirit)과 동일 → 멀티 원거리 facing 공유 로직 문제로 추정(근접 유닛은 안 보임). 사용자 결정으로 이번 미수정·보류, 상세는 task Plan 참조. — 직전 MushroomBomber(버섯폭격기) 착탄형 범위 딜러 구현 완료(2026-07-19, QA PASS·사용자 실기 테스트 완료) — 특수 유닛 5종 중 **4번째**. 착탄형 AoE: 주 타깃 1마리 **직접 100**(건물 공성 포함, 기존 `ExecuteAttack` 단일 피해 `ReplacesPrimaryAttack=false`) + 착탄 중심 **월드 원형 반경**(`blastRadius` 기본 1.0=인접 1칸) 내 적 유닛 **DoT 20/초×3초(총 60)**. DoT는 HoT/DoT 공용 시스템(규칙 34)에 신설한 **초 단위 discrete 틱 모드** — 1초 간격·틱당 올림(CeilToInt)·총량 60 클램프·매초 남은 체력 데미지 텍스트(힐과 반대로 억제 안 함), 서버 권위. 핸들러 `BlastAttackBehavior`(원형 반경 수집 static 헬퍼 — QuakeSpirit 재사용 여지) + 레지스트리 `MushroomBomber → BlastAttackBehavior` 1줄. 식물 라인 생산 배선 완료(SporePatch 1단계=MushroomBomber, FloralNursery 2단계=BloomFairy). 규칙 38~40 등재. VFX(투사체·폭발)는 사용자 별도 제작. 직전 **BloomFairy(꽃요정 힐러)** 구현·실기 테스트 완료(2026-07-18) — 특수 유닛 5종 중 **3번째**. 적을 때리지 않고 부상 아군을 회복하는 **힐러 전용 경로**(적 공격 흐름·`SpecialAttackRegistry`와 분리), 부상 아군 탐색(팀 필터 반대·본인 포함), HoT 공용 시스템(규칙 34 diff 틱)·힐 유휴 감시·쿨다운 예외(발동 준비 1.0s 미포함 → 실제 힐 주기 4.0s, 프로젝트 유일 예외)·HoT 힐 텍스트 완료 시 1회 표시(사망 시 생략). 규칙 32~37 등재. **잔여 특수 유닛(코드 확인 결과 특수 공격 핸들러 미등록 = 미구현)**: QuakeSpirit(착탄형 — MushroomBomber 원형 반경 헬퍼 재사용 예정), InfernoSpirit(DoT — 초 단위 틱 재사용 예정). 그 이전 TorrentSpirit(물의 상급 정령) 파도형 이동 AoE + 힐 서브시스템 구현 완료(2026-07-17) — 특수 유닛 5종 중 **2번째**. special-only(단일 공격 없음, `ReplacesPrimaryAttack`) 파도가 전방으로 이동(서버 권위 전선 `SpawnWave`/`TickWaves`)하며 월드 직사각형 안 **적 유닛·적 건물 피해 / 아군 유닛 힐**(각 1회, hit-set). **힐 서브시스템 신설**(`UnitData.Heal`·`OnEntityHealed`·NetworkHealthSync 힐 동기화·치유 색상 텍스트 — BloomFairy 공용). `SpecialAttackConfig` 파도 파라미터(폭 3·전방 3·이동시간 0.5·힐 100) Inspector. `VfxPoolItem` 다중 파티클 시스템(빈 루트+형제) 재생 수정. UnitStatsConfig(18)/EffectPreset/OnAttackHit(0.5s 임시) 배선. QA CONDITIONAL PASS(BUG-002 건물 공격 수정 완료). 규칙 28~31 등재. VFX 세부 튜닝은 사용자 진행. — 직전 도끼병(BattleAxe) 휩쓸기형 AoE + 특수 공격 전략 핸들러 아키텍처 구현 완료(2026-07-17, 사용자 실기 PASS) — 특수 유닛 5종(BattleAxe/QuakeSpirit/TorrentSpirit/MushroomBomber/BloomFairy) 중 첫 구현. `ISpecialAttackBehavior` + `SpecialAttackRegistry`(UnitType 키) + `SweepAttackBehavior` 신설, `UnitCombatUseCase.ExecuteAttack`을 `ApplyDamageToVictim` 헬퍼로 정리 후 특수 공격 훅 1줄 추가(신규 유닛 = 핸들러 + 등록 1줄). 휩쓸기 판정은 월드 좌표 전방 부채꼴(반경 `sweepReach` 실기값 0.75 · 반각 120°, `SpecialAttackConfig` SO 튜닝), AoE 피격 연출 동시 방출(HitFrameTimes.Length 분기). BattleAxe attackRange 0.5→0.75, Attack 클립 OnAttackHit 1.1667s 주입. 규칙 23~27 등재. 직전 Android AAB 빌드 용량 최적화 완료(2026-07-15, main 반영) — `codex/asset-size-optimization` 작업이 main에 병합되어 AAB 용량을 **190.66 MB → 125.30 MB**로 65.36 MB 절감. 핵심 변경은 3D 건물/유닛 텍스처 Android max texture size `1024 → 512` 적용이며, `_Old` 미사용 에셋 정리와 보수적 FBX import 조정도 함께 수행. UI 스프라이트/초상화/건물 아이콘/UI 배경/TMP 폰트는 최종 품질 확인 전 유지. 직전 코드 정리 3건(2026-07-13), 이동/Walk 애니메이션 동기화(2026-07-13), 전투 타격 타이밍 동기화(2026-07-12) 완료 상태 유지. 싱글플레이 AI 시스템은 Inspector 작업(AIConfig/AIScenarioConfig 3종족 에셋 생성, DifficultySelectView 레이아웃 배치)이 완료되었고, 핵심 흐름(유닛 생산/건물 업그레이드) 실기가 조건부 완료(2026-07-16, PASS·특별한 문제 미발견)됨 — 단 반응 시스템(R1~R3)·3종족 시나리오 무작위 동작 등 세부 정밀 검증은 미완. 후속은 기기 QA로 3D 텍스처 품질 확인, 피격 VFX 프리셋 연결, 잔여 특수 타격(QuakeSpirit/InfernoSpirit) 클립 이벤트, Firebase/EDM 저장소 방침 정리, AI 반응 시스템·3종족 시나리오 정밀 검증, 신규 유닛 프리팹 실기 테스트. **병행 관찰 항목 — 매치메이킹 404(호스트 결정 단계) 수정(A방식, 2026-07-17, 브랜치 `claude/matchmaker-404-error-pi9qdn` 커밋 `a3dbc73`)**: 랜덤 매칭 후 호스트 결정을 매치 결과 조회(P2P 클라 호출 시 404) → Lobby CreateOrJoin(matchId=lobbyId) 원자 선점으로 전환. 초기 매칭 실기에서 404 없이 정상 연결 확인했으나 간헐(intermittent) 버그라 지속 테스트 중(확정 PASS 아님) — 비활성화(주석)한 레거시 코드는 지속 테스트 확정 후 삭제 예정. 상세는 하단 매치메이킹 404 섹션 참조. **※ 2026-08-10 ×10 스케일 반영 정정:** 이 "현재 단계" 문단의 전투 수치(직접 피해·DoT 틱값·파도 힐량)를 ×10 스케일 개편(2026-07-31) 기준으로 정정했다. **비율·반경(100% / 50% / ×0.5 / 반경 1.0)과 쿨다운·사거리·비용은 ×10 대상이 아니라 그대로다.** 단, 위 QuakeSpirit 문장 안의 **2026-07-20 멀티 로그 검증 기록(100%=20·스플래시=10)은 당시 측정 사실이므로 원문을 보존**한다(현재 유효 수치는 200/100). 아래 "✅ 완료 (2026-07-19/07-20)" 표들도 날짜가 박힌 완료 이력이라 원문 보존한다. 단일 진실 소스: `StatsReference.md`.
...
- **상태 (2026-09-03 갱신):** 문서 설계 완료. **1단계(타일 상태 계약 전환)만 구현·에디터 실기 검증 완료** — 상세는 위 「구현 완료 (2026-09-03)」 문단과 「완료된 시스템 > 코어 게임플레이」 표의 **타일 상태 계약** 행. **런타임 생성기·검증기·전송·테스트 모드·건설 불가/차단 지형·경로 완전 차단 대응은 여전히 미구현**이다(2·3단계).
  - **[🔴 2026-09-08 갱신 — 위 문장은 그대로 두고 덧붙인다]** **2단계 A~J 가 구현·싱글 실기 검증까지 끝났다.** 위 문장이 「여전히 미구현」이라고 적은 것 중 **런타임 생성기 5종 · 검증기 · 폴백 5개 · 테스트 모드 설정 필드 · 건설 불가/차단 지형 · 맵 준비 조정자 · `MapDefinition` → `HexGrid` 투영 · 렌더러 · 건설/점령/AI 판정 조건**은 **구현됐다.** **여전히 미구현인 것은 ① 2단계 K(맵 준비 로그 키) ② 맵 전송·해시 대조·재경기(3단계) ③ 경로 완전 차단 대응**이다. 상세는 위 「⚠️ 진행 중 (2026-09-08)」 문단. **[🔴 2026-09-08 갱신 — 원문은 그대로 두고 덧붙인다]** **①(2단계 K)은 커밋 `2e1b305` 로 완료**되어 2단계는 A~K 전부 끝났다. **남은 미구현은 ②와 ③뿐**이며, 상세는 위 「✅ 구현 완료 (2026-09-08)」 문단이다.
  - **이 절이 「확정 설계 — 구현 예정」에 남아 있는 것은 여전히 맞다** — ~~2단계가 K 를 남겨 미완이고~~ **[🔴 2026-09-08 갱신: 2단계는 완료됐다]** **3단계(맵 전송·해시 대조·재경기)가 미착수**이기 때문이다.
  - 종전 표기는 이 항목 전체가 「미구현」이었다. 1단계가 끝나 그 표기가 더는 정확하지 않아 갱신했으나, **이 절이 「확정 설계 — 구현 예정」에 남아 있는 것은 맞다** — 무작위 맵 본체가 아직 없기 때문이다.
...
| 금광 타일 시스템 | ✅ 완료 (2026-09-03 갱신) | `MineKind`(None/Neutral/BlueStart/RedStart), 채굴소 건설 조건, 건물 배치 시 광산 숨김/파괴 시 재표시+타일 중립 복원. **2026-09-03 무작위 맵 1단계로 광산 표시가 전용 저장 필드에서 `MineKind` 로 바뀌었고, 이동 불가 여부는 저장하지 않고 계산한다**(아래 「타일 상태 계약」 행 참조) |
| 타일 상태 계약 (`TileKind`/`MineKind`/`HasBuilding`) | ✅ 완료 · 에디터 실기 검증 완료 (2026-09-03) | 무작위 맵 **1단계**. `IsWalkable` 이 저장 필드에서 **setter 없는 계산 프로퍼티**로 바뀌었다(세 상태로부터 계산). 광산 표시 전용 저장 필드는 삭제(`.cs` 0건), `HexTile` 생성자의 이동 가능 매개변수 제거, 건물 철거 시의 광산 예외 가드 제거. ⚠️ **과대 표기 금지 — 무작위 맵 자체는 미구현이다.** 신설 타입 6종 중 `MapType`·`DecorationDefinition`·`MapDefinition`·`MapDefinitionCodec` **4종은 아직 어느 코드에서도 호출되지 않으며**(2단계가 그 호출부를 만든다), AI 건물 배치 후보 판정과 건설·점령의 전용 조건 전환은 **미착수**이며 **2단계 몫**이다(2026-09-03 재조정) |
...
| 신규 유닛 프리팹 컴포넌트 부착 (32개) | 🔧 스크립트 완료 (2026-06-05) / 실기 테스트 예정 | Human 5종·Spirit 6종·Transcendence 5종 × Blue/Red. `Assets/Editor/Setup/SetupNewUnitPrefabs.cs`(**1회성 — 실행 후 삭제됨**). 후속: Animation Event·UnitFactory 등록·스탯 추가 별도 필요. |
...
| AIConfigSetup.cs Editor 스크립트 | ✅ 완료 (**1회성 — 실행 후 삭제됨**) | `Assets/Editor/` — 메뉴 `Hexiege/Setup/AIConfig 생성` + `AIScenarioConfig_Human_A/B/C 생성` |
...
| 항목 | 상태 | 비고 |
|------|------|------|
| 범용 토스트 UI 시스템 (ToastUI) | ✅ 완료 | DontDestroyOnLoad 독립 Canvas, 큐 방식, 터치 제거, DOTween 페이드아웃 **[🔴 2026-09-08 정정 — 원문은 그대로 두고 덧붙인다: 「큐 방식」은 서로 다른 종류에만 적용된다.** 커밋 `f518997` 로 **같은 종류는 누적하지 않게** 바뀌었다 — 이미 표시 중인 종류가 다시 요청되면 큐에 넣지 않고 **표시 시간만 처음 값으로 되돌리고**(트윈을 먼저 중단하고 알파를 되살린 뒤), 큐에서 대기 중인 종류면 **무시**한다. 빗금 타일만이 아니라 **골드 부족·인구 초과 등 전 토스트 공통**이다. 단일 소스는 `GameSystemRules/GameSystemRules_UI.md` 「생산 패널 UI」 **규칙 25**] |
...
| 수동 생산 실패 토스트 3종 | ✅ 완료 | GoldInsufficient / PopulationFull / ProductionQueueFull **[🔴 2026-09-08 덧붙임 — 이 행의 3종은 「수동 생산 실패」 범위라 그대로 맞다.** 다만 프로젝트 전체 `ToastKey` 는 이후 늘어 위 3종 외에 `UpgradeRequired` · `SkillOnCooldown` · `BuildingNotAllowed` 가 더 있다. **개수는 여기에 적지 않는다**(늘어나면 조용히 낡는다) — **전체 목록의 단일 소스는 `GameSystemRules/GameSystemRules_UI.md` 「생산 패널 UI」 규칙 28** 이다] |

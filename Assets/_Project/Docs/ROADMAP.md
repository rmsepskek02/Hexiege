...
**최종 수정일:** 2026-08-03
**최우선 작업:** 서버 권위 유닛 ActionSequence 전환. 규칙 v2, 25종 감사와 Tracer A0/A1/A2/B0/B1/B2는 완료했다. 다음은 B3 경기 단위 25종 이동·SimulationFacing writer 전환이며, result seam·공격 Impact/피해 시점·권위 전환 후 멀티플레이 전체 검증은 미완료다.
**현재 단계:** B2 pure 이동 reducer와 read-only Shadow observer를 구현하고 Android 1대·Unity Editor counterpart에서 25/25종·Blue/Red 이동 의도/Legacy writer 증거를 확보했다. 현재 구현 게이트는 B3 경기 단위 이동·방향 writer 전환이다.
**작업 이력:** [WORK_HISTORY.md](WORK_HISTORY.md) 참조

> B2는 서버 read-only Shadow와 25/25종 증거 수집까지 완료됐다. 실제 이동·SimulationFacing writer는 아직 Legacy이며, 공격 방향·Impact·피해 시점과 ActionSequence 권위 전환 완료를 뜻하지 않는다. 현재 우선순위와 유닛별 상태는 아래 P0 항목 및 `Assets/_Project/Docs/Assets/UnitCombatAssetMatrix.md`를 따른다.
...
| 우선순위 | 작업 | 카테고리 | 예상 규모 |
|---------|------|---------|---------|
| 🔴 P0 | 서버 권위 Unit ActionSequence 구현 — A0/A1/A2/B0/B1/B2 완료 → B3 경기 단위 writer 전환 → Snapshot/ImpactResult 복제 → FIFO 대체 → 25종 이전 → 멀티 QA → Legacy 제거 | 전투/네트워크 | 특대 |
...
| 🔴 P0 다음 | Tracer B3 경기 단위 25종 이동·SimulationFacing writer 전환 — Legacy/신규 single-writer 모드 고정과 rollback·멀티 검증 필요 | 전투/네트워크 | 대 |

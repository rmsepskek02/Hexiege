...
**코드 작성 완료 (2026-08-17) — 컴파일 통과 확인됨 (2026-08-17) · ⚠️ 실기 동작 테스트 없음:** **로그 체계(`GameLog`) 전환 — 네트워크·인증 계층 정리 task 코드 작업 완료.** 8파일 **205건** 이관(`개발` 120 / `운영` 85)에 이어 남아 있던 **미조치 4항목**을 구현했다(커밋 `4e027e68` · `675203ae`) — ① 민감 데이터 마스킹 **15곳**(UID·PlayerId 해시 / 이메일 출력 제거) ② 에디터 로그 파일명 **`RuntimeLog.txt` 단일화**(역할은 `Role=` 로그 한 줄로) ③ `RuntimeLogger` 헤더 규정 준수(`BeginSession(folderPath, purpose)`) ④ **수동 정리 메뉴** `Hexiege > Logcat > 3. 오래된 에디터 로그 정리`. 감사가 놓쳤던 `LobbyManager` **`HostId` 평문 출력 1건**도 함께 마스킹했다. **⚠️ 잔여:** 두 커밋의 **컴파일 통과는 확인**되었으나(2026-08-17) **실기 동작 테스트 없음**(정리 메뉴는 **실행해 본 적 없음**) · **나머지 계층 이관 미착수**(잔존 **234건** — 별도 task) · `.meta` 취급 규정 명문화 보류. 상세: `LogRules.md` **1.13**, task `_Tasks/2026-08-13/07_13_network-auth-log-cleanup/`.
...
| 우선순위 | 작업 | 카테고리 | 예상 규모 |
|---------|------|---------|---------|
| 🔵 코드 완료 · 검증 대기 (2026-08-17) | **로그 체계(`GameLog`) 전환 — 네트워크·인증 계층** — 8파일 **205건** 이관(`개발` 120 / `운영` 85) + 마스킹 **15곳**(UID·PlayerId 해시 / 이메일 출력 제거) + 에디터 로그 파일명 **`RuntimeLog.txt` 단일화**(역할은 `Role=` 로그 라인) + `RuntimeLogger` 헤더 규정 준수 + **수동 정리 메뉴**(`Hexiege > Logcat > 3. 오래된 에디터 로그 정리`) + 감사 누락 `HostId` 평문 출력 1건 마스킹. 커밋 `668e0aeb` 계열 · `4e027e68` · `675203ae`. **⚠️ 컴파일 통과는 전부 확인됨(2026-08-17) — 다만 실기 동작 테스트가 없다** | 코드 정리/인프라 | 대 |
| 🔴 높음 (미착수) | **로그 체계 전환 — 나머지 계층 이관** — `Assets/_Project/Scripts/` 전체 잔존 `Debug.Log` **234건**(이번 8파일 **밖**). 사용자가 "이번 task 마무리까지"로 범위를 정해 **별도 task 로 분리**했다. `Application/GameLog.cs` **9건은 대상 아님**(sink 폴백 구현 — `LogRules.md` 1.8 이 요구하는 동작). `NetworkCombatController.cs` 잔존 **11건** 포함(그중 하나는 새 역할 로그와 `IsServer` 중복) | 코드 정리 | 대 |
| 🟡 중간 (사용자 확인 대기) | **로그 정리 메뉴 첫 실행 확인** — `Hexiege > Logcat > 3. 오래된 에디터 로그 정리` 는 **한 번도 실행된 적이 없다.** 되돌릴 수 없는 삭제를 수행하므로 첫 실행 시 보존 기간 선택 · 대상 목록 · 삭제 결과를 확인해야 한다. 커밋 `4e027e68` · `675203ae` 의 **컴파일 통과는 확인**되었으나(2026-08-17), 컴파일 통과는 실행 결과를 보장하지 않는다 | QA | 소 |

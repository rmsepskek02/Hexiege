...
**최종 수정일:** 2026-07-23
**설계 확정 / 구현 예정 (2026-07-23):** 연구소 유닛 강화 시스템(공/방/속 + 초월 자연회복) + 전투 스탯 ×10 스케일 개편 **밸런스 최종 확정** — 문서 반영 완료, 코드 구현 예정. 상세: `GameSystemRules/GameSystemRules_Upgrade.md`, `StatsReference.md`, `_Tasks/2026-07-22/10_08_unit-upgrade-system/BalanceReview.md`.
...
| 🔷 설계 확정 / 구현 예정 | 연구소 유닛 강화 시스템 + 전투 스탯 ×10 개편 — 공/방(신규)/속 3스탯 + 초월 자연회복, 종족별 그룹×스탯 트랙, (B) 팀 배율 실시간 소급 적용, 방어력 감쇄(K=120), 연구소 운영(복수 건설·트랙 잠금·파괴 100% 환불·서버 권위), 전투 수치 ×10(TTK 불변). 밸런스 최종 확정(2026-07-23), 문서 반영 완료, 코드 미착수 | 기능/기획 | 대 |
...
### C-2. 연구소 유닛 강화 시스템 + 전투 스탯 ×10 개편 🔷 설계 확정 / 구현 예정 (2026-07-23 밸런스 최종 확정)

연구소(Research 건물)로 팀 단위 유닛을 강화하는 신규 시스템 + 함께 진행하는 전투 수치 ×10 스케일 개편. **밸런스 최종 확정, 문서 반영 완료, 코드 미착수(구현 예정).**

- **전투 스탯 ×10 스케일**: 유닛 HP·공격력, 건물 HP, 타워 HP·공격력, DoT 틱값, 힐량 전부 ×10. HP·공격력 동일 배율로 **TTK 불변**(상성·매치업 불변, 강화 그리드만 규칙적으로 개선). 사거리·이동속도·쿨다운·비용·비율은 불변.
- **강화 스탯**: 공격력 / 방어력(신규, 전 유닛 기본 0) / 이동속도 + 초월 자연회복(3~15 HP/s). 종족별 그룹×스탯 트랙(인간 9·정령 9·초월 7).
- **적용 방식 (B)**: 팀별 연구 레벨 배율을 사용 지점에서 곱함 → 이미 전장에 나온 유닛도 소급 강화. 방어력 감쇄 공식 `Max(1, Round(공격력×(1−방어력/(방어력+120))))`, 모든 데미지 지점 일괄.
- **연구소 운영**: 복수 건설·연구 시간·진행 중 트랙 잠금(중복 차단)·파괴 시 100% 환불·건물 업그레이드 없음·상대 비공개·서버 권위. 표준 트랙 1,000골드(80/120/180/260/360)/15~70초, 그룹 배율(동물 ×2.0·자연회복 ×2.5·그 외 ×1.0).
- **AI 사용**: 방향만 확정(별도 task).
- **구현 범위(미착수)**: Domain 방어력 필드·그룹 매핑, Application `UnitUpgradeUseCase`·데미지 공식 감쇄·(B) 배율·자연회복 HoT, Infrastructure 연구 RPC·서버 동기화, Presentation 연구 패널 UI, AI 시나리오 스텝.
- **문서**: `GameSystemRules/GameSystemRules_Upgrade.md`(구현 계약), `StatsReference.md`(×10 값·업그레이드 표), `GameSystemRules_Units.md` 규칙 44(방어력 감쇄), GDD 연구소 섹션. 밸런스 확정 근거·old/new 대조: `_Tasks/2026-07-22/10_08_unit-upgrade-system/BalanceReview.md`·`Research.md`·`Plan.md`.

...
| 🟢 낮음 (문서/설정 — 2026-08-20 등록) | **`.claude/MEMORY.md`(공용)의 윈도우 절대경로 정리** — **33 · 36 · 44~49행**이 `d:/Dmain/dev/Portfolio/Hexiege/Hexiege/...` 를 가리킨다. 원격 리눅스 세션에 **존재하지 않는 경로**다. `.claude/agents/*.md` 6개는 2026-08-20 에 상대경로로 교체했으나(잔존 0건) **이 공용 파일은 범위 밖이라 손대지 않았다.** 메모리 손실 사고의 원인 ②와 **같은 성질**이므로 정리 대상이다 | 문서 | 소 |

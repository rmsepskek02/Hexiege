...
| 🔴 높음 (**2단계 — 1단계 완료 2026-09-03**) | **FlatTop 11×21 무작위 대전 맵 구현 — 2단계: 맵 생성기 5종 · 검증기 · 폴백.** 1단계(타일 상태 계약 전환)는 **구현·에디터 실기 검증 완료**라 이 표에서 내렸다(이력은 [WORK_HISTORY.md](WORK_HISTORY.md), 상태는 [PROJECT_STATUS.md](PROJECT_STATUS.md)). **남은 2단계 범위:** 유형별 생성 전략 5종, exact 180° 대칭, 광산/초기 골드, 초기 골드 전용 테스트 모드(멀티 Host 권위), 건설 불가·차단 지형 생성, 결정적 seed/검증/폴백. ⚠️ **1단계가 만든 타입 6종 중 `MapType`·`DecorationDefinition`·`MapDefinition`·`MapDefinitionCodec` 4종은 아직 호출부가 0곳**이며, 이 2단계가 그 호출부를 만드는 작업이다 (설계 확정·문서 동기화 2026-07-20) | 기능/맵 | 대 |

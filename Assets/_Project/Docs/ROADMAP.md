...
| 🟡 중간 (미검증) | **전역 훅의 엔진 오류 수집(B) 실기 확인** — 2026-08-19 회차에 엔진 오류가 **나지 않아** `[ERROR]`·`UnhandledEngineError`·`Suppressed=` 가 전부 0건이었다. **훅이 동작한다는 증거가 아직 없다.** 다음에 엔진 오류가 나는 회차에서 로그 파일에 `Event=UnhandledEngineError` 줄이 남는지, `Suppressed=` 가 함께 찍히는지 확인한다 | QA | 소 |
| 🟢 낮음 (범위 밖 — 후속 후보) | **`NetworkCombatController` 의 나머지 2건** — ① `OnUnitDied` → `EntityDiedClientRpc` 가 `if (!IsServer) return;` 하나뿐이라 **`Update` 와 같은 성질의 가드 구멍**이다(코루틴이 Shutdown 이후 깨어날 수 있다) ② **게임 종료 구독이 0건**이라 승패가 갈린 뒤에도 전투 루프가 돈다(2026-08-19 로그 기준 **약 3초**). ①은 A 수정 후에도 같은 오류가 남으면, ②는 `IsSpawned` 가드가 부족하다고 판명되면 착수 | 코드 정리 | 중 |

...
**코드 작성 완료 (2026-08-17) — ⚠️ 컴파일 검증 전 · 실기 동작 테스트 없음:** **로그 체계(`GameLog`) 전환 — 네트워크·인증 계층 정리 task 코드 작업 완료.** 8파일 **205건** 이관(`개발` 120 / `운영` 85)에 이어 남아 있던 **미조치 4항목**을 구현했다(커밋 `4e027e68` · `675203ae`) — ① 민감 데이터 마스킹 **15곳**(UID·PlayerId 해시 / 이메일 출력 제거) ② 에디터 로그 파일명 **`RuntimeLog.txt` 단일화**(역할은 `Role=` 로그 한 줄로) ③ `RuntimeLogger` 헤더 규정 준수(`BeginSession(folderPath, purpose)`) ④ **수동 정리 메뉴** `Hexiege > Logcat > 3. 오래된 에디터 로그 정리`. 감사가 놓쳤던 `LobbyManager` **`HostId` 평문 출력 1건**도 함께 마스킹했다. **⚠️ 잔여:** 두 커밋의 **컴파일 검증 미완**(사용자 확인 대기) · **실기 동작 테스트 없음**(정리 메뉴는 **실행해 본 적 없음**) · **나머지 계층 이관 미착수**(잔존 **234건** — 별도 task) · `.meta` 취급 규정 명문화 보류. 상세: `LogRules.md` **1.13**, task `_Tasks/2026-08-13/07_13_network-auth-log-cleanup/`.
...
| 🟡 중간 (사용자 확인 대기) | **로그 정리 메뉴 첫 실행 확인** — `Hexiege > Logcat > 3. 오래된 에디터 로그 정리` 는 **한 번도 실행된 적이 없다.** 되돌릴 수 없는 삭제를 수행하므로 첫 실행 시 보존 기간 선택 · 대상 목록 · 삭제 결과를 확인해야 한다. 함께 커밋 `4e027e68` · `675203ae` 의 **컴파일 검증**도 필요 | QA | 소 |

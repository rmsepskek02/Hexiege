...
**최우선 작업:** 서버 권위 유닛 ActionSequence 전환. 최신 `main` 기능을 보존한 기준에서 B3 재경로 과잉 종료와 제자리 걷기를 먼저 교정한다. 목표 단위 진행성, stale PendingPath 지연 재요청, 동일 A* 결과의 제한 재평가, 공성 재명령 억제와 서버 정지 표현이 범위다. 이후 역할교대·25종 전체 회귀·Legacy rollback을 재개하며, result seam·공격 방향·Impact/피해 시점도 미완료다.
**현재 단계:** B3는 **FAIL / OPEN**이다. 목표 단위 `WaitingRepath/Blocked` lifecycle, stale PendingPath 재요청, 공성 동일 목표 재명령 억제, `Held` 복제와 중복 `NoIntent` 게시 방지를 구현했고 Unity C# 컴파일은 PASS했다. 다음은 Editor self-validation과 건물 변경/공성 반복, 새 Android Host 1경기다. 기존 21/25종 로그는 정확성 기준선일 뿐 완료 증거로 재사용하지 않는다.
**작업 이력:** [WORK_HISTORY.md](WORK_HISTORY.md) 참조
...
| 🔴 P0 교정 | Tracer B3 연속 이동 — 1차 guard가 동기식 반복은 막았으나 stale/same-path를 과잉 종료해 실기 FAIL. 목표 단위 navigation lifecycle, 공성 동일 목표 재명령 억제, 명시적 Held/terminal 표현을 RED→GREEN으로 교정한 뒤 Android Host·역할교대·25종·Legacy rollback 순으로 재검증 | 전투/네트워크 | 특대 |

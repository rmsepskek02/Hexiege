...
**최우선 작업:** 서버 권위 유닛 ActionSequence 전환. v8 Android의 recoverable 공간 preflight 반복을 v9 typed 후보/preflight와 `WaitingRepath` 경계로 교정했고 코드 게이트를 통과했다. 다음은 Unity 메뉴 self-validation 실제 실행 → 새 Android v9 Host/Editor Client 실기다. 통과 후에만 역할교대·25종 전체 회귀·Legacy rollback을 재개한다. result seam·공격 방향·Impact/피해 시점도 미완료다.
**현재 단계:** B3는 **FAIL / OPEN**이다. observer schema `b3-movement-authority-v9`, Runtime·Editor Roslyn PASS, 독립 정적 QA P0~P2 없음. `planned`/`committed` staged equality와 lifecycle retire 보강은 구현됐지만 Unity self-validation과 Android v9 증거는 없다. Android는 `spatialFinalRecoverableRepaths=0`이 아니라 recoverable repath의 resolved/nonrepeat/no-cleanup 수렴을 검증한다. 기존 v8·21/25종 로그는 완료 증거로 재사용하지 않는다.
**작업 이력:** [WORK_HISTORY.md](WORK_HISTORY.md) 참조
...
| 🔴 P0 교정 | Tracer B3 연속 이동 v9 — v8 Android recoverable preflight 반복을 typed 후보/preflight·동일 probe/final-stage seam·`WaitingRepath`·staged accounting·lifecycle retire로 교정. Runtime/Editor Roslyn PASS, 정적 QA P0~P2 없음. Unity 메뉴 self-validation → Android v9 resolved/nonrepeat/no-cleanup gate → 역할교대·25종·Legacy rollback 순. 현재 FAIL / OPEN | 전투/네트워크 | 특대 |

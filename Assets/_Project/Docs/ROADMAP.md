...
**최종 수정일:** 2026-07-20
**현재 단계:** InfernoSpirit(지옥불 정령) 단일 대상 지속 피해(DoT) 구현 완료(2026-07-20, QA PASS·실기 완료) — 특수 유닛 5종 중 **5번째**. 이미 완성된 원거리 유닛에 DoT 특수만 추가: 주 타깃 1마리 직접 25(기존 단일 피해 `ReplacesPrimaryAttack=false`, 건물 공성) + 그 주 타깃 1마리에게만 DoT 5/초×3초(총 15, AoE 아님·반경 없음, 적 유닛만). MushroomBomber의 규칙 40 초 단위 틱을 단일 대상으로 재사용, 값은 별도 진입점(`ApplyInfernoDot` 5/3)으로 분리(MushroomBomber 2/3 회귀 없음). 규칙 41~42. 직전 MushroomBomber(버섯폭격기) 착탄형 범위 DoT 딜러 구현 완료(2026-07-19) — 5종 중 4번째. 착탄 중심 월드 원형 반경 내 적 유닛 DoT(초 단위 discrete 틱 모드 신설, 규칙 34 확장) + 주 타깃 직접 10. **잔여 특수 유닛(레지스트리 확인 결과 QuakeSpirit이 유일 미구현 — BattleAxe/TorrentSpirit/MushroomBomber/InfernoSpirit 등록됨, BloomFairy는 힐러 전용 경로로 의도적 미등록)**: QuakeSpirit(착탄형 — MushroomBomber 원형 반경 헬퍼 재사용 예정). **알려진 이슈(보류)**: 멀티 원거리 공격 facing 버그(에셋 아닌 공유 회전 로직 추정, 사용자 결정으로 별도 작업 보류). 후속(Phase F): 기기 QA로 3D 텍스처 품질 확인, 피격 VFX 프리셋 연결, 잔여 특수 타격(QuakeSpirit) 클립 이벤트, 멀티 원거리 facing 버그 수정, Firebase/EDM 저장소 방침 정리, AI 반응 시스템·시나리오 정밀 검증 + 잔여 신규 유닛 프리팹 실기 테스트
...
| ✅ 완료 | InfernoSpirit(지옥불 정령) 단일 대상 DoT — 특수 유닛 5종 중 5번째. 이미 완성된 원거리 유닛에 DoT 특수만 추가: 주 타깃 1마리 직접 25(기존 단일 피해 `ReplacesPrimaryAttack=false`, 건물 공성) + 그 주 타깃 1마리에게만 DoT 5/초×3초(총 15, AoE 아님·반경 없음, 적 유닛만). 규칙 40 초 단위 틱을 단일 대상으로 재사용, 값은 별도 진입점(`ApplyInfernoDot` 5/3)으로 분리(MushroomBomber 2/3 회귀 없음). `SpecialAttackConfig` inferno DoT 필드 + GameBootstrapper 주입. 레지스트리 1줄. 규칙 41~42. 공격 방향(facing) 버그는 사용자 결정으로 보류(별도 작업). QA PASS·실기 완료 (2026-07-20) | 기능 | 소 |
| 🟡 중간 | 피격 VFX 프리셋 연결 — `UnitEffectConfig.hitPreset` Inspector 배선(Human 6종 등 미연결) | 에셋 | 소 |
| 🟢 낮음 | 잔여 특수 유닛 1종(QuakeSpirit) 구현 시 Attack 클립 `OnAttackHit` 이벤트 주입 + 특수 공격 핸들러 확장. QuakeSpirit=착탄형(MushroomBomber 원형 반경 헬퍼 재사용). (BattleAxe·TorrentSpirit 2026-07-17, BloomFairy 2026-07-18, MushroomBomber 2026-07-19, InfernoSpirit 2026-07-20 완료) | 기능/에셋 | 중 |
...
**진행 (2026-07-20)**: 특수 유닛 5종 중 **전 5종 구현 완료** — BattleAxe(휩쓸기형 부채꼴, 2026-07-17), TorrentSpirit(파도형 이동 AoE + 힐 서브시스템, 2026-07-17), BloomFairy(힐러 전용 경로 + HoT/DoT 공용 시스템, 2026-07-18), MushroomBomber(착탄형 원형 반경 DoT + DoT 초 단위 틱 모드 + 식물 라인 생산 배선, 2026-07-19), InfernoSpirit(단일 대상 DoT — 규칙 40 초 단위 틱 재사용, 값 별도 진입점 분리, 2026-07-20). 특수 공격 전략 핸들러 아키텍처(핸들러 + 레지스트리 1줄) 확립. **잔여**: QuakeSpirit(착탄형) — 레지스트리 확인 결과 유일 미등록(미구현). BloomFairy는 힐러 전용 경로로 의도적 미등록. 상세: `PROJECT_STATUS.md` / `WORK_HISTORY.md`, task `_Tasks/2026-07-16/18_06_battleaxe-aoe/` · `_Tasks/2026-07-17/12_59_torrentspirit-wave-aoe/` · `_Tasks/2026-07-18/03_40_bloomfairy-healer/` · `_Tasks/2026-07-19/01_42_mushroombomber-impact-dot/` · `_Tasks/2026-07-20/03_22_infernospirit-dot-and-attack-facing/`.
...
- **남은 대상**: QuakeSpirit (레지스트리 확인 결과 유일 미등록 = 미구현. BloomFairy는 힐러 전용 경로로 의도적 미등록).
- **작업**: QuakeSpirit 구현 시점에 Attack 클립 `OnAttackHit` 이벤트를 주입(규칙 17·27). 특수 공격 로직은 규칙 23~42의 전략 핸들러 구조 위에 핸들러 + 레지스트리 1줄로 확장(QuakeSpirit=착탄형 원형 반경 헬퍼 재사용).

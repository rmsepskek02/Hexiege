...
| 🔴 높음 | 스킬 타입 C(전역 상태변경: 버프/디버프/CC/힐) Phase 2 — `SkillMechanicType.GlobalStatusChange`는 enum만 선언·실행기 미구현(`SkillExecutorRegistry` TODO). 전역 즉시 발동(조준 없음), HoT 재사용 + 신규 상태변경 시스템. 업그레이드 연동(규칙 9-x)도 함께 확정 | 기능 | 대 |
| 🟡 중간 | 건물 파괴 시 패널/조준 UI 원복 — 스킬 포함 4개 건물 패널 공통 갭(파괴돼도 열린 패널·조준 락 잔존). `BuildingPanelBase`에서 `OnBuildingDied` 구독→Close 방식 제안, 미착수 | QA/버그 | 소 |
...
- 스킬 건물(FlightFacility / MagicSpirit / WillowShrine): **Phase 1 구현 완료 · 실기기 테스트 PASS (2026-08-04)**. **마나 미도입** — 자원 없이 건물별 글로벌 쿨다운만 사용. 건물당 최대 5개 스킬, 3×3 패널 UI(슬롯 1~5 스킬/6 철거/7~9 예약), 모바일 탭 조준(2단계·엣지 스크롤·취소 X Lerp 확대), 서버 권위. **타입 A(즉발 범위 피해)·B(장판 DoT) 실행기 완료**, 조준 지점 연속 좌표화(HexCoord→Vector3, 반경 판정 무변경·중심만 연속화)·맵 경계 point-in-bounds 재검증·조준원 지면 데칼 렌더링(`SkillAimOverlay.shader`)·취소 버그 수정·쿨다운 안내 토스트까지 반영. **미완: 타입 C(전역 상태변경) Phase 2(enum만 선언·실행기 미구현)·건물 파괴 시 UI 원복·구체 스킬 목록/수치(기획).** 상세: [GameSystemRules_Skills.md](GameSystemRules/GameSystemRules_Skills.md)(규칙 1~26), task `_Tasks/2026-07-28/12_14_skill-building-system-design/`·`_Tasks/2026-08-04/04_46_skill-aim-coordinate-based/`.

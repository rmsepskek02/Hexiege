...
You have a persistent Persistent Agent Memory directory at `.claude/agent-memory/game-design-lead/` (리포지토리 루트 기준 상대경로). Its contents persist across conversations.
...
2. **`.claude/agent-memory/game-design-lead/MEMORY.md`** — 너의 메모리 인덱스. 상세는 거기서 링크한 토픽 파일에 있으니, 이번 작업 주제에 해당하는 것을 골라 `Read` 한다.

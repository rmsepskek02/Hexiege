...
## 공통 중요 교훈
- **경기 종료 후 UI·프로세스 단계 6(결과 화면 무반응 이탈 자체 감시) — 설계 확정 3건 / 🔴 구현 0줄 (2026-09-21, 문서만)** — ① 🔴 **자체 감시는 Host 와 Client 가 각자 돌린다. 서버 전용 금지** — 서버가 판정해 RPC 로 알리면 **Host 가 사라졌을 때 남은 Client 는 통보를 받을 길이 함께 사라진다**(「🔴 최상위 원칙」 위반). ✅ **정상 퇴장 통보 RPC 는 그대로 둔다 — 자체 감시는 통보가 오지 못한 경우를 받는 그물이며 대체 관계가 아니다.** ② 🔴 **판정 전에 「내 인터넷이 되는가」를 먼저 확인하고, 그 도달 확인은 단계 6 에서 만든다**(전적 기록 작업이 나중에 재사용). 근거는 **규칙 17 이 「이탈로 판정하면 연결도 함께 종료한다」고 정하고 있어 오판이 곧 「돌아올 수 있었던 연결을 스스로 끊는 것」**이기 때문이다. ③ ⚠️ **「응답」의 측정 수단은 미확정** — **1순위 기존 RTT(`NetworkGameManager.GetCurrentRttMs()`, 현재 읽는 곳은 `NetworkStatusUI` 핑 표시 1곳) · 2순위 결과 화면 전용 하트비트**라는 **순서만** 확정됐고 🔴 **RTT 로 「갱신이 멈췄다」를 판별할 수 있는지는 확인되지 않았다**(확인 시점 = 단계 6 착수 시). ⚠️ **기존 Plan 의 단계 6 완료 판정 한 줄(**브로드캐스트 `ClientRpc`**)이 「서버에서 돈다」를 전제하고 있어 어긋났다 — 지우지 않고 마커를 덧붙였다.** 단일 소스: 규칙은 `GameSystemRules/GameSystemRules_RandomMap.md` 규칙 17 「2026-09-21 추가 (2차)」 · 구조는 `Assets/_Project/Docs/TechnicalDesignDocument.md` 「결과 화면 이탈 판정·통보 구조」의 2026-09-21 정정 블록과 「이탈 판정 절차」 · 계획은 `_Tasks/2026-09-16/06_27_post-game-leave-ui/Plan.md` §2-1.
- **경기 종료 후 UI·프로세스 단계 1·2·3 + 프리팹 구성 — 구현 완료 / ⚠️ 알림 팝업은 화면에 띄워 본 적 없음 (2026-09-21, 커밋 `d881d23`·`b336d9c`·`a87e404`·`1eba997`)** — 결과 화면에서 **재경기를 눌러도 로비 버튼이 꺼지지 않게** 됐고(규칙 D-3), 자동 로비 복귀가 **60초**가 됐으며(규칙 D-4의 첫 행), **「제목 + 본문 + 버튼 1개」 알림 팝업을 띄울 수단**이 생겼다(규칙 D-5 — 신설이 아니라 **기존 `ConfirmPopup` 확장**, 사용자 승인). ⚠️ **교훈 3가지**: ① 🔴 **코드에만 있는 「프리팹 전제 조건」은 문서가 아니다** — `ApplyTitle()` 이 제목을 `SetActive(false)` 로 숨길 때 **그 자리가 레이아웃에서 사라지려면 프리팹 Panel 에 VerticalLayoutGroup 이 있어야** 하고, 없으면 **제목 없는 기존 팝업의 본문이 아래로 밀린다(= 회귀).** 즉 **코드가 완성돼도 규칙이 성립하지 않는 상태**가 존재하므로 그 전제를 **상시 참조 문서에도** 적는다(규칙 D-5 아래 2026-09-21 블록 · `.claude/agent-memory/game-programmer/ui-system.md`). ② **「직렬화 값이 코드 기본값보다 우선」은 이번에도 두 자리를 강제했다** — `_autoReturnSeconds` 는 코드 `30f→60f` **와** `Game.unity` `30→60` 을 **둘 다** 고쳐야 실제 동작이 60초가 된다. 한쪽만 고치면 **바꿨다고 착각한 채 다음 단계가 얹힌다.** ③ 🔴 **과대 표기 금지 — `UIManager.ShowAlert` 의 실호출처가 0건이라 이 팝업이 뜨는 모습은 아무도 본 적이 없다.** 띄우는 자리는 단계 8(규칙 D-6)·단계 10(규칙 M-3·M-4)이며 둘 다 미착수다. 확인된 것은 **기존 확인/취소 팝업 2건의 회귀 없음**뿐이고 3번째(`ProfileView`)는 **미확인(구조 동일이라는 판정)**이다. ⚠️ **남은 것**: 단계 4~10(전송 계층 타임아웃 60000 두 씬 · 이탈 채널·통보 · 무반응 30초 자체 감시 · 결과 화면 반영 · D-6 · 전송 중 이탈 문구 분기 · 맵 실패 팝업) · **14-4 강제 실패 플래그**(없으면 단계 9·10 화면이 정상 경로에서 관측되지 않는다). 🔴 **사용자 판단 대기 2건**(규칙 5 예외 명문화 여부 · 1회성 에디터 스크립트 삭제/이동)은 **정하지 말 것** — 단일 소스는 `Assets/_Project/Docs/ROADMAP.md` 의 2026-09-21 신설 행 2개. 검증 경계의 단일 소스는 `Assets/_Project/Docs/PROJECT_STATUS.md` 「⚠️ 부분 구현 (2026-09-21)」, 계획·실행 기록은 `_Tasks/2026-09-16/06_27_post-game-leave-ui/Plan.md` §10·§11. **[🔴 2026-09-22 갱신 — 위 문장은 2026-09-21 시점의 기록이라 한 글자도 지우지 않고 덧붙인다(B-7).]** 🔴 **「남은 것: 단계 4~10」과 「`UIManager.ShowAlert` 실호출처 0건 · 띄우는 자리(단계 8·10) 둘 다 미착수」는 더 이상 참이 아니다 — 단계 4~8 이 끝났고 실호출처는 1건이다**(단계 8 = 커밋 `500bbc1`, 규칙 D-6). ⚠️ **그래도 「아무도 본 적이 없다」는 결론은 살아 있다 — 단계 8 은 실기 검증 0건**이며, 재현 조건은 **한쪽이 재경기 버튼을 누른 직후 그 누른 쪽 앱을 강제 종료**하는 것이고 **역할(Host/Client)은 무관**하다(`ForceWin` 수정이 요구하는 「에디터가 Host」 구성과 **다른 조건**이라 한 테스트로 둘이 함께 검증되지 않는다). ⚠️ **지금 남은 것은 단계 9·10 과 14-4 플래그**이며 **단계별 상태의 단일 소스는 위 Plan 의 §10 표**, 단계 8 상세는 같은 문서 **§12**, 검증 경계는 `PROJECT_STATUS.md` 「⚠️ 부분 구현 (2026-09-22)」 문단이다. ✅ **단계 8 은 씬·인스펙터 작업이 0건이었다** — 팝업 참조를 직렬화 필드가 아니라 **런타임 1회 탐색·캐시**로 두면 배선 작업이 사라진다(단계 1 이 프리팹 작업을 불러온 것과 대비되는 선택지).
- **맵 테스트 모드 삭제 + 재경기 랜덤 맵 — 구현·실기 검증 완료 / ⚠️ 실패 경로는 실기 미검증 (2026-09-15, 커밋 `0608f85`·`8c83317`·`20ba0ab`)** — 재경기가 매 판 새 맵을 뽑게 됐고(멀티 재경기 **7회 연속 PASS**, 싱글 2판 PASS), 쓰지 않던 맵 테스트 모드를 코드·에셋·문서에서 전부 걷어냈다. **`MapVersion` 1→2 · canonical 바이트 수 공식 `315 + 4N`.** ⚠️ **교훈 4가지**: ① **직렬화 형식 안에 든 필드의 삭제는 삭제가 아니라 형식 변경**이다 — 바이트 하나를 빼면 **해시 전부가 달라지고 옛 형식 바이너리가 못 읽히며 이미 끝난 실기 검증이 무효가 된다.** 그리고 중간 상태가 없어 **WORKFLOW [4] 의 「주석 비활성화 우선」을 지킬 수 없으므로** 되돌리기 수단을 **커밋 분할**로 대체한다. ② 🔴 **「에디터가 필요하다」고 사용자에게 넘기기 전에 그 타입이 정말 `UnityEngine` 을 건드리는지 확인한다** — 폴백 템플릿 재생성은 계획서에 「사용자 작업」으로 적혀 있었으나 `MapFallbackTemplateFactory` 가 순수 C# 이라 **mono 헤드리스로 끝났다**(500/500 왕복 PASS). ③ **경기 회차를 가르려고 로그 키를 늘리지 않는다** — 최초 경기와 재경기는 같은 프로토콜의 같은 사건이라 키를 나누면 **실패율 집계가 두 벌로 쪼개진다.** 필드(`Round=`) 하나로 가르되 **호출부가 아니라 데이터 조립 메서드 본문에 넣어야** 한 자리도 빠뜨리지 않는다(`LogEvent` 46개 무변경). ④ 🔴 **과대 표기 금지 — 11판 전부 성공이라 실패 경로·폴백 템플릿·timeout/재전송이 한 번도 발화하지 않았다.** 하네스(`mcs`/`mono`)가 상태 기계 불변식 9가지를 확인했을 뿐 **실기가 확인한 것은 정상 경로뿐**이다. 발화시키려면 임시 조작 코드가 필요한데 **사용자 승인 사항**이다. ⚠️ **남은 것**: ~~`NetworkGameEndController.cs` 의 `[재경기맵 대체 대기]` 주석 마커 **2건**(제거 조건은 충족, 아직 코드에 있음)~~ **[🔴 2026-09-15 정정 — 이 항목만 취소선 처리했다. 「남은 것」으로서 **항목 전체가 무효**가 됐기 때문이다(`.claude/MEMORY.md` B-7). 원문은 지우지 않고 그대로 둔다: 마커 2건은 **제거됐고 잔존 0건**이다(`grep -rc "재경기맵 대체 대기" Assets/_Project/Scripts --include=*.cs` = 0). **삭제된 것은 주석 9줄뿐이고 동작 코드 diff 는 0줄**이다 — `git diff --numstat` = **0 추가 / 9 삭제**, 9줄 전부 `//` 로 시작하는 주석(두 상호 동의 분기의 마커 주석 4줄·3줄 + 비활성화돼 있던 `// StartRematch();` 각 1줄)이고 `GameLog.Dev.Info(...)` 2건 · `BeginRematchMapPreparation()` 호출 2건 · `StartRematch()` 본문과 디스폰 루프는 무변경이다. 근거 규정은 `Assets/_Project/Docs/WORKFLOW.md` [4](주석 비활성화된 기존 로직의 최종 삭제는 사용자 테스트 통과 후·문서 업데이트 전)이며 2026-09-15 실기 통과로 충족됐다. 🔴 **한계 — Unity 에디터 컴파일은 확인되지 않았고**(이 파일은 `Unity.Netcode` · `UnityEngine` 의존이라 헤드리스 빌드 불가) **제거 후 재경기 실기 재검증도 하지 않았다.** 「주석만 지웠으니 동작이 바뀔 수 없다」는 **판단이지 실행으로 확인한 사실이 아니다** — 그래서 `ROADMAP.md` 의 행은 내리지 않고 그 두 확인으로 좁혔다.]** · 규칙 M-3 의 「countdown 전체 길이 재시작」 미구현 · 맵이 정한 초기 골드 여전히 미적용(**화면의 5000 은 버그가 아니라 테스트 개발값, 출시 예정 500**). 상세는 `.claude/agent-memory/game-programmer/network-infra.md`(T1~T3 · 재경기 맵 A~D) · 규칙 `GameSystemRules/GameSystemRules_RandomMap.md` 규칙 3·14·16 · 검증 경계의 단일 소스는 `Assets/_Project/Docs/PROJECT_STATUS.md` 「✅ 구현 완료 (2026-09-15)」.

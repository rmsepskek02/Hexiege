...
4. **200행 이내 인덱스로 유지** — 초과분은 프롬프트에서 잘려 **에이전트에게 보이지 않는다.** 상세는 토픽 파일로 빼되 **반드시 인덱스에서 링크한다.**
5. **링크 없는 토픽 파일은 존재하지 않는 것과 같다.**
6. 토픽으로 옮길 때는 **에이전트 폴더 전체 행수 합이 줄지 않아야 한다** — 이동과 삭제를 구분하는 유일한 검증법.

> **실측 사고 기록:** 2026-08-17 `675203ae` game-programmer **-378행**(미복구) · 2026-08-20 `bcf45ec1` 같은 파일 **-18행**(`405538c7` 복원) · 200행 초과 3개(qa-tester 502 · project-orchestrator 325 · game-design-lead 254) · game-programmer 고아 토픽 **16개 1,839행**. 손실 경로가 ①덮어쓰기 ②200행 절삭 ③고아 토픽으로 **서로 다르므로 6개 항목을 모두 지킨다.**

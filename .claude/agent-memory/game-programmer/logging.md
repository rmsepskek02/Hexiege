...
## 맵 전송 로그 — `LogEvent` 41 → **46** (2026-09-14 · 3단계 E)

무작위 맵 3단계가 규칙 16(멀티 맵 전송)의 실패를 관측 가능하게 만들면서 키 5개가 늘었다.
**선언만 했고 호출부는 0건이다** — 실제 호출은 F~H 단계에서 붙는다.

| 키 | 축 A | 축 B | 결말인가 | 조치 |
|---|:-:|:-:|:-:|---|
| `MapTransferSucceeded` | Info | 운영 | ✅ | 없음(정상). 아래 실패 3종의 **분모** |
| `MapTransferRetried` | Warn | 운영 | ❌ **중간 상태 전이** | 조각 크기·timeout 재검토 |
| `MapTransferFailed` | Error | 운영 | ✅ | 전송 경로·용량 한도 |
| `MapHashMismatch` | Error | 운영 | ✅ | 직렬화·해시 계약 (재전송은 규칙 16 이 금지) |
| `MapClientVerificationFailed` | Error | 운영 | ✅ | 생성기·검증기 (Domain/Map) |

- 🔴 **[I] 그룹(맵 준비 4종)과 성질이 다르다.** 그쪽은 넷 다 결말이라 한 판에 줄이 **하나**지만,
  이쪽은 `MapTransferRetried` 가 결말이 아니라서 한 판에 **「재전송 1줄 + 결말 1줄」 최대 2줄**이 남는다.
  이것은 1.14 금지 9 위반이 **아니다** — 금지 9 가 막는 것은 *같은 사건을 두 계층에서* 적는 것이고,
  여기 둘은 시점도 사건도 다르다. 오히려 금지 8("상태 **전이** 시점에만 남긴다")이 요구하는 자리다.
  ⚠️ Plan §7-E 의 검증 항목은 다섯 키가 전부 배타적인 것처럼 적혀 있다 — **4+1 이 맞다.**
- **축 B ② 의 근거를 「실패 UI 가 아직 없다」로 잡지 않았다.** 그 근거는 UI 가 생기는 순간 거짓이 된다.
  대신 규칙 16 의 *"seed·유형·내부 error code는 UI에 노출하지 않고 로그에만 남긴다"* 를 근거로 삼았다 —
  UI 가 생겨도 화면에는 한 문장만 뜨고 원인은 로그에만 남으므로 판정이 바뀌지 않는다
  (LogRules 1.3 원칙 2 의 단서 "원인이 그 로그에만 있으면 개발로 내리지 않는다").
- **왜 `RetryCount=` 필드가 아니라 키인가**: 결말 줄의 필드로만 두면 *"결말 줄이 반드시 남는다"* 는
  전제에 기대는데, 규칙 16 이 「연결 끊김은 즉시 실패」라 상대가 사라지면 결말 줄이 안 남을 수 있다.
- `MapTransferSucceeded` 를 `MapPreparationSucceeded` 로 갈음하지 않은 이유: 준비는 **싱글에서도** 남는
  Host 로컬 사건이고 전송은 **건너간** 사건이다. 묶으면 「맵은 만들었는데 못 보낸 판」이 성공으로 집계된다.

### 이때 확인한 것 (추정 아님)

- `ILogSink.cs` 는 이번에도 **`mcs -langversion:latest -target:library` 단독 컴파일 통과**(종료 코드 0).
- enum 본문을 파이썬으로 파싱해 **46개 실측**(중복 0). 키는 전부 **맨 뒤에** 붙였다.
- ⚠️ `LogRules.md` **1.5 에 신설 블록을 추가하지 않았다** — 호출 세션이 "규칙 문서는 손대지 말라"고
  명시 지시했기 때문이다. Plan §7-E 는 그 갱신을 E 단계 범위로 적고 있어 **문서 갱신이 미이행 상태로 남아 있다.**

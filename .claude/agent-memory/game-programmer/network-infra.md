...
> **[🔴 2026-09-15 정정 — 위 도식은 3단계 I 시점의 배선이라 그대로 두고 덧붙인다]**
> 🔴 **마지막 줄의 두 번째 인자 `GameConfig.MapTestModeEnabled` 는 사라졌다**(커밋 `8c83317` — 맵 테스트 모드
> 삭제). 그 덕에 `NetworkGameManager.ReadMapTestModeEnabled()` 도 함께 없어져 **조합 루트 마찰 2건 중 신규
> 1건이 자동 해소**됐다(기존 1건 — *"로비에는 `GameBootstrapper` 가 없다"* — 은 그대로 남는다).
> 🔴 **`BeginHostMapTransfer` 는 이제 회차 종류(`MapTransferRoundKind`)를 받는다**(커밋 `20ba0ab`).
> **`BeginMapTransferAndLoadGameScene()` 의 이름·시그니처는 무변경**이라 이 도식의 나머지 줄과 호출부
> `BattleViewModel.OnClientConnected` 는 **전부 그대로 유효하다.** 재경기 쪽 배선은 아래
> 「재경기 맵 A~D」의 도식이 단일 소스다.
...
## Map test mode deletion T1~T3 — deleting a field that lives inside a serialized format (2026-09-15)

> Commits: `0608f85` (T1, docs only) · `8c83317` (T2·T3, code + asset + regenerated templates).
> This ran **before** the rematch-map work below, and it is the reason `MapVersion` is now `2`.

**What actually made this hard**: `TestModeFlag` was a fixed-width int **inside the canonical byte
stream** (`Domain/Map/MapDefinitionCodec.cs`). Removing it shifts every later byte forward, so it is
not a deletion — it is a **format change**. The cascade is fixed and worth memorising:

```
canonical byte field removed
  → every following byte shifts        → every SHA-256 changes
  → MapVersion must be bumped (1 → 2)  → old-format .bytes assets are unreadable
  → regenerate the 5 fallback templates
  → 🔴 the transfer 실기 verification that was already finished becomes void → re-verify
```

- 🔴 **WORKFLOW [4]'s "comment it out first" default cannot be obeyed here.** A serialized format has
  **no half state** — you cannot have the flag both in and out of the byte stream. The revert
  mechanism has to be replaced by **commit splitting** (T1 docs / T2·T3 code) instead.
- **Scope, measured**: 12 code files + `Resources/Config/GameConfig.asset` (the `_mapTestModeEnabled`
  and `_testStartingGold` lines were deleted). 🔴 **`_startingGold: 5000` was NOT touched** — it is
  `Economy.StartingGold`, a **different field whose value merely coincided**. That coincidence is why
  turning test mode on never changed anything on screen, and why the feature rotted unnoticed.
  Deleting one of two coincidentally-equal values creates the mirror-image misreading, so the record
  must **name the survivor** every time.
- **New byte-count formula: `315 + 4N`** (N = neutral mine count), exactly 4 bytes less than before.
  This is a **formula, not a size** — it turns "the file is N bytes" into an assertion you can check
  per match, and the device logs did match it on all 9 multiplayer rounds.
- **Regenerated templates (measured)**: Canyon **335 → 331**; FullyOpen · ObstacleOpen · Outer ·
  ThreeLane **343 → 339** each. `_Reference/MapTemplateSource/*.md` (5 files) are **tool output**, not
  hand-written docs, and were regenerated by the same run.
- 🔴 **The regeneration did not need Unity.** The plan had it down as "user work in the editor", but
  `MapFallbackTemplateFactory` is **pure C#**, so the main session ran it headless under `mono`.
  **Before handing a step to the user because it "needs the editor", check whether the type actually
  touches `UnityEngine`** — this project already keeps `Domain/Map/**` buildable with `mcs`/`mono`
  (see [hex-grid.md](hex-grid.md)).
- **What mono could verify**: validator self-check PASS · fallback factory self-check PASS ·
  **500/500 generate→encode→decode round trips PASS** · **500/500 client-side D-method restore PASS**,
  all at `MapVersion=2`.
- ⚠️ **What mono could NOT verify**: everything touching `UnityEngine`, and the `GameConfig.asset`
  edit. Those were confirmed **only by the device run** — keep the two kinds of evidence separate.
- **Docs went first, on purpose.** Deleting the code first would create "a rule that demands a feature
  the code no longer has", and `Tools/check_docs.py` **cannot see that class of drift** (a rule with
  zero references is still a syntactically fine rule).
...
- `AcceptRematchServerRpc` 와 `RequestRematchServerRpc` 의 **상호 동의 분기 두 곳을 반드시 함께**
  ...
  **[🔴 2026-09-15 correction — the line above is kept as written: those 2 markers were **removed
  later the same day; `grep -rc "재경기맵 대체 대기" Assets/_Project/Scripts --include=*.cs` is now **0**.
  What was deleted is **9 comment lines only, with 0 lines of behavioural diff** (`git diff --numstat`
  = **0 added / 9 deleted**, every one starting with `//`): the marker comments in the two branches
  (4 + 3 lines) plus the disabled `// StartRematch();` in each. `GameLog.Dev.Info(...)` ×2,
  `BeginRematchMapPreparation()` ×2 and the whole `StartRematch()` body incl. the despawn loop are
  unchanged. Basis: `Assets/_Project/Docs/WORKFLOW.md` [4] (delete the commented-out original after
  the user's test passes, before the doc update) — the 2026-09-15 device test met that condition.
  🔴 **Limits — the Unity editor compile was never confirmed** (this file depends on `Unity.Netcode` ·
  `UnityEngine`, so it cannot be built headless) **and no rematch was re-run after the removal.**
  "Only comments were deleted, so behaviour cannot change" is a **judgement, not an executed check**.]**
...
**[🔴 2026-09-15 added — the lines above are kept as written; this block records what the user's device test came back with.]**

- ✅ **The build shipped and ran. Multiplayer 9 matches, all PASS** (2 first-match + **7 consecutive
  rematches**), editor + SM-S901N. All 9: `HashMatch=True` · `ResendCount=0` · `ErrorCode=None` ·
  `UsedFallback=False` · `MapVersion=2` · `ChunkCount=1`. **9 distinct seeds, 9 distinct hashes.**
  Byte counts matched **`315 + 4N`** (N = neutral mine count) on all 9 — see the T1~T3 section above
  for where that formula comes from.
- ✅ **`Round=` landed in all three places it was meant to**: host "맵 전송 회차 시작", client
  "시작 통보 수신", and **both sides'** `MapTransferSucceeded`. This is the direct evidence that
  putting the field in `BuildTransferLogData` (rather than at the call sites) covered every site.
- ✅ **The despawn-loop bet paid off.** Spawning the transfer object on top of the result screen and
  letting `StartRematch()`'s generic dynamic-spawn despawn loop clean it up worked for 7 rematches in
  a row — **no exception carved into that loop, and none needed.**
- ✅ **Team/race survived every rematch** (editor as client: `Team=Red, BlueRace=0, RedRace=1` on all 8).
- ✅ **Cost of the inserted step**: map prep `ElapsedMs=0~3`, spawn → transfer complete **99~134ms**.
  `[ERROR]` **0** across the whole new-build window on both sides.
- ✅ **Singleplayer did not regress**: 2 matches, `[Map/GameBootstrapper]` local path, `MapVersion=2`,
  `AttemptCount=1`, `UsedFallback=False`, distinct hashes, `[ERROR]`/`[WARN]` **0**.
- 🔴 **Still unverified on device, and it must not be written up as done** — the three paths listed
  above never fired: **failure-path state restore · fallback template · timeout/resend.** 11/11
  matches succeeded. The harness proved the state machine; **the device proved only the happy path.**
- ⚠️ **Same map archetype came up back to back twice** (#3·#4 Canyon, #7·#8 ObstacleOpen). Different
  seed, mine count and hash each time, so they are **different maps**. The user has ruled that
  randomness producing a repeat is **not a defect and no avoidance check is to be added** — do not
  "fix" this.
- 🔴 **`[재경기맵 대체 대기]` (2 hits in `NetworkGameEndController.cs`) is still there.** WORKFLOW [4]
  says the commented-out original is deleted **after the user's test passes and before the doc update**;
  the test passed on 2026-09-15 but the doc round touched no code. It is a **leftover chore, not a
  defect** — a roadmap row now tracks it. Verify with `grep -c "재경기맵 대체 대기"` → must reach **0**.
  **[🔴 2026-09-15 correction — the lines above are kept as written: **the chore is done.** Later the
  same day the 2 markers were deleted, so `grep -rc "재경기맵 대체 대기" Assets/_Project/Scripts
  --include=*.cs` now reads **0** and the verification above has reached its target. What went out is
  **9 comment lines only — `git diff --numstat` = 0 added / 9 deleted**, every deleted line starting
  with `//`: the marker comments in both branches (4 + 3) plus the disabled `// StartRematch();` in
  each. **0 lines of behavioural diff**; `GameLog.Dev.Info(...)` ×2, `BeginRematchMapPreparation()` ×2
  and the whole `StartRematch()` body incl. the despawn loop are untouched. Ground for the deletion is
  WORKFLOW [4] itself, whose condition (user test passed) was met on 2026-09-15.
  🔴 **Not yet confirmed: the Unity editor compile.** This file depends on `Unity.Netcode` /
  `UnityEngine`, so it cannot be built headlessly — "comments only, therefore behaviour cannot change"
  is a **judgement from the diff, not something that was executed**. 🔴 **Nor was the rematch re-run on
  a device after the deletion.** Do not read "removal complete" as "recompiled and re-verified".]**

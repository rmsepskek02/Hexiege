...
### 연구소 유닛 강화 시스템 Phase 1 — 전투 데이터·공식 기반 (2026-07-25) — 코드 완료(Unity 컴파일/실기 미검증)
- **범위**: 방어력 필드 + 데미지 감쇄 공식 + Tank/CannonCart 건물 2배 + ×10 스케일 config 재조정. 연구 UseCase·팀 배율·자연회복·네트워크·UI·AI는 **Phase 2**(미구현). Phase 1은 방어력·공격력 항상 0/기본.
...
- **×10 config**: 코드 폴백 default 변경(`SpecialAttackConfig` blast2→20/inferno5→50/bloom20→200/wave10→100, GameBootstrapper.Setup 폴백 리터럴 동일). **실제 .asset은 Editor 스크립트로 재설정 필수**(Inspector 우선 교훈). `Assets/_Project/Scripts/Editor/ApplyX10CombatScale.cs` 메뉴 `Hexiege/Balance/Apply x10 Scale`: UnitStatsConfig maxHp·attackPower ×10, BuildingStatsConfig 종족 HP·공격력 6필드 ×10(곱셈), SpecialAttackConfig DoT/힐 4필드 **명시 set**(멱등). **가드**: 확인 다이얼로그+EditorPrefs 재실행 경고+unit0 HP≥200 heuristic. ⚠️Unit/Building은 곱셈이라 2회 실행=×100. **사용자가 1회 실행해야 반영**. 방어력/사거리/이동/쿨/골드/비율/반경은 불변.

...
- [network-infra.md](network-infra.md) — NGO 컨트롤러 구조, 스폰 레이스, **종료(Shutdown) 시점 뒷정리 관례 +
  ...
  **3단계 F·G·H: Host 준비·전송·timeout/재전송 · Client 해시 대조 순서 계약 · 결말 1줄(`_outcomeLogged`) ·
  ...
  🔴 **`ForceWin` 이 Host 에서 `OnGameEnd` 를 발행하지 않던 기존 버그(2026-09-21 수정):
  발행 누락의 피해자가 UI 하나가 아니라 **전투 틱 정지**까지 둘이라는 점 ·
  구독자의 로그 한 줄로 「발행이 없었다」를 증명하는 법 · 공유 RPC 의 한 발신자를 고칠 때
  나머지 발신자를 그 자리에서 전수 열거해야 하는 이유(같은 함정 3회째) ·
  `OnGameEnd` 발행과 이탈 감시 켜기는 서로 독립이라는 점.**
  🔴 **결과 화면 재경기의 역할(Host/Client) 비대칭 제거(2026-09-22): 이탈 확정 뒤 재경기 요청·수락을
  서버가 처리하지 않게 막는 깃발을 **두 자리**에 세우는 이유 · 수락 접수 통보 ClientRpc 를
  맵 준비 **앞**에 보내는 이유 · 🔴 구독자가 둘인 UniRx 채널에서 앞 구독자의 예외가 뒤 구독자를
  통째로 막는다는 것과 그것이 하필 고치려는 그 상황에서 터진다는 점 ·
  「진행 중 맵 준비 취소」를 일부러 만들지 않은 근거.**
  **네트워크 작업은 여기부터 읽는다.**

...
## 네트워크 종료 시점 가드 8곳 실기 검증 (2026-08-24) — 회귀 없음 / 부분 검증

**대상:** 커밋 `bcf45ec1`(가드 8곳). **판정: 회귀 없음 확인 · 「실기 PASS」 아님**(8곳 중 6곳 발화 확인 불가).
**근거:** `Assets/_Project/Docs/_Logs/_editor/2026-08-24/RuntimeLog.txt` (13,003행). 아래 수치는 전부 직접 재실측.

### 세션 구성 — 한 세션 안에서 역할이 바뀌었다 (이 QA의 핵심 조건)
| 구간 | 행 범위 | 에디터 역할 | 결과 |
|---|---|---|---|
| 1경기 12:13:59~12:16:42 | 1~473 | **호스트** (33행 `IsServer=True, IsHost=True`) | `WinnerTeam=Blue` (447행) |
| 2경기 12:17:15~12:19:31 | 474~5540 | **클라이언트** (481행) | `AnnounceWinnerClientRpc 수신` `WinnerTeam=Red` (5536행) |
| 3경기 12:19:41~12:22:23 | 5541~13003 | **클라이언트** (5541행) | 〃 (12993행) |

전부 `IsRandomMatch=True`. **과거 실기 로그는 전부 호스트 쪽**이었다 — 2026-08-19 로그(1,408행) 재실측: `네트워크 스폰 | IsServer=False` **0건** · `IsServer=True` **30건**.

### 핵심 지표
- `[ERROR]` **0건**(13,003행) · 승자 발표 **3회** · `게임 종료 — 전투 틱 정지` **1경기에 정확히 1회**(446행) · `NetworkControllerSpawnedWithoutGameServices` **0건** · `Event=` **0건**.
- 사용자 육안 확인: *"특별히 문제없어보였어"*.

### 🔴 부분 검증인 이유 — 8곳 중 서버 발화가 로그로 확인된 것은 2곳
가드에 로그를 0건 추가하기로 한 설계(Plan §6-4)라 *"가드를 통과했다"* 를 직접 셀 수 없다. 셀 수 있는 것은 **가드 아래 본문의 기존 로그**뿐이다.
- ✅ ③ `NetworkGameEndController.OnGameEndServer`(178행) → `서버: 게임 종료 감지 — 결과 전파 시작` 447행 **1회**
- ✅ ⑧ `NetworkProductionController.OnUnitProduced`(319행) → `서버 유닛 생산 완료` 1경기 **189회**
- ⚠️ ①`NetworkResourceSync`(188) ②`NetworkTileSync`(164) ④⑤`NetworkHealthSync`(131·186) ⑥⑦`NetworkProductionController`(209·253) → **호출당 로그 없음 = 발화 횟수 셀 수 없음**

**⚠️ ①② 의 근거를 섞으면 틀린다** — 2·3경기의 `클라이언트 골드를 서버 값으로 보정` **6,168건**(516행~) · `타일 동기화 수신` **733건**(492행~)은 **상대 호스트가 보낸 것을 받은 기록**이라 **에디터의 서버 측 가드를 통과한 트래픽이 아니다**(상대 빌드의 커밋을 로그로 확인 불가 → 규칙 10). 서버 쪽 근거는 1경기의 `서버 모드로 골드 동기화 시작`(37행)·`서버 모드로 타일 소유권 동기화 시작 | IsServer=True`(43행)=**분기 진입·구독 성립**과, `서버 유닛 생산 완료` 189회+2분43초 정상 종료=**골드가 실제로 흘렀다**는 간접 근거까지다.

### 🔴 `_combatStopped` 재경기 리셋 — **이 세션으로 검증되지 않았다 (미검증 유지)**
가드가 `NetworkCombatController.cs` **380행 `if (!IsSpawned || !IsServer || _combatStopped) return;`** 이라, **2·3경기처럼 에디터가 클라이언트면 `_combatStopped` 를 평가하기 전에 `!IsServer` 에서 반환**된다.
실측 내역 — 1경기(서버) `서버: 유닛 사망` **129** · `서버: 건물 사망` **5** / 2경기(클라) `EntityDiedClientRpc 수신` **64** → 유닛 **62** · 건물 **2** / 3경기(클라) 수신 **179** → 유닛 **175** · 건물 **4**. `NetworkCombatController` 총 **137 / 129 / 359**.
2·3경기에 `서버: 유닛 사망` **0건** · `서버 측 … 이벤트 구독 완료` **0건** = **상대 호스트의 틱이 돈 것**이다.
> **확인 조건: 에디터가 호스트로 연속 2경기.** ⚠️ *"판정 로직은 그 로직이 실제로 실행되는 조건까지 확인할 것"*(`.claude/MEMORY.md` MistShrine 교훈 ①)과 같은 함정.

### 🔴 `[WARN]` 1,421건 — 새로 생긴 문제가 아니라 「알려진 현상(정상 동작)」
내역: `InitializeUnitView: 해당 UnitId 의 GameObject 가 아직 없다` **1,099** + `SpawnUnitClientRpc — UnitView 초기화 지연. 재시도 대기` **319** + 연결 끊김/재접속 **3**.
1. 재시도 **319건 ↔ 성공 319건**, **소진·실패 0건**. 대기 시간 실측 **0.01초 1 / 0.02초 51 / 0.03초 192 / 0.04초 70 / 0.05초 3 / 0.06초 2**(최빈 0.03, 최대 0.06).
2. 1,099건 **전부 클라 전환 뒤** — 첫 937행 `12:17:35.694`, 마지막 12917행. **1경기 구간 0건.**
3. `SpawnUnitClientRpc` 수신부는 **클라 전용**, `bcf45ec1` 가드는 **전부 서버 측**, 수신부 `if (IsServer) return;` 10곳 무변경.
4. 경고 코드는 **2026-07-19부터 존재**(※ `git log -S` = 호출 세션 측정값, 규칙 5로 재검증 안 함).
> `LogEvent.UnitViewInitializeTimeout` 은 enum 에 있으나 이 회차 `Event=` **0건** — **재시도가 전부 성공해 타임아웃에 이른 적이 없기 때문**이다. 이 키가 찍히는 회차가 오면 그때는 **실제 실패**다.

### 미해결로 남긴 것
- **B(엔진 오류 수집) 3차 재확인에도 미검증** — `[ERROR]` 0건이라 잡을 대상이 없었다(*수정이 잘 돼서 검증 기회가 사라지는 역설*).
- **스폰 레이스 미재현** — `NetworkControllerSpawnedWithoutGameServices` 0건(위 「알려진 취약 지점」의 지연 재조회 패턴 검증 기회 없음).

### 인계 수치가 실측과 어긋난 3건 (옮겨 적지 않고 실측값 사용)
유닛 사망 `135/128/358` → 위 실측값 / 재시도 대기 *"30ms 안에"* → **0.01~0.06초** / `서버 모드로 골드 동기화 시작` 에 `| IsServer=True` 접미 **없음**(43행 `NetworkTileSync` 쪽에만 있다).

task: `_Tasks/2026-08-20/12_24_network-guard-sweep/Plan.md` §14 · `LogRules.md` 1.13「2026-08-24」.
